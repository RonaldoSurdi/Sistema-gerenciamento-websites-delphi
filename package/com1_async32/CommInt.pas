...
type
  ...
  public
    ...
    function Write(var Buf; Count: Integer): Cardinal;
    function Read(var Buf; Count: Integer): Cardinal;
...
function TCustomComm.Write(var Buf; Count: Integer): Cardinal;
var
  Overlapped: TOverlapped;
  ErrorCode: Integer;
begin
  ...
  try
    FillChar(Overlapped, Sizeof(Overlapped), 0);
    Overlapped.hEvent := FEvent.Handle;
    if not WriteFile(FHandle, Buf, Count, Result, @Overlapped) and (GetLastError <> ERROR_IO_PENDING) then
    begin
...
function TCustomComm.Read(var Buf; Count: Integer): Cardinal;
var
  Overlapped: TOverlapped;
  ErrorCode: Integer;
begin
  ...
  try
    FillChar(Overlapped, Sizeof(Overlapped), 0);
    Overlapped.hEvent := FEvent.Handle;
    if not ReadFile(FHandle, Buf, Count, Result, @Overlapped) and (GetLastError <> ERROR_IO_PENDING) then
    begin
      ErrorCode := GetLastError;
...
begin
  ...
  if GetCommConfig(Handle, CommConfig, Size) then
    Result := CommConfig.dwProviderSubType
  else Result := -1;
end;
