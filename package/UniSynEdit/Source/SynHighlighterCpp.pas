...
$Id: SynHighlighterCpp.pas,v 1.22.2.4 2005/07/01 17:17:40 maelh Exp $
...
type
  ...
  private
    ...
    function Func__asm(Index: Integer): TtkTokenKind;
    function Func_asm(Index: Integer): TtkTokenKind;
...
begin
  ...
  fIdentFuncTable[70] := Func__asm;
  fIdentFuncTable[191] := Func_asm;
...
function TSynCppSyn.Func__asm(Index: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords[Index]) then
  begin
    Result := tkKey;
...
function TSynCppSyn.Func_asm(Index: Integer): TtkTokenKind;
begin
  if IsCurrentToken(KeyWords[Index]) then
  begin
    Result := tkKey;
  ...
  case FLine[Run + 1] of
    '/':                               {c++ style comments}
      begin
        fTokenID := tkComment;
        inc(Run, 2);
        while not IsLineEnd(fLine[Run]) do Inc(Run);
      end;
    '*':                               {c style comments}
      begin
        fTokenID := tkComment;
...
begin
  ...
  repeat
    ...
    inc(Run);
  until IsLineEnd(fLine[Run]) or (fLine[Run] = #34);
