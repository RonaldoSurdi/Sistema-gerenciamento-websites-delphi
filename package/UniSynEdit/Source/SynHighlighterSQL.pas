...
$Id: SynHighlighterSQL.pas,v 1.39.2.8 2005/07/20 13:39:17 maelh Exp $
...
//---MySQL----------------------------------------------------------------------
  // keywords
  MySqlKW: WideString =
    'ACTION,AFTER,AGAINST,ALGORITHM,AGGREGATE,ALL,ALTER,ANALYZE,AND,AS,ASC,' +
    'AVG_ROW_LENGTH,BACKUP,BEFORE,BEGIN,BENCHMARK,BETWEEN,BINLOG,BIT,BOOL,' +
    'BOTH,BY,CALL,CASCADE,CASCADED,CHANGE,CHARACTER,CHARSET,CHECK,CHECKSUM,' +
    'COLLATE,COLLATION,COLUMN,COLUMNS,COMMENT,COMMIT,CONSISTENT,CONSTRAINT,' +
    'CONTENTS,CREATE,CROSS,DATA,DATABASE,DATABASES,DEC,DEFAULT,DELAYED,' +
    'DELAY_KEY_WRITE,DELETE,DESC,DESCRIBE,DISTINCT,DISTINCTROW,DIV,DROP,' +
    'DUPLICATE,EACH,ELSE,ENCLOSED,END,ENGINE,ENGINES,ESCAPE,ESCAPED,ERRORS,' +
    'EVENTS,EXISTS,EXPLAIN,FIELDS,FILE,FIRST,FLOAT4,FLOAT8,FLUSH,FOR,FOREIGN,' +
    'FROM,FULL,FULLTEXT,FUNCTION,FUNCTIONS,GLOBAL,GRANT,GRANTS,GROUP,HAVING,' +
    'HELP,HIGH_PRIORITY,HOSTS,IDENTIFIED,IGNORE,INDEX,INFILE,INNER,INSERT,' +
    'INT1,INT2,INT3,INT4,INT8,INTO,IS,ISAM,ISOLATION,JOIN,KEY,KILL,LEADING,' +
    'LEVEL,LIKE,LIMIT,LINES,LOAD,LOCAL,LOCK,LOGS,LONG,LOW_PRIORITY,MASTER,' +
    'MASTER_LOG_FILE,MASTER_LOG_POS,MATCH,MAX_ROWS,MERGE,MIDDLEINT,MIN_ROWS,' +
    'MOD,MODIFY,NAMES,NATURAL,NO,NOT,NULL,OPTIMIZE,OPTION,OPTIONALLY,ON,OPEN,' +
    'OR,ORDER,OUTER,OUTFILE,PACK_KEYS,PARTIAL,PRECISION,PRIMARY,PRIVILEGES,' +
    'PROCEDURE,PROCESS,PROCESSLIST,READ,REFERENCES,REGEXP,RELAY_LOG_FILE,' +
    'RELAY_LOG_POS,RELOAD,RENAME,REPAIR,RESTRICT,RESTORE,RETURN,RETURNS,' +
    'REVOKE,RLIKE,ROLLBACK,ROLLUP,ROUTINE,ROW,ROWS,SCHEMA,SCHEMAS,SELECT,SET,' +
    'SHOW,SHUTDOWN,SLAVE,SONAME,SQL_BIG_RESULT,SQL_BIG_SELECTS,' +
    'SQL_BIG_TABLES,SQL_LOG_OFF,SQL_LOG_UPDATE,SQL_LOW_PRIORITY_UPDATES,' +
    'SQL_SELECT_LIMIT,SQL_SMALL_RESULT,SQL_WARNINGS,SESSION,SNAPSHOT,START,' +
    'STARTING,STATUS,STOP,STRAIGHT_JOIN,TABLE,TABLES,TEMPORARY,TEMPTABLE,' +
    'TERMINATED,THEN,TO,TRAILING,TRANSACTION,TRIGGER,TYPE,UNDEFINED,UNIQUE,' +
    'UNLOCK,UPDATE,USAGE,USE,USING,VALUES,VARIABLES,VARYING,VIEW,WARNINGS,' +
  ...
  MySqlTypes: WideString =
    'auto_increment,bigint,binary,blob,char,date,datetime,decimal,double,' +
    'enum,float,geometry,geometrycollection,int,integer,linestring,longblob,' +
    'longtext,mediumblob,mediumint,mediumtext,multilinestring,multipoint,' +
    'multipolygon,national,numeric,point,polygon,real,smallint,string,text,' +
    'time,timestamp,tinyblob,tinyint,tinytext,unsigned,varbinary,varchar,' +
    'year,zerofill';
  ...
  MySqlFunctions: WideString =
    ...
    'FOUND_ROWS,FROM_DAYS,FROM_UNIXTIME,GET_FORMAT,GET_LOCK,GREATEST,' +
    ...
    'RADIANS,RAND,RELEASE_LOCK,REPEAT,REPLACE,REVERSE,RIGHT,ROUND,ROW_COUNT,' +
    'RPAD,RTRIM,SECOND,SEC_TO_TIME,SESSION_USER,SIGN,SIN,SOUNDEX,SPACE,' +
    ...
    'UPPER,USER,UTC_DATE,UTC_TIME,UTC_TIMESTAMP,UUID,VARIANCE,VAR_POP,' +
    'VAR_SAMP,VERSION,WEEK,WEEKDAY,WEEKOFYEAR,WHEN,YEARWEEK,YEAR_MONTH';
