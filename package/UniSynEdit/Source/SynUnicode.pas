...
$Id: SynUnicode.pas,v 1.1.2.19 2005/09/24 22:08:40 maelh Exp $
...
- An adapted and cleansed version of TWideStrings/TWideStringList taken
...
{$IFNDEF USE_JCL_UNICODE_SUPPORT}
const
  WideNull = WideChar(#0);
...
{$IFNDEF SYN_CLX}
function IsCharMsgAlreadyUnicode: Boolean;
{$ENDIF}
{$IFDEF SYN_WIN32}
...
{$IFNDEF SYN_CLX}
type
  TUnicodeControl = class(TWinControl)
  private
    FCharMsgIsUnicode: Boolean;
  protected
    procedure CreateParams(var Params: TCreateParams); override;
    procedure CreateWnd; override;
    procedure WMChar(var Msg: TWMChar); message WM_CHAR;
  public
    property CharMsgIsUnicode: Boolean read FCharMsgIsUnicode;
  end;

{ TUnicodeControl }

const
  // 511 has no special meaning it is just a number bigger than 255
  // (the maximum AnsiChar)
  WideCharTestValue = 511;

procedure TUnicodeControl.CreateParams(var Params: TCreateParams);
begin
  inherited;
  // Necessary for unicode support, especially IMEs won't work else
  Params.WindowClass.lpfnWndProc := @DefWindowProcW;
end;

procedure TUnicodeControl.CreateWnd;
begin
  inherited;
...
procedure TUnicodeControl.WMChar(var Msg: TWMChar);
begin
  if Msg.CharCode = WideCharTestValue then
    FCharMsgIsUnicode := True;
end;

var
  CheckedForUnicode: Boolean;
  CharMsgAlreadyUnicode: Boolean;

function IsCharMsgAlreadyUnicode: Boolean;
begin
  if CheckedForUnicode then
  begin
    Result := CharMsgAlreadyUnicode;
    exit;
  end;
...
{$ELSE}
  Result := True;
{$ENDIF}
  CharMsgAlreadyUnicode := Result;
  CheckedForUnicode := True;
end;
{$ENDIF}
