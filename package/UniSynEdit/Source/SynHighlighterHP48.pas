...
$Id: SynHighlighterHP48.pas,v 1.10.2.4 2005/07/01 17:17:41 maelh Exp $
...
type
  ...
  public
    ...
    procedure SetLine(const NewValue: WideString; LineNumber: Integer); override;
...
procedure TSynHP48Syn.SetLine(const NewValue: WideString; LineNumber: Integer);
begin
  fLine := NewValue;
  Run := 1;
  fEol := False;
...
begin
  if fRange = rsRpl then
    ...
    Result := tkAsmComment;
    while (run <= Length(fLine)) do
      if FLine[Run] in [WideChar(#10), WideChar(#13)] then begin
        inc(Run);
        break;
      end
      else
...
begin
  inc(Run);
  while (Run <= Length(FLine)) and (FLine[Run] in [WideChar(#1)..WideChar(#32)]) do
    inc(Run);
...
begin
  ...
  if Run > Length(fLine) then
    ...
    result := sasmproc3
  else if fLine[Run] in [WideChar(#1)..WideChar(#32)] then
    result := SpaceProc
