...
$Id: GenLex.pas,v 1.4.2.2 2005/02/24 21:02:22 maelh Exp $
...
begin
  inc(Walker);
  while Identifiers[fOrigin[Walker]] do
    inc(Walker);
end;
...
begin
  if fTokenizing then
    if not (FOrigin[Walker - 1] in [#10, #13]) then
      FLinePosList.Add(Walker);
end;
...
begin
  while fOrigin[Walker] in [#1..#9, #11, #12, #14..#32] do
    inc(Walker);
end;
