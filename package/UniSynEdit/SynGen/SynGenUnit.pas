...
$Id: SynGenUnit.pas,v 1.18.2.6 2005/07/01 17:29:44 maelh Exp $
...
{$IFDEF SYN_COMPILER_6_UP}
  StrUtils,
{$ENDIF}
  Registry, SynUnicode, HashTableGen;
...
function StuffString(const Value: AnsiString): AnsiString;
var
  i: Integer;
begin
  Result := '';
...
function FirstLetterCap(S: WideString): WideString;
begin
...
function ToAlphaNum(S: WideString): AnsiString;
var
  c: AnsiChar;
begin
  Result := S;
  for c := #33 to #47 do
    Result := AnsiReplaceStr(Result, c, IntToStr(Ord(c)));
...
function IsASCIIAlphaNum(S: WideString): Boolean;
var
  i: Integer;
begin
  Result := True;
...
begin
  ...
  Writeln(OutFile, '    function GetSampleSource: WideString; override;');
  ...
  Writeln(OutFile, '    class function GetFriendlyLanguageName: WideString; override;');  
  ...
  if ChkGetKeyWords.Checked then
    Writeln(OutFile, '    function GetKeyWords(TokenKind: Integer): WideString; override;');
  ...
  begin
    ...
    Writeln(OutFile, '  while (FLine[Run] <= #32) and not IsLineEnd(FLine[Run]) do inc(Run);');
    Writeln(OutFile, 'end;');
    Writeln(OutFile);
  end;
  ...
  begin
    ...
    Writeln(OutFile, sPrefix, '    if not IsLineEnd(fLine[Run]) then');
    Writeln(OutFile, sPrefix, '      Inc(Run);');
    Writeln(OutFile, sPrefix, '  until IsLineEnd(fLine[Run]);');
    Writeln(OutFile, sPrefix, 'end;');
    if TLexEnclosedBy(EnclosedList[I]).MultiLine then
    begin
      Writeln(OutFile, '  end;');
  ...
  if ChkGetKeyWords.Checked then
  begin
    Writeln(OutFile, 'function ' + LexName + '.GetKeyWords(TokenKind: Integer): WideString;');
    Writeln(OutFile, 'begin');
  ...
  Writeln(OutFile, 'function ' + LexName + '.GetSampleSource: WideString;');
  Writeln(OutFile, 'begin');
  ...
  Writeln(OutFile, 'class function ' + LexName + '.GetFriendlyLanguageName: WideString;');
  Writeln(OutFile, 'begin');
