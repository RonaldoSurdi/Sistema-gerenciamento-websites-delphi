...
type
  TPercentage        = 0..100;
  THWScompressMode        = (bmAll, bmIncremental);
  ...
  THWScompressErrorEvent  = procedure(Sender : TObject; const Error: integer; ErrString: string) of object;

  THWScompress = class(TComponent)
  private
    FHWScompressTitle: string;
    fMaxSize: Integer;
    fHWScompressMode: THWScompressmode;
    ...
    fOnError: THWScompressErrorEvent;
    function  GetVersion: string;
    procedure SetVersion(dummy: string);
    procedure SetHWScompressMode(value: THWScompressmode);
...
//    old compression method used in version 1
  ...
  public
    ...
    function  HWScompressToStream(const Filelist: TStrings; Target: TStream): boolean;
    function  HWScompress(const Filelist: TStrings; Target: string): boolean;
  ...
  published
    ...
    property HWScompressTitle: string read fHWScompressTitle write fHWScompressTitle;
    property HWScompressMode: THWScompressmode read fHWScompressMode write SetHWScompressMode;
    ...
    property OnError: THWScompressErrorEvent read fOnError write fOnError;
...
function THWScompress.GetVersion: string;
begin
  Result := cVersion;
end;

procedure THWScompress.SetVersion(dummy: string);
begin
end;

procedure THWScompress.SetHWScompressMode(value: THWScompressmode);
begin
  if value <> fHWScompressMode then
  begin
    if value = bmIncremental then SetArchiveFlag := true;
    fHWScompressMode := value;
  end;
end;

function THWScompress.HWScompressToStream(const Filelist: TStrings; Target: TStream): boolean;
begin
  Result := False;
  if Target <> nil then
  try
    ...
    fMaxSize := 0;
    Result := HWScompress(Filelist, ':STREAM');
  except
  end;
end;

function THWScompress.HWScompress(const Filelist: TStrings; Target: string): boolean;
const
...
//      if  (DiskFree(0)-10*1024) < size then break;
  ...
  begin
    ...
    begin
      ...
      begin
        if HWScompressMode = bmAll then Files.add(S)
...
begin
  ...
  try
    ...
    L := length(fHWScompressTitle);
    TStr.writeBuffer(L, sizeof(L));    //Size of title
    TStr.writeBuffer(PChar(fHWScompressTitle)^,L);     //title
    TStr.writeBuffer(fSizeTotal, sizeof(fSizeTotal));    //Total Size of HWScompress
...
//        while (Size > 0) and Continue do HWScompress; //zero file size bug
          if Continue then
          repeat
              HWScompress;
  ...
  try
    L := 0;
    TStr.writeBuffer(L, sizeof(L));    //end HWScompress
    result := true;
  except
    fLastErr := idCantwriteArchive;
    Continue := false;
...
procedure THWScompress.MessageError(err: integer);
var
   S: string;
begin
  case err of
...
function THWScompress.CompressionRate: integer;
begin
  try
...
function THWScompress.RestoreFromStream(Source: TStream; TargetPath: string): boolean;
begin
  Result := False;
  if Source <> nil then
  try
...
function THWScompress.Restore(Source: String; TargetPath: string): boolean;
var
   L: Integer;
   FA, FAT: integer;
   S, Disk: String;
...
function THWScompress.Busy: boolean;
begin
   Result := IsBusy;
end;

procedure THWScompress.Stop;
begin
   Continue := false;
end;

function THWScompress.GetArchiveTitleFromStream(Source: TStream; var Filelist: TStringlist): string;
begin
  if Source <> nil then
  try
    fInputStream := Source;
...
function THWScompress.GetArchiveTitle(const Source: String; var Filelist: TStringlist): string;
var
  L: Integer;
  S: String;
  SStr: TFilestream;
...
procedure THWScompress.DeCompress(InStream, OutStream: TStream; DoWrite: Boolean);
var
  InBuffer:  array[0..BufferSize-1] of Byte;
  OutBuffer: array[0..BufferSize-1] of Byte;
...
{procedure THWScompress.DeCompressOldMethod(InStream, OutStream: TStream; DoWrite: Boolean);
var
...
procedure Register;
begin
  RegisterComponents('HWS', [THWScompress]);
end;
