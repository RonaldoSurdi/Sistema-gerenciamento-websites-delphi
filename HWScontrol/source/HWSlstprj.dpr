...
uses
  ...
  umdl_lstprj in 'umdl_lstprj.pas' {mdl_lstprj},
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '')then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_lstprj, mdl_lstprj);
                if (ParamStr(5) <> '') then
                    mdl_lstprj.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_lstprj.xiniCodab:= 0;
             end else begin
