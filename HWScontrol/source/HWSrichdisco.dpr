...
uses
  ...
  umdl_richdisco in 'umdl_richdisco.pas' {mdl_richdisco},
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') and (ParamStr(4) <> '') then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richdisco, mdl_richdisco);
                if (ParamStr(5) <> '') then
                    mdl_richdisco.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richdisco.xiniCodab:= 0;
             end else begin
