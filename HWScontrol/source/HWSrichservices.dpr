...
uses
  ...
  umdl_richservices in 'umdl_richservices.pas' {mdl_richservices},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol Notícias';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol Catálogo Serviços';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') and (ParamStr(4) <> '') then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richservices, mdl_richservices);
                if (ParamStr(5) <> '') then
                    mdl_richservices.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richservices.xiniCodab:= 0;
             end else begin
