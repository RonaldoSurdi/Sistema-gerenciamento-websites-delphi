...
uses
  ...
  uadm_guestbook in 'uadm_guestbook.pas' {adm_guestbook};
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol guestbook';
    loadx.Show;
    loadx.Update;
    ...
    HprevHist := FindWindow(Nil, PChar('HWScontrol guestbook'));
    if HprevHist = 0 then begin
    Application.Title := 'HWScontrol guestbook';
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             Application.CreateForm(Tadm_guestbook, adm_guestbook);
          end else begin
