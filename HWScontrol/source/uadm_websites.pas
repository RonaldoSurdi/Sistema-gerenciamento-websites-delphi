...
uses
  ...
  ImgList, XPMan, Menus, jpeg, Tradutor, AppEvnts;
...
begin
     ...
     adm_websites.Caption := AxDadosUsr.axtitle;
     ...
     lb_razaosocial.Caption:=AxDadosUsr.razao_social;
     DBGrid1.Columns.Add;
     ...
     With dm.ds_sql2 do begin
        ...
        Open;
        Filter:='cod_ent='+#39+IntToStr(AxDadosUsr.cod_ent_1)+#39;
        Filtered:=true;
     end;
     ...
     With dm.ds_sql4 do begin
        CommandText:= 'SELECT codigo,descricao,obs FROM '+EnDecryptString(Serverconect.vrf24,37249)+
                      ' WHERE cp2='+#39+'0'+#39+
                      ' ORDER BY descricao';
        Open;
        Filter:='';
     ...
     With dm.ds_sql5 do begin
        CommandText:='SELECT * FROM '+EnDecryptString(Serverconect.vrf32,37249)+' ORDER BY codigo';
        Open;
        Filter:='cod_web='+#39+'0'+#39;
        Filtered:=true;
     end;
     With dm.ds_sql11 do begin
        CommandText:='SELECT * FROM '+EnDecryptString(Serverconect.vrf32,37249)+'_ax ORDER BY codigo';
        Open;
        Filter:='cod_web='+#39+'0'+#39;
        Filtered:=true;
     end;
...
begin
      ...
      if dm.ds_sql3.RecordCount <= 0 then begin
         ...
         with dm.ds_sql3 do begin
            ...
            montaskins(InttoStr(AxDadosUsr.cod_ent_1));
...
begin
     ...
     With dm.executa7 do begin
        CommandText:='SELECT codigo,logo FROM '+EnDecryptString(Serverconect.vrf30,37249)+' WHERE cod_web='+#39+lbcp1.Text+#39;
        Open;
...
begin
     if (lbcp4.Text='') or (lbcp5.Text='') then begin
        hwsf.BtMensagem('É necessário selecionar a entidade de registro do domínio.');
        lbcp5.SetFocus;
        exit;
     end;
     if (lbcp6.Text='') or (lbcp7.Text='') then begin
        hwsf.BtMensagem('É necessário selecionar o ramo de atividade.');
        lbcp7.SetFocus;
        exit;
     end;
     if (lbcp9.Text='') or (lbcp10.Text='') then begin
        hwsf.BtMensagem('É necessário selecionar o skin para o website.');
        lbcp10.SetFocus;
        exit;
     end;
     if lbcp8.Text='' then begin
        hwsf.BtMensagem('É necessário digitar o website.');
        ScrollBox1.VertScrollBar.Position:=62;
        lbcp8.SetFocus;
        exit;
     end;
     if lbcp1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf29,37249),'codigo','');
        with dm.executa do begin
            ...
            Params[1].AsInteger:=AxDadosUsr.cod_ent_1;
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf29,37249)+' SET cod_ent_reg=:p0,cod_ar=:p1,cod_mdl=:p2,website=:p3 '+
                       'WHERE codigo=:p4';
            Params[0].AsInteger:=StrToInt(lbcp4.Text);
            Params[1].AsInteger:=StrToInt(lbcp6.Text);
            Params[2].AsInteger:=StrToInt(lbcp9.Text);
            Params[3].AsString:=lbcp8.Text;
     ...
     //gravar logo
     if lbcp1.Text = 'novo' then begin
        axcod2:=dm.GetCodMax(EnDecryptString(Serverconect.vrf30,37249),'codigo','');
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf30,37249)+' (codigo,cod_web,logo) '+
                       'VALUES(:p0,:p1,:p2)';
            Params[0].AsInteger:=axcod2;
            Params[1].AsInteger:=axcod;
            xlogotmp.Seek(0,soFromBeginning);
            Params[2].LoadFromStream(xlogotmp,ftblob);
        end;
     end else begin
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf30,37249)+' SET logo=:p0 '+
                       'WHERE cod_web=:p1';
            xlogotmp.Seek(0,soFromBeginning);
...
begin
     ...
     if marcar then begin
        With dm.executa do begin
            axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf32,37249),'codigo','');
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf32,37249)+' (codigo,cod_web,tipo,cod_grp,cod_grp2,obs,xconf) '+
        ...
        loadx.Progressloading.position:=50;
        dm.deleta_reg(EnDecryptString(Serverconect.vrf32,37249),'codigo',InttoStr(axcod),false);
        loadx.Progressloading.position:=100;
     end;
     dm.ds_sql5.Close;
     dm.ds_sql5.Open;
...
begin
     if marcar then begin
        With dm.executa do begin
            axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf49,37249),'codigo','');
...
begin
    ...
    if xgravamdl>0 then begin
       With dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf32,37249)+' SET obs=:p0 '+
                       'WHERE codigo=:p1';
          ...
          if StrtoInt(axlbcd)>0 then begin
            ...
            for xmnmenu:=0 to menutmp.count-1 do begin
              ...
              if Locate('cod_lng',menutmp.cod_lng[xmnmenu],[]) then begin
                 ...
                 xmninc:=true;
                 axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf50,37249),'codigo','');
                 with dm.executa do begin
...
begin
     ...
     if lbcd1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf43,37249),'codigo','');
        with dm.executa do begin
...
begin
     With dm.ds_sql8 do begin
          ...
          CommandText:= 'SELECT * FROM '+EnDecryptString(Serverconect.vrf43,37249)+
                                ' WHERE cod_web='+#39+xcodmenu+#39+' OR cod_sk='+#39+lbcp9.Text+#39+
                                ' ORDER BY descricao';
          Open;
          Filter:='';
          Filtered:=false;
...
procedure Tadm_websites.StatusBar1Click(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_websites.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
