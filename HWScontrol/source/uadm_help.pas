...
uses
  ...
  MPlayer, Tradutor, BmpToJpg, JpgToBmp, Jpeg, AppEvnts;
...
begin
      ...
      ax_searh:=false;
      adm_help.Caption := 'HWScontrol Tópicos de Ajuda';
     xinit:=true;
     ...
     With dm.ds_sql6 do begin
        CommandText:= 'SELECT codigo,mdl FROM '+EnDecryptString(Serverconect.vrf24,37249)+' ORDER BY descricao';
        Open;
        Filter:='';
        Filtered:=true;
        First;
     ...
     With dm.executa3 do begin
        CommandText:='SELECT codigo,tp FROM '+EnDecryptString(HelpSys.vrf03,37249)+' ORDER BY codigo';
        Open;
     end;
     loadx.Progressloading.position:=100;
     re_editor1.Justify;
...
begin
     ...
     With dm.executa5 do begin
          ...
          CommandText:= 'SELECT * FROM '+EnDecryptString(HelpSys.vrf01,37249)+
...
begin
     ...
     With dm.executa do begin
        CommandText:='SELECT img FROM '+EnDecryptString(HelpSys.vrf03,37249)+' WHERE codigo='+#39+InttoStr(xcodref)+#39;
        Open;
    ...
    try
        ...
        if xcoditem>0 then begin
           ...
           With dm.executa4 do begin
                if Active then Close;
                CommandText:= 'SELECT * FROM '+EnDecryptString(HelpSys.vrf02,37249)+
...
procedure Tadm_help.FormShow(Sender: TObject);
begin
    bt_barra.Top:=StrtoInt(formatfloat('0',(barra.Height-bt_barra.Height)/2));
    bt_barra.Left:=barra.Left;
    Label_title.Width:=ToolBar1.Width-87;
...
procedure Tadm_help.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
