...
uses
  ...
  umdl_statgalery in 'umdl_statgalery.pas' {mdl_statgalery},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol StatGallery';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol Stat';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '') then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_statgalery, mdl_statgalery);
                if (ParamStr(5) <> '') then
                    mdl_statgalery.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_statgalery.xiniCodab:= 0;
             end else begin
