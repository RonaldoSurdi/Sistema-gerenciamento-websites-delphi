...
type
  IHWSwsFnApp = interface(IInvokable)
    ...
    //function HWSdbAppDOWNbinSYS(const xcodmdl_value,xnammdl_value: string): TByteDynArray; stdcall;
    //function HWSdbAppDOWNbinSZsys(const xcodmdl_value,xnammdl_value: string): Integer; stdcall;
    ...
    function HWSdbAppDOWNbinSZ(const xtablename,xcpcod,xcpdt,xcpcod_value: string): Integer; stdcall;
...
function Tdm.GetCodMax(xtabela,xcampo,xwhere:string):Integer;
var ax_cod1 : integer;
begin
    Try
...
function Tdm.winsysreg(tipo: byte; xhelpdesk,xhelpsys: boolean):boolean;
var Reg: Tregistry;
...
function Tdm.GetIPcntHWS(var xipup:String):Boolean;
...
function Tdm.conecta_bd(cnt_usr,cnt_pass,xax_parm:string):boolean;
Var x_error,xCommandAut:TstringList;
    ...
    xpos_paramX: Integer;
    xtp_paramX, xcodgrp_paramX: String;
begin
      loadx := Tloadx.Create(Application);
      ...
      With Serverconect do begin
           ...
           Try
                  ...
                  xax_parm_Default:=xax_parm;
                  if (xax_parm <> '') then begin
                      if (Copy(xax_parm,1,1)='*') then begin
                        xpos_paramX:= POS('/',xax_parm);
                        xtp_paramX:=copy(xax_parm,2,xpos_paramX-2);
                        xcodgrp_paramX:=copy(xax_parm,xpos_paramX+1,Length(xax_parm)-2);
                        xax_parm_Default:=xtp_paramX;
                        xax_parmX:=' AND G.cp5='+#39+xax_parm+#39+' AND G.codigo='+#39+xcodgrp_paramX+#39;
                      end else xax_parmX:=' AND G.cp5='+#39+xax_parm+#39;
                  end else xax_parmX:=' AND G.cp5 is null';
...
function Tdm.PostBinaryStream4PH(xcptp,xcpcod,xcpdt: string; xFileData: TMemoryStream): Boolean;
var FileData,FileData2: TByteDynArray;
    txarqDataOut: TMemoryStream;
    axtest: Boolean;
begin
...
function Tdm.sentMailHWS(xHWSfld1,xHWSfld2: TStringList): Integer;
var FileData1,FileData2: TByteDynArray;
    txarqData,txarqDataout: TMemoryStream;
    axtest: Integer;
begin
...
function Tdm.RtfToHtml2(contenthead:string; xhead: Boolean; re_editor_tmp: TRxRichEdit):TStringList;
...
function Tdm.rotateBitmap(xstrTPmem1:TMemoryStream; var xstrTPmem2:TMemoryStream; xTPcompress:Integer; xangleTP:Double): Boolean;
...
function Tdm.rotateFoto(xangulo:Integer; var xImagem_Out: TMemoryStream; var xImagem_size: Integer; var xImagem_UPDT: Boolean;
                                xcpTPmodelo,xcpTP1,xcpTP2,xcpTP3,xcpTP4: String): Boolean;
...
procedure Tdm.HWSconnectionAppHTTPWebNode1ReceivingData(Read,
  Total: Integer);
