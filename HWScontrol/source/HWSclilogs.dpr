...
uses
  ...
  uadm_clilogs in 'uadm_clilogs.pas' {adm_clilogs};
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol logs';
    loadx.Show;
    loadx.Update;
    ...
    HprevHist := FindWindow(Nil, PChar('HWScontrol logs'));
    if HprevHist = 0 then begin
    Application.Title := 'HWScontrol logs';
  ...
  dm.winsysreg(1,false,false);
          if dm.conecta_bd(ParamStr(1),ParamStr(3),'') then begin
             Application.CreateForm(Tadm_clilogs, adm_clilogs);
          end else begin
