...
uses
  ...
  umdl_divulg in 'umdl_divulg.pas' {mdl_divulg},
...
begin
    ...
    Application.Title := 'HWScontrol Divulgação';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '')then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_divulg, mdl_divulg);
                if (ParamStr(5) <> '') then
                    mdl_divulg.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_divulg.xiniCodab:= 0;
             end else begin
