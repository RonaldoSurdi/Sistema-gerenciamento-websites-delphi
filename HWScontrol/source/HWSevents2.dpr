...
uses
  ...
  umdl_events2 in 'umdl_events2.pas' {mdl_events2},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol Eventos';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol Events';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '')then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_events2, mdl_events2);
                if (ParamStr(5) <> '') then
                    mdl_events2.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_events2.xiniCodab:= 0;
             end else begin
