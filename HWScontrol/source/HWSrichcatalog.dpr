...
uses
  ...
  umdl_richcatalog in 'umdl_richcatalog.pas' {mdl_richcatalog},
...
begin
    ...
    Application.Title := 'HWScontrol Catálogo de Produtos';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') and (ParamStr(4) <> '') then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richcatalog, mdl_richcatalog);
                if (ParamStr(5) <> '') then
                    mdl_richcatalog.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richcatalog.xiniCodab:= 0;
             end else begin
