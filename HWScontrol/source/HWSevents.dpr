...
uses
  ...
  umdl_events in 'umdl_events.pas' {mdl_events},
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_events, mdl_events);
                if (ParamStr(5) <> '') then
                    mdl_events.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_events.xiniCodab:= 0;
             end else begin
