...
uses
  ...
  Tradutor, RxRichEd, ImgList, AppEvnts, ShellApi;
...
type
  ...
  Tmdl_survey = class(TForm)
    ...
    TabSheet4: TTabSheet;
    Label25: TLabel;
    axdt1: TDateTimePicker;
    axdt2: TDateTimePicker;
    Label26: TLabel;
    axdt3: TDateTimePicker;
    axdt4: TDateTimePicker;
    Panel11: TPanel;
    ToolBar17: TToolBar;
    DBGrid4: TDBGrid;
    export_rel: TSpeedButton;
    TabSheet5: TTabSheet;
    xrellb2: TLabel;
    xrellb1: TLabel;
    atualiza_rel: TSpeedButton;
    xrellb3: TLabel;
    ApplicationEvents1: TApplicationEvents;
    TabSheet6: TTabSheet;
    SaveDialog1_exporthtml: TSaveDialog;
    DBGrid5: TDBGrid;
    Panel23: TPanel;
    ToolBar19: TToolBar;
    atualiza_men: TSpeedButton;
    DBMemo_men: TDBMemo;
    Splitter7: TSplitter;
    Lb_countreg_men: TLabel;
    CheckBox_seccao: TCheckBox;
    Memo_result: TMemo;
    Splitter8: TSplitter;
    ...
    procedure atualiza_relClick(Sender: TObject);
    procedure ApplicationEvents1Exception(Sender: TObject; E: Exception);
    procedure export_relClick(Sender: TObject);
    procedure atualiza_menClick(Sender: TObject);
  private
    //menus
    ...
    function ColorToHtml_rel(mColor: TColor): string;
    function StrToHtml_rel(mStr: string; mFont: TFont = nil): string;
    function DBGridToHtmlTable_rel(mDBGrid: TDBGrid; mStrings: TStrings; mCaption: TCaption = ''): Boolean;
...
procedure Tmdl_survey.FormClose(Sender: TObject; var Action: TCloseAction);
...
begin
     ...
     dm.ds_sql6.Close;
     if dm.ds_sql9.Active then begin
        dm.ds_sql9.Filter:='';
...
procedure Tmdl_survey.FormCreate(Sender: TObject);
var axshowing: Boolean;
...
begin
     ...
     mdl_survey.Caption:= AxDadosUsr.axtitle;
     xinit:=true;
     axcod_ent:=0;
...
procedure Tmdl_survey.monta_lng;
...
procedure Tmdl_survey.monta_table1;
var axshowing: Boolean;
...
procedure Tmdl_survey.monta_table2(xnewgal,xuplng:Boolean);
var xcodgal: string;
begin
...
procedure Tmdl_survey.monta_relatorio();
var xcodrel: string;
    sumtotal,sumtotal2,sumtotal3,xcod1,xcod2: Integer;
    xdescnm: String;
begin
     DBGrid4.DataSource:=dm.source_sql9;
     DBGrid4.Columns.Clear;
     Memo_result.Lines.Clear;
     Memo_result.Lines.Add('RELATÓRIO DE VOTOS');
     With DBGrid4.Columns do begin
          Add;
          Items[0].Title.Caption:='Código';
          Items[0].FieldName:='cod_img';
          Items[0].Width:=60;
          Add;
          Items[1].Title.Caption:='Votos';
          Items[1].FieldName:='COUNT(*)';
          Items[1].Width:=60;
          Add;
     ...
     With dm.ds_sql9 do begin
          if Active then Close;
          xcodrel:=dm.ds_sql.fieldbyname('cod_gal').AsString;
          if Active then Close;
          if CheckBox_seccao.Checked then begin
             CommandText:= 'SELECT codigo,cod_img,descricao,COUNT(*) FROM '+LstTables.Table[2]+'_ax'+
                            ' WHERE cod_gal='+#39+xcodrel+#39+
                            ' GROUP BY cod_img,ax1 '+
                            ' ORDER BY cod_img';//distinct(ax1)
              Open;
              Filtered:=false;
              sumtotal:=0;
              sumtotal2:=0;
              ...
              First;
              xcod1:=0;
              xcod2:=fieldbyname('cod_img').AsInteger;
              xdescnm:=fieldbyname('descricao').AsString;
              While not Eof do begin
                  xcod1:=fieldbyname('cod_img').AsInteger;
                  if (xcod1 <> xcod2) then begin
                      Memo_result.Lines.Add(xdescnm+'= '+Inttostr(sumtotal2)+' votos');
                      sumtotal3:=sumtotal3+sumtotal2;
                      xdescnm:=fieldbyname('descricao').AsString;
                      xcod2:=xcod1;
                      sumtotal2:=0;
                  end else begin
                      if (fieldbyname('COUNT(*)').AsInteger>10) then Inc(sumtotal2)
                      else sumtotal2:=sumtotal2+fieldbyname('COUNT(*)').AsInteger;
                  end;
                  sumtotal:=sumtotal+fieldbyname('COUNT(*)').AsInteger;
                  Next;
              end;
              sumtotal3:=sumtotal3+sumtotal2;
              Memo_result.Lines.Add(xdescnm+'= '+Inttostr(sumtotal2)+' votos');
              ...
              Memo_result.Lines.Add('TOTAL DE VOTOS (SECÇÃO): '+Inttostr(sumtotal3));
              if Active then Close;
              CommandText:= 'SELECT codigo,cod_img,descricao,COUNT(*) FROM '+LstTables.Table[2]+'_ax'+
                            ' WHERE cod_gal='+#39+xcodrel+#39+
                            ' GROUP BY cod_img '+
              ...
              First;
              While not Eof do begin
                  Memo_result.Lines.Add(fieldbyname('descricao').AsString+'= '+Inttostr(fieldbyname('COUNT(*)').AsInteger)+' votos');
                  sumtotal:=sumtotal+fieldbyname('COUNT(*)').AsInteger;
                  Next;
              end;
              First;
     ...
     Memo_result.Lines.Add('');
     Memo_result.Lines.Add('TOTAL DE VOTOS: '+Inttostr(sumtotal));
end;

procedure Tmdl_survey.monta_mensagens();
var xcodrel: string;
begin
     DBGrid5.DataSource:=dm.source_sql10;
     DBGrid5.Columns.Clear;
     DBMemo_men.DataSource:=dm.source_sql10;
     DBMemo_men.DataField :='textft';
     With DBGrid5.Columns do begin
          Add;
          Items[0].Title.Caption:='Código';
          Items[0].FieldName:='cod_img';
          Items[0].Width:=60;
     ...
     With dm.ds_sql10 do begin
          if Active then Close;
          xcodrel:=dm.ds_sql.fieldbyname('cod_gal').AsString;
          CommandText:= 'SELECT codigo,cod_img,descricao,axdt,textft FROM '+LstTables.Table[2]+'_ax'+
                        ' WHERE cod_gal='+#39+xcodrel+#39+
                        ' ORDER BY codigo';
          Open;
          Filtered:=false;
     ...
     Lb_countreg_men.Caption:='REGISTROS: '+Inttostr(dm.ds_sql10.RecordCount);
end;

procedure Tmdl_survey.FormResize(Sender: TObject);
begin
...
procedure Tmdl_survey.FormActivate(Sender: TObject);
begin
      mdl_survey.Refresh;
      if xinit then begin
         xinit:=false;
...
procedure Tmdl_survey.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      Case key of
...
procedure Tmdl_survey.travaformImages(xtrava: Boolean);
begin
...
procedure Tmdl_survey.atualiza_grade;
     ...
     axshowing:= loadx.Showing;
      ...
      if dm.ds_sql.RecordCount <= 0 then begin
         ...
         with dm.ds_sql do begin
            ...
            axdt1.DateTime:= fieldbyname('xdt1').AsDateTime;
            axdt2.DateTime:= fieldbyname('xdt1').AsDateTime;
            axdt3.DateTime:= fieldbyname('xdt2').AsDateTime;
            axdt4.DateTime:= fieldbyname('xdt2').AsDateTime;
            loadx.Progressloading.position:=40;
            ComboBox_model.Enabled:=true;
...
procedure Tmdl_survey.atualiza_grade3;
var axshowing,xuplng: Boolean;
    axlocid: Integer;
    st_stream: TMemoryStream;
begin
...
procedure Tmdl_survey.limpaphoto1(xupdateset:Boolean);
begin
...
procedure Tmdl_survey.atualiza_grade2;
var axshowing: Boolean;
    st_stream : TMemoryStream;
begin
     //banco de imagens
...
procedure Tmdl_survey.limpaphoto2(xupdateset:Boolean);
begin
...
procedure Tmdl_survey.limpaphoto3(xupdateset:Boolean);
begin
...
procedure Tmdl_survey.bt_new1Click(Sender: TObject);
begin
      addreg;
      ab_titulo.SetFocus;
end;

procedure Tmdl_survey.bt_new2Click(Sender: TObject);
begin
      addreg2;
      ms_titulo.SetFocus;
end;

procedure Tmdl_survey.bt_del1Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
     axshowing:= loadx.Showing;
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
         loadx.lbloading.Caption:='Excluindo pergunta';
         loadx.Show;
         loadx.Update;
      end else begin
         loadx.lbloading.Caption:='Excluindo pergunta';
     ...
     if (dm.ds_sql.RecordCount>0) and (ab_cod.Caption<>'novo') then begin
        ...
        if dm.deleta_reg(LstTables.Table[0],'cod_gal',ab_cod.Caption,true) then begin
           ...
           dm.deleta_reg(LstTables.Table[2]+'_ax','cod_gal',ab_cod.Caption,false);
           loadx.Progressloading.position:=75;
           With dm.ds_sql3 do begin
                First;
                While not Eof do begin
...
procedure Tmdl_survey.bt_save1Click(Sender: TObject);
var axcod,axcod2,axgal_id,cod_model,xidxlng,xid_default,axcodgrp,ax_ab_codinsert: integer;
    ...
    xtxt1,axgrpname,aydt1,aydt2: String;
    xtxt2: TStringList;
    xtxt3: TMemoryStream;
begin
     //banco de imagens
     ...
     if ab_titulo.Text='' then begin
        hwsf.BtMensagem('É necessário digitar a Questão.');
        ab_titulo.SetFocus;
        exit;
     end;
     ...
     aydt1:=#39+FormatDateTime('yyyymmdd',axdt1.Date)+' '+FormatDateTime('hh:nn:ss',axdt2.Time)+#39;
     aydt2:=#39+FormatDateTime('yyyymmdd',axdt3.Date)+' '+FormatDateTime('hh:nn:ss',axdt4.Time)+#39;
     if ab_cod.Caption = 'novo' then begin
        ...
        for axgal_id:=0 to listaLng.count-1 do begin
            ...
            with dm.executa do begin
                 CommandText:= 'INSERT INTO '+LstTables.Table[0]+' (codigo,cod_gal,cod_web,cod_lng,cod_psn,cod_ls,descricao,textft,blb,xtp,cod_usrw,cod_class,xdt1,xdt2) '+
                            'VALUES(:p0,:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,'+aydt1+','+aydt2+')';
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+LstTables.Table[0]+' SET descricao=:p0,textft=:p1,blb=:p2,xdt1='+aydt1+',xdt2='+aydt2+' '+
                          'WHERE codigo=:p3';
            loadx.Progressloading.position:=20;
...
procedure Tmdl_survey.ComboBox_webChange(Sender: TObject);
begin
...
procedure Tmdl_survey.bt_save2Click(Sender: TObject);
...
procedure Tmdl_survey.getLogo1;
...
procedure Tmdl_survey.getLogo3;
var axcodBlb: Integer;
begin
     try
...
procedure Tmdl_survey.abretmplogo1;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp1.Seek(0,soFromBeginning);
...
procedure Tmdl_survey.abretmplogo3;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp3.Seek(0,soFromBeginning);
...
procedure Tmdl_survey.getLogo2;
var axcodBlb: Integer;
begin
     try
...
procedure Tmdl_survey.abretmplogo2;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp2.Seek(0,soFromBeginning);
...
procedure Tmdl_survey.getBanner;
begin
     xbannertmp.Clear;
...
procedure Tmdl_survey.abretmpbanner;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xbannertmp.Seek(0,soFromBeginning);
...
procedure Tmdl_survey.bt_limpar1Click(Sender: TObject);
begin
      limpaphoto1(True);
...
procedure Tmdl_survey.bt_limpar2Click(Sender: TObject);
begin
      limpaphoto2(True);
...
procedure Tmdl_survey.bt_view1Click(Sender: TObject);
begin
      if dm.ds_sql.RecordCount>0 then begin
...
procedure Tmdl_survey.bt_open1Click(Sender: TObject);
...
procedure Tmdl_survey.bt_open2Click(Sender: TObject);
var xfilext,xnamefile: String;
    xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
    xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp: Integer;
...
procedure Tmdl_survey.chk_view1Click(Sender: TObject);
begin
      if chk_view1.Checked then getLogo1;
end;

procedure Tmdl_survey.chk_view2Click(Sender: TObject);
begin
      if chk_view2.Checked then getLogo2;
end;

procedure Tmdl_survey.Splitter1Moved(Sender: TObject);
begin
      ComboBox_web.Width:=ToolBar_web.Width-53;
end;

procedure Tmdl_survey.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tmdl_survey.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_survey.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_survey.DBGrid1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade;
end;

procedure Tmdl_survey.DBGrid2CellClick(Column: TColumn);
begin
      atualiza_grade2;
end;

procedure Tmdl_survey.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_survey.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_survey.DBGrid2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade2;
end;

procedure Tmdl_survey.bt_view2Click(Sender: TObject);
begin
      getLogo2;
end;

procedure Tmdl_survey.Ampliar1Click(Sender: TObject);
begin
      if dm.ds_sql3.RecordCount>0 then begin
         Application.CreateForm(Tmdl_richgalery_view,mdl_richgalery_view);
         mdl_richgalery_view.TableImage.xcp_cod:='codigo';
...
procedure Tmdl_survey.bt_recursiveClick(Sender: TObject);
...
function Tmdl_survey.inserimg(xnamefile,xfilext:String;
                      xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
                      xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp:Integer;
                      xic_ajt,xic_pr,xil_ajt,xil_pr: Boolean):Boolean;
...
function Tmdl_survey.retorna_TTabSheet(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtb1;
          1: Result:=lngtb2;
...
procedure Tmdl_survey.PageControl_languagensChange(Sender: TObject);
begin
...
procedure Tmdl_survey.Timer_traduzTimer(Sender: TObject);
begin
      Inc(xtimerresponse);
end;

procedure Tmdl_survey.Tradutor1AoTraduzir(Sender: TObject;
  Traducao: String);
begin
    xreceivetrad:=Traducao;
end;

function Tmdl_survey.recebetraducao(xtrad_text,xtrad_de,xtrad_para: String):String;
var xrecebido: Bool;
begin
      xrecebido:=false;
      xreceivetrad:='';
...
procedure Tmdl_survey.save_cfg(xcodlng: Integer);
var axcod : integer;
    axshowing: Boolean;
begin
     axshowing:= loadx.Showing;
...
procedure Tmdl_survey.monta_table3;
var listaLng_id,axcoddefault,xid_default: Integer;
    axshowing: Boolean;
    //inclui linguagens
...
procedure Tmdl_survey.atualiza_grade_pers;
var axshowing: Boolean;
    axlocid: Integer;
begin
     //banner
...
procedure Tmdl_survey.limpaphoto_pers(xupdateset:Boolean);
begin
...
procedure Tmdl_survey.bt_open_persClick(Sender: TObject);
var xfilext,xnamefile: String;
    xerror: Boolean;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
...
procedure Tmdl_survey.bt_limpar_persClick(Sender: TObject);
begin
      limpaphoto_pers(true);
end;

procedure Tmdl_survey.bt_save_persClick(Sender: TObject);
begin
      save_cfg(listaLng.cod_lng[PageControl_languagens.TabIndex]);
end;

procedure Tmdl_survey.PageControl1Change(Sender: TObject);
begin
      if PageControl1.TabIndex=1 then atualiza_grade_pers;
end;

procedure Tmdl_survey.MenuItem1Click(Sender: TObject);
var axshowing: Boolean;
begin
...
procedure Tmdl_survey.bt_del2Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
     ...
     if (dm.ds_sql3.RecordCount>0) and (ms_cod.Caption<>'novo') then begin
        ...
        if dm.deleta_reg(LstTables.Table[2],'cod_img',ms_cod.Caption,true) then begin
           loadx.Progressloading.position:=50;
           dm.deleta_reg(LstTables.Table[2]+'_ax','cod_img',ms_cod.Caption,false);
           loadx.Progressloading.position:=75;
...
procedure Tmdl_survey.ComboBox_entChange(Sender: TObject);
begin
      if StrtoInt(AxDadosUsr.axparm) > 5000 then begin
...
procedure Tmdl_survey.Salvarcomo1Click(Sender: TObject);
var xfiljpg : String;
...
procedure Tmdl_survey.bt_helpClick(Sender: TObject);
begin
      //Tópicos de ajuda
...
procedure Tmdl_survey.StatusBar1Click(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tmdl_survey.ToolBar_grpResize(Sender: TObject);
begin
     cb_group.Width := ToolBar_grp.Width - 59;
end;

procedure Tmdl_survey.Desfazer1Click(Sender: TObject);
begin
     //desfazer
...
procedure Tmdl_survey.Recortar1Click(Sender: TObject);
begin
     //recortar
     retorna_TRxRichEdit(PageControlab.TabIndex).CutToClipboard;
end;

procedure Tmdl_survey.Copiar1Click(Sender: TObject);
begin
     //copiar
     retorna_TRxRichEdit(PageControlab.TabIndex).CopyToClipboard;
end;

procedure Tmdl_survey.Colar1Click(Sender: TObject);
begin
      //colar
     retorna_TRxRichEdit(PageControlab.TabIndex).PasteFromClipboard;
end;

procedure Tmdl_survey.Cor1Click(Sender: TObject);
begin
     //cor da fonte
...
procedure Tmdl_survey.Negrito1Click(Sender: TObject);
begin
     //negrito
     if FUpdating then Exit;
...
procedure Tmdl_survey.Italico1Click(Sender: TObject);
begin
     //italico
     if FUpdating then Exit;
...
procedure Tmdl_survey.Sublinhado1Click(Sender: TObject);
begin
     //sublinhado
     if FUpdating then Exit;
...
procedure Tmdl_survey.Marcador1Click(Sender: TObject);
begin
     //marcador
     if FUpdating then Exit;
...
procedure Tmdl_survey.AlignButtonClick(Sender: TObject);
begin
...
procedure Tmdl_survey.esquerdo1Click(Sender: TObject);
begin
     retorna_LeftAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_survey.centro1Click(Sender: TObject);
begin
     retorna_CenterAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_survey.direita1Click(Sender: TObject);
begin
     retorna_RightAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_survey.justificado1Click(Sender: TObject);
begin
     retorna_JustifyAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_survey.re_editorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tmdl_survey.SelectionChange(Sender: TObject);
begin
  //if processoand then exit;
...
function Tmdl_survey.CurrText: TRxTextAttributes;
begin
...
procedure Tmdl_survey.SetEditRect;
var
  R: TRect;
begin
  with retorna_TRxRichEdit(PageControlab.TabIndex) do
...
procedure Tmdl_survey.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
function Tmdl_survey.retorna_TRxRichEdit(xidxcod:Integer):TRxRichEdit;
begin
      Case xidxcod of
          0: Result:=re_editor3;
          1: Result:=re_editor1;
...
function Tmdl_survey.retorna_fontName(xidxcod:Integer):TComboBox;
begin
      Case xidxcod of
          0: Result:=FontName3;
          1: Result:=FontName1;
...
function Tmdl_survey.retorna_fontSize(xidxcod:Integer):TEdit;
begin
      Case xidxcod of
          0: Result:=FontSize3;
          1: Result:=FontSize1;
...
function Tmdl_survey.retorna_BoldButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_survey.retorna_ItalicButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_survey.retorna_UnderlineButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_survey.retorna_LeftAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
          0: Result:=LeftAlign3;
          1: Result:=LeftAlign1;
...
function Tmdl_survey.retorna_CenterAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_survey.retorna_RightAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_survey.retorna_JustifyAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_survey.retorna_BulletsButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
begin
      if (PageControlab.TabIndex = 1) then begin
         ...
         With dm.ds_sql do begin
             if (recordcount <= 0) then begin
                hwsf.BtMensagem('Não há enquetes cadastradas.');
      ...
      end else if (PageControlab.TabIndex = 3) then begin
          monta_relatorio();
      end else if (PageControlab.TabIndex = 4) then begin
          monta_mensagens();
      end;
end;

procedure Tmdl_survey.cb_groupChange(Sender: TObject);
begin
...
procedure Tmdl_survey.monta_tableclass;
...
begin
      ...
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
         loadx.lbloading.Caption:='Montando grupos';
         loadx.Show;
         loadx.Update;
      end else begin
         loadx.lbloading.Caption:='Montando grupos';
...
procedure Tmdl_survey.PanelclassResize(Sender: TObject);
begin
...
procedure Tmdl_survey.EVsaveKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_survey.EVsave2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_survey.Cordefundo1Click(Sender: TObject);
begin
      //cor de fundo
      if ColorDialog1.Execute then begin
...
procedure Tmdl_survey.validaCbClass();
begin
      if CheckBox_class.Checked then begin
...
procedure Tmdl_survey.CheckBox_classClick(Sender: TObject);
begin
      validaCbClass();
end;

procedure Tmdl_survey.ComboBox_classKeyPress(Sender: TObject;
  var Key: Char);
begin
      if key=#13 then validaCbClass();
end;

procedure Tmdl_survey.ComboBox_classClick(Sender: TObject);
begin
      validaCbClass();
end;

procedure Tmdl_survey.bt_new3Click(Sender: TObject);
begin
      addreg3;
      cl_titulo.SetFocus;
end;

procedure Tmdl_survey.bt_del3Click(Sender: TObject);
...
begin
      ...
      With dm.ds_sql8 do begin
         ...
         if (recordcount>0) then begin
            axvincul:=true;
            if not hwsf.BtConfirma('Existem opções vinculados a esta enquete, remover vínculo?') then begin
               Close;
...
procedure Tmdl_survey.DBGrid3CellClick(Column: TColumn);
begin
      atualiza_grade3;
end;

procedure Tmdl_survey.DBGrid3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade3;
end;

procedure Tmdl_survey.DBGrid3KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade3;
end;

procedure Tmdl_survey.DBGrid3MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade3;
end;

procedure Tmdl_survey.bt_save3Click(Sender: TObject);
...
procedure Tmdl_survey.bt_limpar3Click(Sender: TObject);
begin
      limpaphoto3(True);
      if hwsf.BtConfirma('Gravar dados?') then begin
         bt_save3.Click;
...
procedure Tmdl_survey.bt_view3Click(Sender: TObject);
begin
      getLogo3;
end;

procedure Tmdl_survey.bt_open3Click(Sender: TObject);
var xfilext,xnamefile: String;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
    codAbrirtmp: TPicture;
...
procedure Tmdl_survey.atualiza_relClick(Sender: TObject);
begin
      monta_relatorio();
end;

procedure Tmdl_survey.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
      hwsf.BtMensagem('AVISO: Falha na conexão.');
end;

function Tmdl_survey.ColorToHtml_rel(mColor: TColor): string;
begin
...
function Tmdl_survey.StrToHtml_rel(mStr: string; mFont: TFont = nil): string;
var
  vLeft, vRight: string;
begin
  Result := mStr;
  ...
  vLeft := Format('<FONT FACE="%s" COLOR="%s">',
    [mFont.Name, ColorToHtml_rel(mFont.Color)]);
  vRight := '</FONT>';
...
function Tmdl_survey.DBGridToHtmlTable_rel(mDBGrid: TDBGrid; mStrings: TStrings;
...
begin
  ...
  try
    ...
    mStrings.Add(Format('<TABLE BGCOLOR="%s" BORDER=1 WIDTH="100%%">',
      [ColorToHtml_rel(mDBGrid.Color)]));
    if mCaption <> '' then
      mStrings.Add(Format('<CAPTION>%s</CAPTION>', [StrToHtml_rel(mCaption)]));
    vColFormat := '';
    vColText := '';
...
'  <TD BGCOLOR="%s" ALIGN=%s WIDTH="%d%%">DisplayText%d</TD>'#13#10,
          [ColorToHtml_rel(mDBGrid.Columns[I].Color),
...
'  <TD BGCOLOR="%s" ALIGN=%s WIDTH="%d%%">%s</TD>'#13#10,
          [ColorToHtml_rel(mDBGrid.Columns[I].Title.Color),
          cAlignText[mDBGrid.Columns[I].Alignment],
          Round(vWidths[J] / vAllWidth * 100),
          StrToHtml_rel(mDBGrid.Columns[I].Title.Caption,
      ...
      for I := 0 to mDBGrid.Columns.Count - 1 do
        if mDBGrid.Columns[I].Visible then begin
          vColText := StringReplace(vColText, Format('>DisplayText%d<', [J]),
            Format('>%s<', [StrToHtml_rel(mDBGrid.Columns[I].Field.DisplayText,
...
{ uses ShellApi; }
procedure Tmdl_survey.export_relClick(Sender: TObject);
var xstrexp: TStringList;
    bufferexport: String;
begin
  try
    xstrexp:= TStringList.Create;
    xstrexp.Clear;
    if SaveDialog1_exporthtml.Execute then begin
       DBGridToHtmlTable_rel(DBGrid4, xstrexp, Caption+' - '+ab_titulo.Text);
       xstrexp.SaveToFile(SaveDialog1_exporthtml.FileName);
       bufferexport:='file://'+SaveDialog1_exporthtml.FileName;
       ShellExecute(Handle, nil, PChar(bufferexport), nil, nil, SW_SHOW);
    end;
  finally
    xstrexp.Free;
  end;
...
procedure Tmdl_survey.atualiza_menClick(Sender: TObject);
begin
      monta_mensagens();
end;
