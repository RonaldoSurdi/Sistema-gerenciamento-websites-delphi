...
uses
  ...
  umdl_lstprj2 in 'umdl_lstprj2.pas' {mdl_lstprj2},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol Projetos';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol Projetos em Andamento';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') and (ParamStr(4) <> '')then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_lstprj2, mdl_lstprj2);
                if (ParamStr(5) <> '') then
                    mdl_lstprj2.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_lstprj2.xiniCodab:= 0;
             end else begin
