...
uses
  ...
  umdl_survey in 'umdl_survey.pas' {mdl_survey};
...
begin
    ...
    Application.Title := 'HWScontrol Enquetes';
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_survey, mdl_survey);
                if (ParamStr(5) <> '') then
                    mdl_survey.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_survey.xiniCodab:= 0;
             end else begin
