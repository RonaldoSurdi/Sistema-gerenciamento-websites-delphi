...
type
  ...
  Tmdl_statgalery = class(TForm)
    ...
    Panel43: TPanel;
    bt_imgRotateh3: TSpeedButton;
    bt_imgRotatea3: TSpeedButton;
    Label26: TLabel;
    ...
    procedure chk_view3Click(Sender: TObject);
    procedure SpeedButton2Click(Sender: TObject);
    procedure bt_exp3Click(Sender: TObject);
    procedure chk_view1Click(Sender: TObject);
    procedure Panelbtview3Resize(Sender: TObject);
    procedure PanelbtviewResize(Sender: TObject);
    procedure Panelbtview2Resize(Sender: TObject);
    procedure bt_imgRotateh3Click(Sender: TObject);
    procedure bt_imgRotatea3Click(Sender: TObject);
  ...
  private
    //menus
    ordem_dbprocessos1,ordem_dbprocessos2,ordem_dbprocessos3: string;
    ordem_dbdesc1,ordem_dbdesc2,ordem_dbdesc3: Boolean;
...
var
  mdl_statgalery: Tmdl_statgalery;
...
procedure Tmdl_statgalery.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure Tmdl_statgalery.FormCreate(Sender: TObject);
var axshowing: Boolean;
...
begin
     ...
     mdl_statgalery.Caption:= AxDadosUsr.axtitle;
     xinit:=true;
     axcod_ent:=0;
     ...
     xupdatelogo2:=False;
     xupdatebanner:=False;
     ordem_dbprocessos1:='descricao';
     ordem_dbprocessos2:='descricao';
     ordem_dbprocessos3:='cp1';
     ordem_dbdesc1:=true;
     ordem_dbdesc2:=true;
     ordem_dbdesc3:=true;
     ...
     if not AxDadosUsr.master then begin
        ...
        bt_new3.Enabled:=false;
        bt_del3.Enabled:=false;
...
procedure Tmdl_statgalery.monta_lng;
...
procedure Tmdl_statgalery.monta_table1;
var axshowing: Boolean;
    xfiltdatax,xfiltdata,xfiltdata2,ordem_dbdesc1_str: String;
begin
     axshowing:= loadx.Showing;
     if not axshowing then begin
        loadx := Tloadx.Create(Application);
     ...
     With dm.ds_sql do begin
          ...
          if ordem_dbdesc1 then ordem_dbdesc1_str:=''
          else ordem_dbdesc1_str:=' desc';
          CommandText:= 'SELECT * FROM '+LstTables.Table[0]+
                                ' WHERE cod_web='+#39+InttoStr(AxDadosUsr.cod_web)+#39+
                                ' AND xtp='+#39+AxDadosUsr.axparm+#39+
                                ' AND cod_usrw='+#39+IntToStr(axcod_ent)+#39+
                                ' ORDER BY '+ordem_dbprocessos1+ordem_dbdesc1_str;
          Open;
          Filter:='';
          Filtered:=true;
          xdsqlfilter:='cod_lng='+#39+IntToStr(listaLng.cod_lng[PageControl_languagens.TabIndex])+#39;
          //Filter:=xdsqlfilter;
          if dm.ds_sql6.RecordCount>0 then
             Filter:='(('+xdsqlfilter+') AND (cod_class='+#39+dm.ds_sql6.fieldbyname('cod_class').AsString+#39+'))'
          else Filter:=xdsqlfilter;
     end;
     atualiza_grade;
     try loadx.Close; except end;
end;

procedure Tmdl_statgalery.monta_table2(xnewgal,xuplng:Boolean);
var xcodgal,ordem_dbdesc2_str: string;
begin
     ...
     With dm.ds_sql3 do begin
          if ((xuplng) or (not Active)) then begin
             if Active then Close;
             if ordem_dbdesc2 then ordem_dbdesc2_str:=''
             else ordem_dbdesc2_str:=' desc';
             CommandText:= 'SELECT * FROM '+LstTables.Table[2]+
                                ' WHERE cod_gal='+#39+xcodgal+#39+
                                ' ORDER BY '+ordem_dbprocessos2+ordem_dbdesc2_str;
             Open;
             Filtered:=true;
          end;
...
procedure Tmdl_statgalery.FormResize(Sender: TObject);
begin
...
procedure Tmdl_statgalery.FormActivate(Sender: TObject);
begin
      mdl_statgalery.Refresh;
      if xinit then begin
         xinit:=false;
...
procedure Tmdl_statgalery.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      Case key of
...
procedure Tmdl_statgalery.addreg;
begin
     //ComboBox_model.Enabled:=true;
     if ComboBox_class.ItemIndex > 0 then
...
procedure Tmdl_statgalery.travaformImages(xtrava: Boolean);
begin
...
procedure Tmdl_statgalery.addreg2;
begin
     ms_cod.Caption:='novo';
     ax_ms_cod:=0;
     ms_titulo.Text:='';
...
procedure Tmdl_statgalery.addreg3;
begin
     cl_cod.Caption:='novo';
     ax_cl_cod:=0;
     axcod_class:=0;
...
procedure Tmdl_statgalery.atualiza_grade;
var axshowing: Boolean;
    axlocid: Integer;
     ...
     axshowing:= loadx.Showing;
      ...
      if dm.ds_sql.RecordCount <= 0 then begin
         ...
         with dm.ds_sql do begin
            ...
            else ComboBox_class2.Text:= 'Não classificar por Estado';//ComboBox_class2.ItemIndex:=0;asdfasdf
            validaCbClass;
            //end;
...
procedure Tmdl_statgalery.atualiza_grade3;
...
procedure Tmdl_statgalery.limpaphoto1(xupdateset:Boolean);
begin
...
procedure Tmdl_statgalery.atualiza_grade2;
var axshowing: Boolean;
    st_stream : TMemoryStream;
begin
     //banco de imagens
...
procedure Tmdl_statgalery.limpaphoto2(xupdateset:Boolean);
begin
...
procedure Tmdl_statgalery.limpaphoto3(xupdateset:Boolean);
begin
...
procedure Tmdl_statgalery.bt_new1Click(Sender: TObject);
begin
      addreg;
      ab_titulo.SetFocus;
end;

procedure Tmdl_statgalery.bt_new2Click(Sender: TObject);
begin
      addreg2;
      ms_titulo.SetFocus;
end;

procedure Tmdl_statgalery.bt_del1Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
     axshowing:= loadx.Showing;
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
         loadx.lbloading.Caption:='Excluindo Região';
         loadx.Show;
         loadx.Update;
      end else begin
         loadx.lbloading.Caption:='Excluindo Região';
     ...
     if (dm.ds_sql.RecordCount>0) and (ab_cod.Caption<>'novo') then begin
        ...
        if dm.deleta_reg(LstTables.Table[0],'cod_gal',ab_cod.Caption,true) then begin
           ...
           loadx.Progressloading.position:=50;
           loadx.lbloading.Caption:='Excluindo Lista de Representantes';
           loadx.lbloading.Repaint;
...
procedure Tmdl_statgalery.bt_save1Click(Sender: TObject);
...
begin
     ...
     if ab_titulo.Text='' then begin
        hwsf.BtMensagem('É necessário digitar a Região ou Cidade.');
        ab_titulo.SetFocus;
        exit;
     end;
     ...
     if (axcodgrp = 0) then begin
        hwsf.BtMensagem('Selecione o Estado.');
        ComboBox_class2.SetFocus;
        exit;
     end;
...
procedure Tmdl_statgalery.ComboBox_webChange(Sender: TObject);
begin
      if (not xinit) and (dm.executa3.Active) then begin
          if dm.executa3.Locate('website',ComboBox_web.Text,[]) then begin
             ...
             monta_table1;
             if (PageControlab.TabIndex = 2) then monta_table2(False,True);
             if PageControl1.TabIndex=1 then monta_table3;
          end;
      end;
end;

procedure Tmdl_statgalery.bt_save2Click(Sender: TObject);
var axcod,axcod2,axgal_id,xidxlng,xid_default,axcodinsert : integer;
    axshowing,xexecutaimg,axInc_traduzir: Boolean;
    xtxt1: String;
    xtxt2: TStringList;
    xtxt3: TMemoryStream;
    ximagemImportada: Boolean;
begin
     axcodinsert:=0;
     if ms_titulo.Text='' then begin
        hwsf.BtMensagem('É necessário digitar a Razão Social, Fantasia ou Nome do representante.');
        ms_titulo.SetFocus;
        exit;
     end;
     if ab_cod.Caption = 'novo' then begin
     ...
     xampltmp2.Seek(0,soFromBeginning);
     xexecutaimg:=true;
     if ximagemImportada then begin
       if ms_cod.Caption = 'novo' then begin
...
procedure Tmdl_statgalery.getLogo1;
...
procedure Tmdl_statgalery.getLogo3;
var axcodBlb: Integer;
begin
     try
...
procedure Tmdl_statgalery.abretmplogo1;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp1.Seek(0,soFromBeginning);
...
procedure Tmdl_statgalery.abretmplogo3;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp3.Seek(0,soFromBeginning);
...
procedure Tmdl_statgalery.getLogo2;
var axcodBlb: Integer;
begin
     try
...
procedure Tmdl_statgalery.abretmplogo2;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp2.Seek(0,soFromBeginning);
...
procedure Tmdl_statgalery.getBanner;
begin
     xbannertmp.Clear;
...
procedure Tmdl_statgalery.abretmpbanner;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xbannertmp.Seek(0,soFromBeginning);
...
procedure Tmdl_statgalery.bt_limpar1Click(Sender: TObject);
begin
      limpaphoto1(True);
...
procedure Tmdl_statgalery.bt_limpar2Click(Sender: TObject);
begin
      limpaphoto2(True);
...
procedure Tmdl_statgalery.bt_view1Click(Sender: TObject);
begin
      if dm.ds_sql.RecordCount>0 then begin
         ...
         mdl_richgalery_view.Left:=mdl_statgalery.Width-mdl_richgalery_view.Width;
         mdl_richgalery_view.Top:=mdl_statgalery.Height-mdl_richgalery_view.Height;
         mdl_richgalery_view.Show;
      end;
      //getLogo1;
end;

procedure Tmdl_statgalery.bt_open1Click(Sender: TObject);
...
procedure Tmdl_statgalery.bt_open2Click(Sender: TObject);
var xfilext,xnamefile: String;
    xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
    xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp: Integer;
...
procedure Tmdl_statgalery.chk_view2Click(Sender: TObject);
begin
      if chk_view2.Checked then getLogo2;
end;

procedure Tmdl_statgalery.Splitter1Moved(Sender: TObject);
begin
      ComboBox_web.Width:=ToolBar_web.Width-53;
end;

procedure Tmdl_statgalery.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tmdl_statgalery.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_statgalery.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_statgalery.DBGrid1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade;
end;

procedure Tmdl_statgalery.DBGrid2CellClick(Column: TColumn);
begin
      atualiza_grade2;
end;

procedure Tmdl_statgalery.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_statgalery.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_statgalery.DBGrid2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade2;
end;

procedure Tmdl_statgalery.bt_view2Click(Sender: TObject);
begin
      if dm.ds_sql3.RecordCount>0 then begin
         Application.CreateForm(Tmdl_richgalery_view,mdl_richgalery_view);
         mdl_richgalery_view.TableImage.xcp_cod:='codigo';
...
procedure Tmdl_statgalery.bt_recursiveClick(Sender: TObject);
...
function Tmdl_statgalery.inserimg(xnamefile,xfilext:String;
                      xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
                      xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp:Integer;
                      xic_ajt,xic_pr,xil_ajt,xil_pr: Boolean):Boolean;
...
function Tmdl_statgalery.retorna_TTabSheet(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtb1;
          1: Result:=lngtb2;
...
procedure Tmdl_statgalery.PageControl_languagensChange(Sender: TObject);
begin
...
procedure Tmdl_statgalery.Timer_traduzTimer(Sender: TObject);
begin
      Inc(xtimerresponse);
end;

procedure Tmdl_statgalery.Tradutor1AoTraduzir(Sender: TObject;
  Traducao: String);
begin
    xreceivetrad:=Traducao;
end;

function Tmdl_statgalery.recebetraducao(xtrad_text,xtrad_de,xtrad_para: String):String;
var xrecebido: Bool;
begin
      xrecebido:=false;
      xreceivetrad:='';
...
procedure Tmdl_statgalery.save_cfg(xcodlng: Integer);
var axcod : integer;
    axshowing: Boolean;
begin
     axshowing:= loadx.Showing;
...
procedure Tmdl_statgalery.monta_table3;
var listaLng_id,axcoddefault,xid_default: Integer;
    axshowing: Boolean;
    //inclui linguagens
...
procedure Tmdl_statgalery.atualiza_grade_pers;
var axshowing: Boolean;
    axlocid: Integer;
begin
     //banner
...
procedure Tmdl_statgalery.limpaphoto_pers(xupdateset:Boolean);
begin
...
procedure Tmdl_statgalery.bt_open_persClick(Sender: TObject);
var xfilext,xnamefile: String;
    xerror: Boolean;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
...
procedure Tmdl_statgalery.bt_limpar_persClick(Sender: TObject);
begin
      limpaphoto_pers(true);
end;

procedure Tmdl_statgalery.bt_save_persClick(Sender: TObject);
begin
      save_cfg(listaLng.cod_lng[PageControl_languagens.TabIndex]);
end;

procedure Tmdl_statgalery.PageControl1Change(Sender: TObject);
begin
...
procedure Tmdl_statgalery.MenuItem1Click(Sender: TObject);
var axshowing: Boolean;
begin
...
procedure Tmdl_statgalery.bt_del2Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
     axshowing:= loadx.Showing;
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
         loadx.lbloading.Caption:='Excluindo Representante';
         loadx.Show;
         loadx.Update;
      end else begin
         loadx.lbloading.Caption:='Excluindo Representante';
...
procedure Tmdl_statgalery.ComboBox_entChange(Sender: TObject);
begin
      if StrtoInt(AxDadosUsr.axparm) > 5000 then begin
...
procedure Tmdl_statgalery.bt_helpClick(Sender: TObject);
begin
      //Tópicos de ajuda
...
procedure Tmdl_statgalery.StatusBar1Click(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tmdl_statgalery.ToolBar_grpResize(Sender: TObject);
begin
     cb_group.Width := ToolBar_grp.Width - 50;
end;

procedure Tmdl_statgalery.Desfazer1Click(Sender: TObject);
begin
     //desfazer
...
procedure Tmdl_statgalery.Recortar1Click(Sender: TObject);
begin
     //recortar
     retorna_TRxRichEdit(PageControlab.TabIndex).CutToClipboard;
end;

procedure Tmdl_statgalery.Copiar1Click(Sender: TObject);
begin
     //copiar
     retorna_TRxRichEdit(PageControlab.TabIndex).CopyToClipboard;
end;

procedure Tmdl_statgalery.Colar1Click(Sender: TObject);
begin
      //colar
     retorna_TRxRichEdit(PageControlab.TabIndex).PasteFromClipboard;
end;

procedure Tmdl_statgalery.Cor1Click(Sender: TObject);
begin
     //cor da fonte
...
procedure Tmdl_statgalery.Negrito1Click(Sender: TObject);
begin
     //negrito
     if FUpdating then Exit;
...
procedure Tmdl_statgalery.Italico1Click(Sender: TObject);
begin
     //italico
     if FUpdating then Exit;
...
procedure Tmdl_statgalery.Sublinhado1Click(Sender: TObject);
begin
     //sublinhado
     if FUpdating then Exit;
...
procedure Tmdl_statgalery.Marcador1Click(Sender: TObject);
begin
     //marcador
     if FUpdating then Exit;
...
procedure Tmdl_statgalery.AlignButtonClick(Sender: TObject);
begin
...
procedure Tmdl_statgalery.esquerdo1Click(Sender: TObject);
begin
     retorna_LeftAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_statgalery.centro1Click(Sender: TObject);
begin
     retorna_CenterAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_statgalery.direita1Click(Sender: TObject);
begin
     retorna_RightAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_statgalery.justificado1Click(Sender: TObject);
begin
     retorna_JustifyAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_statgalery.re_editorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tmdl_statgalery.SelectionChange(Sender: TObject);
begin
  //if processoand then exit;
...
function Tmdl_statgalery.CurrText: TRxTextAttributes;
begin
...
procedure Tmdl_statgalery.SetEditRect;
var
  R: TRect;
begin
  with retorna_TRxRichEdit(PageControlab.TabIndex) do
...
procedure Tmdl_statgalery.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
function Tmdl_statgalery.retorna_TRxRichEdit(xidxcod:Integer):TRxRichEdit;
begin
      Case xidxcod of
          0: Result:=re_editor3;
          1: Result:=re_editor1;
...
function Tmdl_statgalery.retorna_fontName(xidxcod:Integer):TComboBox;
begin
      Case xidxcod of
          0: Result:=FontName3;
          1: Result:=FontName1;
...
function Tmdl_statgalery.retorna_fontSize(xidxcod:Integer):TEdit;
begin
      Case xidxcod of
          0: Result:=FontSize3;
          1: Result:=FontSize1;
...
function Tmdl_statgalery.retorna_BoldButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_statgalery.retorna_ItalicButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_statgalery.retorna_UnderlineButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_statgalery.retorna_LeftAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
          0: Result:=LeftAlign3;
          1: Result:=LeftAlign1;
...
function Tmdl_statgalery.retorna_CenterAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_statgalery.retorna_RightAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_statgalery.retorna_JustifyAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_statgalery.retorna_BulletsButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
procedure Tmdl_statgalery.setPaCtrlLab;
var axcod,xidxcb,xidxcbsum,xidCbclass: Integer;
    axshowing: Boolean;
    xstrCbclass: Tstringlist;
    axdssqlfilt: String;
begin
      if (PageControlab.TabIndex = 1) then begin
         if ComboBox_class.Items.Count>0 then begin
            if ComboBox_class.Text<>cl_titulo.Text then begin
               try
                   xstrCbclass:= Tstringlist.Create;
                   xstrCbclass.Clear;
                   xstrCbclass.AddStrings(ComboBox_class.Items);
                   for xidCbclass:=0 to xstrCbclass.count-1 do begin
                       if xstrCbclass.Strings[xidCbclass]=cl_titulo.Text then begin
                          //if ComboBox_class.Text<>cl_titulo.Text then begin
                          ComboBox_class.ItemIndex:=xidCbclass;
                          validaCbClass();
                          //end;
                          break;
                       end;
         ...
         if dm.ds_sql6.RecordCount>0 then begin
            axdssqlfilt:='(('+xdsqlfilter+') AND (cod_class='+#39+dm.ds_sql6.fieldbyname('cod_class').AsString+#39+'))';
            if dm.ds_sql.Filter<>axdssqlfilt then
               dm.ds_sql.Filter:=axdssqlfilt;
         end else dm.ds_sql.Filter:=xdsqlfilter;
      end else if (PageControlab.TabIndex = 2) then begin
         cb_group.Clear;
         With dm.ds_sql do begin
             if (recordcount <= 0) then begin
                hwsf.BtMensagem('Não há Representantes cadastrados.');
                PageControlab.TabIndex:=1;
                exit;
             end else if ((ab_cod.Caption = '0') or (ab_cod.Caption = 'novo')) then begin
                hwsf.BtMensagem('Antes de inserir os itens grave as informações do Representante.');
                PageControlab.TabIndex:=1;
                exit;
             end;
             axshowing:= loadx.Showing;
...
procedure Tmdl_statgalery.cb_groupChange(Sender: TObject);
begin
...
procedure Tmdl_statgalery.monta_tableclass;
var axshowing,axcodclass2: Boolean;
    axclass2,ordem_dbdesc3_str: String;
begin
      axshowing:= loadx.Showing;
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
      ...
      With dm.ds_sql6 do begin
          if Active then close;
          if ordem_dbdesc3 then ordem_dbdesc3_str:=''
          else ordem_dbdesc3_str:=' desc';
          CommandText:= 'SELECT * FROM '+LstTables.Table[0]+'_ax'+
                        ' WHERE cod_web='+#39+InttoStr(AxDadosUsr.cod_web)+#39+
                        ' AND xtp='+#39+AxDadosUsr.axparm+#39+
                        ' ORDER BY '+ordem_dbprocessos3+ordem_dbdesc3_str;
          Open;
          Filtered:=true;
          try
          ...
          ComboBox_class.Items.Add('Não classificar por Estado');
          First;
          While not Eof do begin
          ...
          ComboBox_class2.ItemIndex:=0;
          //ComboBox_class.ItemIndex:=0;
          //if axcodclass2 then ComboBox_class2.Text:=axclass2
          //else ComboBox_class2.ItemIndex:=-1;
      end;
      atualiza_grade3;
      try loadx.Close; except end;
end;

procedure Tmdl_statgalery.PanelclassResize(Sender: TObject);
begin
    ComboBox_class.Width := Panelclass.Width-11;
end;

procedure Tmdl_statgalery.EVsaveKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_statgalery.EVsave2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_statgalery.Cordefundo1Click(Sender: TObject);
begin
      //cor de fundo
      if ColorDialog1.Execute then begin
...
procedure Tmdl_statgalery.validaCbClass();
var axfilterIDCl,axfilterIDCl_ax1,axfilterIDCl_ax2: String;
begin
      if (ComboBox_class.ItemIndex > 0) then begin
          axfilterIDCl_ax1:=ComboBox_class.Text;
          axfilterIDCl_ax2:=dm.ds_sql6.fieldbyname('cp1').AsString;
          if axfilterIDCl_ax1<>axfilterIDCl_ax2 then begin
             if (dm.ds_sql6.Locate('cp1',ComboBox_class.Text,[])) then begin
                axfilterIDCl:='(('+xdsqlfilter+') AND (cod_class='+#39+dm.ds_sql6.fieldbyname('cod_class').AsString+#39+'))';
                dm.ds_sql.Filter:=axfilterIDCl;
                cl_cod.Caption:=dm.ds_sql6.fieldbyname('cod_class').AsString;
                ax_cl_cod:=dm.ds_sql6.fieldbyname('codigo').AsInteger;
                if axcod_class<>dm.ds_sql6.fieldbyname('cod_class').AsInteger then
                   axcod_class:=dm.ds_sql6.fieldbyname('cod_class').AsInteger;
                cl_titulo.Text:=dm.ds_sql6.fieldbyname('cp1').AsString;
             end;
          end;
      end else if dm.ds_sql.Filter<>xdsqlfilter then
          dm.ds_sql.Filter:=xdsqlfilter;
end;

function Tmdl_statgalery.Btsearch_auto_ax(pathdefault,filtrofind:string):Tbt_search_ax;
...
procedure Tmdl_statgalery.ComboBox_classKeyPress(Sender: TObject;
  var Key: Char);
begin
      if key=#13 then validaCbClass();
end;

procedure Tmdl_statgalery.ComboBox_classClick(Sender: TObject);
begin
      validaCbClass();
end;

procedure Tmdl_statgalery.bt_new3Click(Sender: TObject);
begin
      addreg3;
      cl_titulo.SetFocus;
end;

procedure Tmdl_statgalery.bt_del3Click(Sender: TObject);
...
begin
      ...
      With dm.ds_sql8 do begin
         ...
         if (recordcount>0) then begin
            axvincul:=true;
            if not hwsf.BtConfirma('Existem Representantes vinculados com este Estado, remover vínculo?') then begin
               Close;
      ...
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
         loadx.lbloading.Caption:='Excluindo Estado';
         loadx.Show;
         loadx.Update;
      end else begin
         loadx.lbloading.Caption:='Excluindo Estado';
...
procedure Tmdl_statgalery.DBGrid3CellClick(Column: TColumn);
begin
      atualiza_grade3;
end;

procedure Tmdl_statgalery.DBGrid3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade3;
end;

procedure Tmdl_statgalery.DBGrid3KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade3;
end;

procedure Tmdl_statgalery.DBGrid3MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade3;
end;

procedure Tmdl_statgalery.bt_save3Click(Sender: TObject);
...
begin
     ...
     end else if xupdatelogo3 then begin
     ...
     Lb_countreg3.Caption:=Inttostr(dm.ds_sql6.RecordCount);
     Application.ProcessMessages;
     dm.ds_sql6.Locate('codigo',ax_cl_codinsert,[]);
     atualiza_grade3;
     if chk_view3.Checked then getLogo3;
     loadx.Progressloading.position:=100;
     loadx.lbloading.Caption:='Dados gravados com sucesso.';
...
procedure Tmdl_statgalery.bt_exp1Click(Sender: TObject);
var xfiljpg : String;
...
procedure Tmdl_statgalery.bt_exp2Click(Sender: TObject);
var xfiljpg : String;
    xImagem_tmp : TMemoryStream;
begin
      if ExportarImgDialog.Execute then begin
...
procedure Tmdl_statgalery.bt_open3Click(Sender: TObject);
var xfilext,xnamefile: String;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
    codAbrirtmp: TPicture;
...
procedure Tmdl_statgalery.bt_limpar3Click(Sender: TObject);
begin
      limpaphoto3(True);
      if hwsf.BtConfirma('Gravar dados?') then begin
         bt_save3.Click;
...
procedure Tmdl_statgalery.bt_view3Click(Sender: TObject);
begin
      if dm.ds_sql6.RecordCount>0 then begin
         Application.CreateForm(Tmdl_richgalery_view,mdl_richgalery_view);
         mdl_richgalery_view.TableImage.xcp_cod:='codigo';
         mdl_richgalery_view.TableImage.xcp_descricao:='cp1';
         mdl_richgalery_view.TableImage.xcp_img:='icon';
         mdl_richgalery_view.TableImage.xcp_tableimg:=LstTables.Table[1]+'_ax';
         mdl_richgalery_view.TableImage.xcp_codimg:='cod_class';
         mdl_richgalery_view.TableImage.xcp_table:= dm.ds_sql6;
         mdl_richgalery_view.Left:=mdl_statgalery.Width-mdl_richgalery_view.Width;
         mdl_richgalery_view.Top:=mdl_statgalery.Height-mdl_richgalery_view.Height;
         mdl_richgalery_view.Show;
      end;
...
procedure Tmdl_statgalery.chk_view3Click(Sender: TObject);
begin
      if chk_view3.Checked then getLogo3;
end;

procedure Tmdl_statgalery.SpeedButton2Click(Sender: TObject);
...
procedure Tmdl_statgalery.bt_exp3Click(Sender: TObject);
var xfiljpg : String;
    xImagem_tmp : TMemoryStream;
begin
      if ExportarImgDialog.Execute then begin
         ...
         xImagem_tmp.Clear;
         xImagem_tmp.Position:=0;
         xImagem_tmp := dm.GetBinaryStreamPH(LstTables.Table[0]+'_ax','cod_class','icon',cl_cod.Caption);
         xImagem_tmp.Seek(0,soFromBeginning);
         xImagem_tmp.SaveToFile(xfiljpg);
         xImagem_tmp.Free;
      end;
...
procedure Tmdl_statgalery.chk_view1Click(Sender: TObject);
begin
      if chk_view1.Checked then getLogo1;
end;

procedure Tmdl_statgalery.Panelbtview3Resize(Sender: TObject);
begin
    bt_view3.Height:=Panelbtview3.Height-49;
end;

procedure Tmdl_statgalery.PanelbtviewResize(Sender: TObject);
begin
    bt_view1.Height:=Panelbtview.Height-49;
end;

procedure Tmdl_statgalery.Panelbtview2Resize(Sender: TObject);
begin
    bt_view2.Height:=Panelbtview2.Height-49;
end;

procedure Tmdl_statgalery.bt_imgRotateh3Click(Sender: TObject);
begin
     if dm.rotateFoto(90,xlogotmp3,xlogosz3,xupdatelogo3,'0',LstTables.Table[0]+'_ax','cod_class','icon',cl_cod.Caption) then
        abretmplogo3;
end;

procedure Tmdl_statgalery.bt_imgRotatea3Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp3,xlogosz3,xupdatelogo3,'0',LstTables.Table[0]+'_ax','cod_class','icon',cl_cod.Caption) then
        abretmplogo3;
end;

procedure Tmdl_statgalery.bt_imgRotateh1Click(Sender: TObject);
begin
...
procedure Tmdl_statgalery.bt_imgRotatea1Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp1,xlogosz1,xupdatelogo1,ComboBox_model.Text,LstTables.Table[1],'cod_gal','icon',ab_cod.Caption) then
        abretmplogo1;
end;

procedure Tmdl_statgalery.bt_imgRotateh2Click(Sender: TObject);
begin
...
procedure Tmdl_statgalery.bt_imgRotatea2Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp2,xlogosz2,xupdatelogo2,ComboBox_model.Text,LstTables.Table[3],'cod_img','img_ico',ms_cod.Caption) then begin
        if dm.rotateFoto(-90,xampltmp2,xamplsz2,xupdatelogo2,ComboBox_model.Text,LstTables.Table[3],'cod_img','img_larg',ms_cod.Caption) then
           abretmplogo2
...
procedure Tmdl_statgalery.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
      hwsf.BtMensagem('AVISO: Falha na conexão.');
end;

procedure Tmdl_statgalery.DBGrid3TitleClick(Column: TColumn);
var xinddbs: integer;
begin
      if (ordem_dbprocessos3=column.fieldname) then begin
          ordem_dbdesc3:=not ordem_dbdesc3;
      end else begin
          ordem_dbdesc3:=true;
          ordem_dbprocessos3:=column.fieldname; // CAMPO RECEBE O NOME DA COLUNA CLICADA,
          application.processmessages; // para considerar algo que aconteça no dbgrid durante a entrada nesta procedure
          for xinddbs:=0 to DBGrid3.Columns.Count-1 do begin
              DBGrid3.Columns.Items[xinddbs].Title.Color:=$00E5E5E5;
          end;
          DBGrid3.Columns.Items[Column.Index].Title.Color:=$00ADADAD;
      end;
      monta_tableclass;
end;

procedure Tmdl_statgalery.DBGrid1TitleClick(Column: TColumn);
var xinddbs: integer;
begin
      if (ordem_dbprocessos1=column.fieldname) then begin
          ordem_dbdesc1:=not ordem_dbdesc1;
      end else begin
          ordem_dbdesc1:=true;
          ordem_dbprocessos1:=column.fieldname; // CAMPO RECEBE O NOME DA COLUNA CLICADA,
          application.processmessages; // para considerar algo que aconteça no dbgrid durante a entrada nesta procedure
...
procedure Tmdl_statgalery.DBGrid2TitleClick(Column: TColumn);
var xinddbs: integer;
begin
      if (ordem_dbprocessos2=column.fieldname) then begin
          ordem_dbdesc2:=not ordem_dbdesc2;
      end else begin
          ordem_dbdesc2:=true;
          ordem_dbprocessos2:=column.fieldname; // CAMPO RECEBE O NOME DA COLUNA CLICADA,
          application.processmessages; // para considerar algo que aconteça no dbgrid durante a entrada nesta procedure
          for xinddbs:=0 to DBGrid2.Columns.Count-1 do begin
              DBGrid2.Columns.Items[xinddbs].Title.Color:=$00E5E5E5;
          end;
          DBGrid2.Columns.Items[Column.Index].Title.Color:=$00ADADAD;
      end;
      monta_table2(False,True);
end;
