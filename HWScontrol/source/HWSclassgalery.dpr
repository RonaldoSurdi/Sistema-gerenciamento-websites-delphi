...
uses
  ...
  umdl_classgalery in 'umdl_classgalery.pas' {mdl_classgalery},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol ClassGallery';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol ClassGallery';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '') then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_classgalery, mdl_classgalery);
                if (ParamStr(5) <> '') then
                    mdl_classgalery.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_classgalery.xiniCodab:= 0;
             end else begin
