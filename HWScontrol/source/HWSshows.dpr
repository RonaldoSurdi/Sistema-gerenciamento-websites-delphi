...
uses
  ...
  umdl_shows in 'umdl_shows.pas' {mdl_shows},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol Shows';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol Shows';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '')then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_shows, mdl_shows);
                if (ParamStr(5) <> '') then
                    mdl_shows.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_shows.xiniCodab:= 0;
             end else begin
