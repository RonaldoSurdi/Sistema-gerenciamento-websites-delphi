...
uses
  ...
  umdl_richdiscotxt in 'umdl_richdiscotxt.pas' {mdl_richdiscotxt},
...
begin
    ...
    Application.Title := 'HWScontrol Discografia';
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richdiscotxt, mdl_richdiscotxt);
                if (ParamStr(5) <> '') then
                    mdl_richdiscotxt.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richdiscotxt.xiniCodab:= 0;
             end else begin
