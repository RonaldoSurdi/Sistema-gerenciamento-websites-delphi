...
type
  ...
  Tmdl_usrsac = class(TForm)
    ...
    ToolBar1: TToolBar;
    Panel2: TPanel;
    btareasDel: TSpeedButton;
    btareasInc: TSpeedButton;
    Label2: TLabel;
    ...
    procedure btareasDelClick(Sender: TObject);
    procedure btareasIncClick(Sender: TObject);
  ...
  private
    ...
    clbAreas_cod, clbAreas_tp: TStringList;
    procedure atualiza_grade2;
    procedure addreg2;
    procedure filtrar_reg;
    procedure monta_table2;
    procedure atualiza_xcpw25;
    function montaAreasSelect(xidSelect: Integer):Boolean;
    procedure montaAreasUsr;
...
var
  mdl_usrsac: Tmdl_usrsac;

implementation
uses udm, hwsfunctions, DateUtils, Math, U_Cipher, StdConvs;

{$R *.dfm}

procedure Tmdl_usrsac.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure Tmdl_usrsac.FormCreate(Sender: TObject);
var axshowing: Boolean;
...
begin
     ...
     mdl_usrsac.Caption := AxDadosUsr.axtitle;
     ...
     loadx.lbloading.Caption:='Montando Setores';
     loadx.lbloading.Repaint;
     montaAreasUsr;
...
procedure Tmdl_usrsac.atualiza_grade2;
begin
...
procedure Tmdl_usrsac.FormResize(Sender: TObject);
begin
...
procedure Tmdl_usrsac.filtrar_reg;
var axsearch : string;
begin
...
procedure Tmdl_usrsac.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tmdl_usrsac.ComboBox_searchChange(Sender: TObject);
begin
...
procedure Tmdl_usrsac.Edit_searchKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tmdl_usrsac.Edit_searchChange(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tmdl_usrsac.edKeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tmdl_usrsac.atualiza_xcpw25;
begin
     with dm.ds_sql5 do begin
       First;
       xcpw25_ax.Clear;
...
procedure Tmdl_usrsac.ComboBox_webChange(Sender: TObject);
begin
...
procedure Tmdl_usrsac.monta_table2;
begin
     With dm.ds_sql4 do begin
...
procedure Tmdl_usrsac.bt_prior2Click(Sender: TObject);
begin
    dm.ds_sql4.Prior;
    atualiza_grade2;
end;

procedure Tmdl_usrsac.bt_next2Click(Sender: TObject);
begin
    dm.ds_sql4.Next;
    atualiza_grade2;
end;

procedure Tmdl_usrsac.bt_new2Click(Sender: TObject);
begin
    addreg2;
    xcpw6.SetFocus;
end;

procedure Tmdl_usrsac.bt_del2Click(Sender: TObject);
begin
...
procedure Tmdl_usrsac.bt_save2Click(Sender: TObject);
...
procedure Tmdl_usrsac.DBGrid2CellClick(Column: TColumn);
begin
     atualiza_grade2;
end;

procedure Tmdl_usrsac.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     atualiza_grade2;
end;

procedure Tmdl_usrsac.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     atualiza_grade2;
end;

procedure Tmdl_usrsac.xcpw24_axChange(Sender: TObject);
begin
     if (xinitAUX) then begin
...
procedure Tmdl_usrsac.xcpw24Change(Sender: TObject);
begin
     if (xinitAUX) then begin
       if xcpw24.Text <> '' then
...
procedure Tmdl_usrsac.xcpw25_axChange(Sender: TObject);
begin
     if (xinitAUX) then begin
...
procedure Tmdl_usrsac.xcpw25Change(Sender: TObject);
begin
     if (xinitAUX) then begin
       if xcpw25.Text <> '' then
...
procedure Tmdl_usrsac.xcpw18_axChange(Sender: TObject);
begin
...
procedure Tmdl_usrsac.FormShow(Sender: TObject);
begin
    Edit_search.Width:= Panel_search.Width-164;
    ComboBox_search.Left:= Edit_search.Width+5;
    bt_search.Left:= ComboBox_search.Left+ComboBox_search.Width+1;
...
procedure Tmdl_usrsac.SpeedButton8Click(Sender: TObject);
begin
      JumpTo('www.correios.com.br/servicos/cep/cep_default.cfm');
end;

procedure Tmdl_usrsac.SpeedButton1Click(Sender: TObject);
begin
      dm.ds_sql4.Close;
      dm.ds_sql4.Open;
      atualiza_grade2;
...
function Tmdl_usrsac.montaAreasSelect(xidSelect: Integer):Boolean;
var idSlc,idSlcCod: Integer;
begin
    Result:=true;
    try
      if clbAreas_cod.Count=0 then exit;
      Case xidSelect of
         0: begin
            for idSlc:=0 to clbAreas_cod.Count-1 do begin
                clbAreas.Checked[idSlc]:=true;
            end;
         end;
         1: begin
            for idSlc:=0 to clbAreas_cod.Count-1 do begin
                clbAreas.Checked[idSlc]:=false;
            end;
         end;
         2: begin
             With dm.ds_sql7 do begin
                if Active then Close;
                CommandText:= 'SELECT * from '+LstTables.Table[5]+'_ax'+
                              ' WHERE cod_ent='+#39+xcodigo2.Text+#39;
                Open;
                for idSlc:=0 to clbAreas_cod.Count-1 do begin
                    clbAreas.Checked[idSlc]:=Locate('cod_md',clbAreas_cod.Strings[idSlc],[]);
                end;
                Close;
             end;
         end;
         3: begin
             With dm.executa2 do begin
                CommandText:='DELETE FROM '+LstTables.Table[5]+'_ax'+
                             ' WHERE cod_ent='+#39+xcodigo2.Text+#39;
                Execute;
                for idSlc:=0 to clbAreas_cod.Count-1 do begin
                    if clbAreas.Checked[idSlc] then begin
                       idSlcCod:=dm.GetCodMax(LstTables.Table[5]+'_ax','codigo','');
                       CommandText:='INSERT INTO '+LstTables.Table[5]+'_ax (codigo,cod_ent,cod_md,xtp)'+
                                    ' VALUES(:p0,:p1,:p2,:p3)';
                       Params[0].AsInteger:=idSlcCod;
                       Params[1].AsInteger:=StrToInt(xcodigo2.Text);
                       Params[2].AsInteger:=StrToInt(clbAreas_cod.Strings[idSlc]);
                       Params[3].AsString:=clbAreas_tp.Strings[idSlc];
                       Execute;
                    end;
                end;
             end;
...
procedure Tmdl_usrsac.montaAreasUsr;
begin
      With dm.ds_sql6 do begin
           if Active then Close;
           CommandText:='SELECT A.codigo,A.cod_web,A.cod_grp,B.codigo,B.mdl,B.descricao,B.cp5 FROM '+
                          EnDecryptString(Serverconect.vrf32,37249)+' A,'+
                          EnDecryptString(Serverconect.vrf24,37249)+' B'+
                          ' WHERE A.cod_web='+#39+IntToStr(AxDadosUsr.cod_web)+#39+
                          ' AND (B.mdl='+#39+'HWSusrwebsite.exe'+#39+
                          ' OR B.mdl='+#39+'HWSusrsis.exe'+#39+')'+
                          ' AND B.codigo=A.cod_grp';
           Open;
           First;
           clbAreas.Clear;
           clbAreas_cod.Clear;
           clbAreas_tp.Clear;
           While not Eof do begin
               clbAreas.Items.Add(fieldbyname('descricao').AsString);
               clbAreas_cod.Add(fieldbyname('cod_grp').AsString);
               clbAreas_tp.Add(fieldbyname('cp5').AsString);
               Next;
           end;
           Close;
      end;
...
procedure Tmdl_usrsac.btareasDelClick(Sender: TObject);
begin
      montaAreasSelect(1);
end;

procedure Tmdl_usrsac.btareasIncClick(Sender: TObject);
begin
      montaAreasSelect(0);
end;

procedure Tmdl_usrsac.SpeedButton2Click(Sender: TObject);
begin
      if xcodigo2.Text='novo' then begin
         PageControl2.TabIndex:=0;
         bt_save2.Click;
      end else begin
         if montaAreasSelect(3) then
            hwsf.BtMensagem('Dados gravados com sucesso.');
      end;
end;

procedure Tmdl_usrsac.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
