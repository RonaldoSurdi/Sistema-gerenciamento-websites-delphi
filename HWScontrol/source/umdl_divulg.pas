...
procedure Tmdl_divulg.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure Tmdl_divulg.FormCreate(Sender: TObject);
var axshowing: Boolean;
...
begin
     ...
     mdl_divulg.Caption:= AxDadosUsr.axtitle;
     xinit:=true;
     axcod_ent:=0;
...
procedure Tmdl_divulg.monta_lng;
...
procedure Tmdl_divulg.monta_table1;
var axshowing: Boolean;
...
procedure Tmdl_divulg.monta_table2(xnewgal,xuplng:Boolean);
var xcodgal: string;
begin
...
procedure Tmdl_divulg.FormResize(Sender: TObject);
begin
...
procedure Tmdl_divulg.FormActivate(Sender: TObject);
begin
      mdl_divulg.Refresh;
      if xinit then begin
         xinit:=false;
...
procedure Tmdl_divulg.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      Case key of
...
procedure Tmdl_divulg.travaformImages(xtrava: Boolean);
begin
...
procedure Tmdl_divulg.atualiza_grade;
var axshowing: Boolean;
    axlocid: Integer;
...
procedure Tmdl_divulg.atualiza_grade2;
var axshowing: Boolean;
    st_stream : TMemoryStream;
begin
     //banco de imagens
      ...
      if dm.ds_sql3.RecordCount <= 0 then begin
         ...
         with dm.ds_sql3 do begin
            ...
            st_stream.Free;
            ab_url.Text:=fieldbyname('url').AsString;
            loadx.Progressloading.position:=100;
...
procedure Tmdl_divulg.limpaphoto2(xupdateset:Boolean);
begin
...
procedure Tmdl_divulg.bt_new1Click(Sender: TObject);
begin
      addreg;
      ab_titulo.SetFocus;
end;

procedure Tmdl_divulg.bt_new2Click(Sender: TObject);
begin
      addreg2;
      ms_titulo.SetFocus;
end;

procedure Tmdl_divulg.bt_del1Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
...
procedure Tmdl_divulg.bt_save1Click(Sender: TObject);
...
procedure Tmdl_divulg.ComboBox_webChange(Sender: TObject);
begin
...
procedure Tmdl_divulg.bt_save2Click(Sender: TObject);
var axcod,axcod2,axgal_id,xidxlng,xid_default,axcodinsert : integer;
    axshowing,xexecutaimg,axInc_traduzir: Boolean;
    xtxt1: String;
    xtxt2: TStringList;
    xtxt3: TMemoryStream;
    xLinkURL,xLinkTarjet: String;
    function limpaText(xtext:String):String;
    var xresult:String;
    begin
        xresult:=xtext;
        xresult:=LowerCase(xresult);
        xresult:=Trim(xresult);
        xresult:=StringReplace(xresult,'(','',[rfReplaceAll, rfIgnoreCase]);
        xresult:=StringReplace(xresult,')','',[rfReplaceAll, rfIgnoreCase]);
        xresult:=StringReplace(xresult,#39,'',[rfReplaceAll, rfIgnoreCase]);
        Result:=xresult
    end;
begin
     axcodinsert:=0;
     ...
     xLinkURL:=limpaText(ab_url.Text);
     xLinkTarjet:=limpaText(ab_tarjet.Text);
     if xLinkURL<>'' then begin
        if xLinkTarjet='' then begin
           hwsf.BtMensagem('É necessário selecionar o destino.');
           ab_tarjet.SetFocus;
           exit;
        end;
     end else xLinkTarjet:='';
     if ab_timer.Value<1 then begin
        hwsf.BtMensagem('Tempo de exibição inválido.');
        ab_timer.SetFocus;
        exit;
     end;
     axshowing:= loadx.Showing;
     ...
     if ms_cod.Caption = 'novo' then begin
        ...
        for axgal_id:=0 to listaLng.count-1 do begin
            ...
            with dm.executa do begin
                 CommandText:= 'INSERT INTO '+LstTables.Table[2]+' (codigo,cod_img,cod_gal,cod_lng,descricao,textft,blb,url,trj,tir) '+
                 ...
                 Params[7].AsString:=ab_url.Text;
                 Params[8].AsString:=ab_tarjet.Text;
                 Params[9].AsInteger:=ab_timer.Value;
                 try
                     Execute;
                 except
                     try loadx.Close; except end;
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+LstTables.Table[2]+' SET descricao=:p0,textft=:p1,blb=:p2,url=:p3,trj=:p4,tir=:p5 '+
                       'WHERE codigo=:p6';
            ...
            xtxt3.Seek(0,soFromBeginning);
            Params[2].LoadFromStream(xtxt3,ftblob);
            Params[3].AsString:=ab_url.Text;
            Params[4].AsString:=ab_tarjet.Text;
            Params[5].AsInteger:=ab_timer.Value;
...
procedure Tmdl_divulg.getLogo2;
...
procedure Tmdl_divulg.abretmplogo2;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp2.Seek(0,soFromBeginning);
...
procedure Tmdl_divulg.getBanner;
begin
     xbannertmp.Clear;
...
procedure Tmdl_divulg.abretmpbanner;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xbannertmp.Seek(0,soFromBeginning);
...
procedure Tmdl_divulg.bt_limpar2Click(Sender: TObject);
begin
      limpaphoto2(True);
...
procedure Tmdl_divulg.bt_open2Click(Sender: TObject);
...
procedure Tmdl_divulg.chk_view2Click(Sender: TObject);
begin
      if chk_view2.Checked then getLogo2;
end;

procedure Tmdl_divulg.Splitter1Moved(Sender: TObject);
begin
      ComboBox_web.Width:=ToolBar_web.Width-53;
end;

procedure Tmdl_divulg.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tmdl_divulg.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_divulg.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_divulg.DBGrid1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade;
end;

procedure Tmdl_divulg.DBGrid2CellClick(Column: TColumn);
begin
      atualiza_grade2;
end;

procedure Tmdl_divulg.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_divulg.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_divulg.DBGrid2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade2;
end;

procedure Tmdl_divulg.bt_recursiveClick(Sender: TObject);
...
function Tmdl_divulg.inserimg(xnamefile,xfilext:String;
                      xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
                      xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp:Integer;
                      xic_ajt,xic_pr,xil_ajt,xil_pr: Boolean):Boolean;
...
function Tmdl_divulg.retorna_TTabSheet(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtb1;
          1: Result:=lngtb2;
...
procedure Tmdl_divulg.PageControl_languagensChange(Sender: TObject);
begin
...
procedure Tmdl_divulg.Timer_traduzTimer(Sender: TObject);
begin
      Inc(xtimerresponse);
end;

procedure Tmdl_divulg.Tradutor1AoTraduzir(Sender: TObject;
  Traducao: String);
begin
    xreceivetrad:=Traducao;
end;

function Tmdl_divulg.recebetraducao(xtrad_text,xtrad_de,xtrad_para: String):String;
var xrecebido: Bool;
begin
      xrecebido:=false;
      xreceivetrad:='';
...
procedure Tmdl_divulg.save_cfg(xcodlng: Integer);
var axcod : integer;
    axshowing: Boolean;
begin
     axshowing:= loadx.Showing;
...
procedure Tmdl_divulg.monta_table3;
var listaLng_id,axcoddefault,xid_default: Integer;
    axshowing: Boolean;
    //inclui linguagens
...
procedure Tmdl_divulg.atualiza_grade3;
var axshowing: Boolean;
    axlocid: Integer;
begin
     //banner
...
procedure Tmdl_divulg.limpaphoto3(xupdateset:Boolean);
begin
...
procedure Tmdl_divulg.bt_open3Click(Sender: TObject);
var xfilext,xnamefile: String;
    xerror: Boolean;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
...
procedure Tmdl_divulg.bt_limpar3Click(Sender: TObject);
begin
      limpaphoto3(true);
end;

procedure Tmdl_divulg.bt_save3Click(Sender: TObject);
begin
      save_cfg(listaLng.cod_lng[PageControl_languagens.TabIndex]);
end;

procedure Tmdl_divulg.PageControl1Change(Sender: TObject);
begin
...
procedure Tmdl_divulg.MenuItem1Click(Sender: TObject);
var axshowing: Boolean;
begin
...
procedure Tmdl_divulg.bt_del2Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
...
procedure Tmdl_divulg.ComboBox_entChange(Sender: TObject);
begin
      if StrtoInt(AxDadosUsr.axparm) > 5000 then begin
...
procedure Tmdl_divulg.bt_helpClick(Sender: TObject);
begin
      //Tópicos de ajuda
...
procedure Tmdl_divulg.StatusBar1Click(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tmdl_divulg.ToolBar_grpResize(Sender: TObject);
begin
     cb_group.Width := ToolBar_grp.Width - 50;
end;

procedure Tmdl_divulg.Desfazer1Click(Sender: TObject);
begin
     //desfazer
...
procedure Tmdl_divulg.Recortar1Click(Sender: TObject);
begin
     //recortar
     retorna_TRxRichEdit(PageControlab.TabIndex).CutToClipboard;
end;

procedure Tmdl_divulg.Copiar1Click(Sender: TObject);
begin
     //copiar
     retorna_TRxRichEdit(PageControlab.TabIndex).CopyToClipboard;
end;

procedure Tmdl_divulg.Colar1Click(Sender: TObject);
begin
      //colar
     retorna_TRxRichEdit(PageControlab.TabIndex).PasteFromClipboard;
end;

procedure Tmdl_divulg.Cor1Click(Sender: TObject);
begin
     //cor da fonte
...
procedure Tmdl_divulg.Negrito1Click(Sender: TObject);
begin
     //negrito
     if FUpdating then Exit;
...
procedure Tmdl_divulg.Italico1Click(Sender: TObject);
begin
     //italico
     if FUpdating then Exit;
...
procedure Tmdl_divulg.Sublinhado1Click(Sender: TObject);
begin
     //sublinhado
     if FUpdating then Exit;
...
procedure Tmdl_divulg.Marcador1Click(Sender: TObject);
begin
     //marcador
     if FUpdating then Exit;
...
procedure Tmdl_divulg.AlignButtonClick(Sender: TObject);
begin
...
procedure Tmdl_divulg.esquerdo1Click(Sender: TObject);
begin
     retorna_LeftAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_divulg.centro1Click(Sender: TObject);
begin
     retorna_CenterAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_divulg.direita1Click(Sender: TObject);
begin
     retorna_RightAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_divulg.justificado1Click(Sender: TObject);
begin
     retorna_JustifyAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_divulg.re_editorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tmdl_divulg.SelectionChange(Sender: TObject);
begin
  //if processoand then exit;
...
function Tmdl_divulg.CurrText: TRxTextAttributes;
begin
...
procedure Tmdl_divulg.SetEditRect;
var
  R: TRect;
begin
  with retorna_TRxRichEdit(PageControlab.TabIndex) do
...
procedure Tmdl_divulg.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
function Tmdl_divulg.retorna_TRxRichEdit(xidxcod:Integer):TRxRichEdit;
begin
      Case xidxcod of
          0: Result:=re_editor1;
          1: Result:=re_editor2;
...
function Tmdl_divulg.retorna_fontName(xidxcod:Integer):TComboBox;
begin
      Case xidxcod of
          0: Result:=FontName1;
          1: Result:=FontName2;
...
function Tmdl_divulg.retorna_fontSize(xidxcod:Integer):TEdit;
begin
      Case xidxcod of
          0: Result:=FontSize1;
          1: Result:=FontSize2;
...
function Tmdl_divulg.retorna_BoldButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_divulg.retorna_ItalicButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_divulg.retorna_UnderlineButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_divulg.retorna_LeftAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
          0: Result:=LeftAlign1;
          1: Result:=LeftAlign2;
...
function Tmdl_divulg.retorna_CenterAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_divulg.retorna_RightAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_divulg.retorna_JustifyAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_divulg.retorna_BulletsButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
procedure Tmdl_divulg.cb_groupChange(Sender: TObject);
begin
...
procedure Tmdl_divulg.EVsaveKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_divulg.EVsave2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_divulg.Cordefundo1Click(Sender: TObject);
begin
      //cor de fundo
      if ColorDialog1.Execute then begin
...
function Tmdl_divulg.Btsearch_auto_ax(pathdefault,filtrofind:string):Tbt_search_ax;
...
procedure Tmdl_divulg.bt_view2Click(Sender: TObject);
begin
      if dm.ds_sql3.RecordCount>0 then begin
         ...
         mdl_richgalery_view.Left:=mdl_divulg.Width-mdl_richgalery_view.Width;
         mdl_richgalery_view.Top:=mdl_divulg.Height-mdl_richgalery_view.Height;
         mdl_richgalery_view.Show;
      end;
end;

procedure Tmdl_divulg.bt_exp2Click(Sender: TObject);
var xfiljpg : String;
...
procedure Tmdl_divulg.SpeedButton2Click(Sender: TObject);
...
procedure Tmdl_divulg.bt_imgRotateh2Click(Sender: TObject);
begin
...
procedure Tmdl_divulg.bt_imgRotatea2Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp2,xlogosz2,xupdatelogo2,ComboBox_model.Text,LstTables.Table[3],'cod_img','img_ico',ms_cod.Caption) then begin
        if dm.rotateFoto(-90,xampltmp2,xamplsz2,xupdatelogo2,ComboBox_model.Text,LstTables.Table[3],'cod_img','img_larg',ms_cod.Caption) then
           abretmplogo2
...
procedure Tmdl_divulg.bt_invertmv1Click(Sender: TObject);
begin
      invertEdt(false);
end;

procedure Tmdl_divulg.invertEdt(axIv:Boolean);
var tx : TStringList;
begin
      try
...
procedure Tmdl_divulg.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
