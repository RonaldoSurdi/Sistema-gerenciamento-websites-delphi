...
uses
  ...
  umdl_richnews in 'umdl_richnews.pas' {mdl_richnews},
...
begin
    ...
    Application.Title := 'HWScontrol Notícias Redator';
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richnews, mdl_richnews);
                if (ParamStr(5) <> '') then
                    mdl_richnews.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richnews.xiniCodab:= 0;
             end else begin
