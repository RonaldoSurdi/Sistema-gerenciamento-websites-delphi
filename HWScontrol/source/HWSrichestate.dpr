...
uses
  ...
  umdl_richestate in 'umdl_richestate.pas' {mdl_richestate},
...
begin
    ...
    Application.Title := 'HWScontrol Imóveis';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '')  and (ParamStr(4) <> '')then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richestate, mdl_richestate);
                if (ParamStr(5) <> '') then
                    mdl_richestate.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richestate.xiniCodab:= 0;
             end else begin
