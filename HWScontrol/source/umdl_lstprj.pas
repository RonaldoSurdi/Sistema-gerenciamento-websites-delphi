...
uses
  ...
  Tradutor, RxRichEd, ImgList, AppEvnts, Spin;
...
type
  ...
  Tmdl_lstprj = class(TForm)
    ...
    GroupBox8: TGroupBox;
    TrackBarFAS: TTrackBar;
    LabelFAS: TLabel;
    Panel34: TPanel;
    SpinEditFAS: TSpinEdit;
    chk_clone: TCheckBox;
    ...
    procedure SpinEditFASChange(Sender: TObject);
    procedure TrackBarFASChange(Sender: TObject);
  private
    //menus
    axposFASexec: Boolean;
    ...
    function ResultFAS(xidx:Integer):String;
    function AjustFas(xidFAS:Integer):Boolean;
...
procedure Tmdl_lstprj.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure Tmdl_lstprj.FormCreate(Sender: TObject);
var axshowing: Boolean;
...
begin
     ...
     mdl_lstprj.Caption:= AxDadosUsr.axtitle;
     axposFASexec:=false;
     xinit:=true;
     axcod_ent:=0;
...
procedure Tmdl_lstprj.monta_lng;
...
procedure Tmdl_lstprj.monta_table1;
var axshowing: Boolean;
    xfiltdatax,xfiltdata,xfiltdata2,xcodcls: String;
begin
     axshowing:= loadx.Showing;
     if not axshowing then begin
        loadx := Tloadx.Create(Application);
     ...
     With dm.ds_sql do begin
          ...
          //Filter:=xdsqlfilter;
          if dm.ds_sql6.RecordCount>0 then xcodcls:=dm.ds_sql6.fieldbyname('cod_class').AsString
          else xcodcls:='0';
          Filter:='(('+xdsqlfilter+') AND (cod_class='+#39+xcodcls+#39+'))';
     end;
     atualiza_grade;
     try loadx.Close; except end;
end;

procedure Tmdl_lstprj.monta_table2(xnewgal,xuplng:Boolean);
var xcodgal: string;
begin
...
procedure Tmdl_lstprj.FormResize(Sender: TObject);
begin
...
procedure Tmdl_lstprj.FormActivate(Sender: TObject);
begin
      mdl_lstprj.Refresh;
      if xinit then begin
         xinit:=false;
...
procedure Tmdl_lstprj.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      Case key of
...
procedure Tmdl_lstprj.travaformImages(xtrava: Boolean);
begin
     ...
     TabSheet_itens.TabVisible :=false;//xtrava;
end;

procedure Tmdl_lstprj.addreg2;
begin
     ms_cod.Caption:='novo';
     ax_ms_cod:=0;
     ms_titulo.Text:='';
...
procedure Tmdl_lstprj.atualiza_grade;
var axshowing: Boolean;
    axlocid: Integer;
...
begin
     ...
     axshowing:= loadx.Showing;
      ...
      try loadx.Close; except end;
      ax_ab_cod:=dm.ds_sql.fieldbyname('codigo').AsInteger;
end;

procedure Tmdl_lstprj.atualiza_grade3;
var axshowing,xuplng: Boolean;
    axlocid,xpostrackfas: Integer;
    st_stream: TMemoryStream;
begin
     if PageControlab.TabIndex>0 then exit;
     axshowing:= loadx.Showing;
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
      ...
      if dm.ds_sql6.RecordCount <= 0 then begin
         ...
         with dm.ds_sql6 do begin
            ...
            loadx.Progressloading.position:=40;
            axposFASexec:=true;
            xpostrackfas:=11-fieldbyname('xfas').AsInteger;
            TrackBarFAS.SelStart:=xpostrackfas;
            TrackBarFAS.Position:=xpostrackfas;
            SpinEditFAS.Value:=fieldbyname('xfas').AsInteger+1;
            ResultFAS(xpostrackfas);
            axposFASexec:=false;
            loadx.Progressloading.position:=50;
         end;
      end;
...
procedure Tmdl_lstprj.limpaphoto1(xupdateset:Boolean);
begin
...
procedure Tmdl_lstprj.atualiza_grade2;
var axshowing: Boolean;
    st_stream : TMemoryStream;
begin
     //banco de imagens
...
procedure Tmdl_lstprj.limpaphoto2(xupdateset:Boolean);
begin
...
procedure Tmdl_lstprj.limpaphoto3(xupdateset:Boolean);
begin
...
procedure Tmdl_lstprj.bt_new1Click(Sender: TObject);
begin
      addreg;
      ab_titulo.SetFocus;
end;

procedure Tmdl_lstprj.bt_new2Click(Sender: TObject);
begin
      //addreg2;
      ms_titulo.SetFocus;
end;

procedure Tmdl_lstprj.bt_del1Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
     ...
     if (dm.ds_sql.RecordCount>0) and (ab_cod.Caption<>'novo') then begin
        ...
        if dm.deleta_reg(LstTables.Table[0],'cod_gal',ab_cod.Caption,true) then begin
           ...
           dm.deleta_reg(LstTables.Table[7],'cod_gal',ab_cod.Caption,false);
           loadx.Progressloading.position:=75;
...
procedure Tmdl_lstprj.bt_save1Click(Sender: TObject);
...
begin
     ...
     cb_loc.Text:=StringReplace(cb_loc.Text, '’’', '"', [rfReplaceAll]);
     cb_loc.Text:=StringReplace(cb_loc.Text, '’', #39, [rfReplaceAll]);
     cb_loc.Text:=StringReplace(cb_loc.Text, #39+#39, '"', [rfReplaceAll]);
     ...
     if ab_cod.Caption = 'novo' then begin
        ...
        with dm.executa do begin
            if chk_clone.Checked then begin
...
procedure Tmdl_lstprj.ComboBox_webChange(Sender: TObject);
begin
...
procedure Tmdl_lstprj.bt_save2Click(Sender: TObject);
...
procedure Tmdl_lstprj.getLogo1;
...
procedure Tmdl_lstprj.getLogo3;
var axcodBlb: Integer;
begin
     try
...
procedure Tmdl_lstprj.abretmplogo1;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp1.Seek(0,soFromBeginning);
...
procedure Tmdl_lstprj.abretmplogo3;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp3.Seek(0,soFromBeginning);
...
procedure Tmdl_lstprj.getLogo2;
var axcodBlb: Integer;
begin
     try
...
procedure Tmdl_lstprj.abretmplogo2;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp2.Seek(0,soFromBeginning);
...
procedure Tmdl_lstprj.getBanner;
begin
     xbannertmp.Clear;
...
procedure Tmdl_lstprj.abretmpbanner;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xbannertmp.Seek(0,soFromBeginning);
...
procedure Tmdl_lstprj.bt_limpar1Click(Sender: TObject);
begin
      limpaphoto1(True);
...
procedure Tmdl_lstprj.bt_limpar2Click(Sender: TObject);
begin
      limpaphoto2(True);
...
procedure Tmdl_lstprj.bt_view1Click(Sender: TObject);
begin
      if dm.ds_sql.RecordCount>0 then begin
         ...
         mdl_richgalery_view.Left:=mdl_lstprj.Width-mdl_richgalery_view.Width;
         mdl_richgalery_view.Top:=mdl_lstprj.Height-mdl_richgalery_view.Height;
         mdl_richgalery_view.Show;
      end;
      //getLogo1;
end;

procedure Tmdl_lstprj.bt_open1Click(Sender: TObject);
...
procedure Tmdl_lstprj.bt_open2Click(Sender: TObject);
var xfilext,xnamefile: String;
    xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
    xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp: Integer;
...
procedure Tmdl_lstprj.chk_view2Click(Sender: TObject);
begin
      if chk_view2.Checked then getLogo2;
end;

procedure Tmdl_lstprj.Splitter1Moved(Sender: TObject);
begin
      ComboBox_web.Width:=ToolBar_web.Width-53;
end;

procedure Tmdl_lstprj.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tmdl_lstprj.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_lstprj.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_lstprj.DBGrid1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade;
end;

procedure Tmdl_lstprj.DBGrid2CellClick(Column: TColumn);
begin
      atualiza_grade2;
end;

procedure Tmdl_lstprj.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_lstprj.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_lstprj.DBGrid2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade2;
end;

procedure Tmdl_lstprj.bt_view2Click(Sender: TObject);
begin
      if dm.ds_sql3.RecordCount>0 then begin
         Application.CreateForm(Tmdl_richgalery_view,mdl_richgalery_view);
         mdl_richgalery_view.TableImage.xcp_cod:='codigo';
...
procedure Tmdl_lstprj.bt_recursiveClick(Sender: TObject);
...
function Tmdl_lstprj.inserimg(xnamefile,xfilext:String;
                      xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
                      xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp:Integer;
                      xic_ajt,xic_pr,xil_ajt,xil_pr: Boolean):Boolean;
...
function Tmdl_lstprj.retorna_TTabSheet(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtb1;
          1: Result:=lngtb2;
...
procedure Tmdl_lstprj.PageControl_languagensChange(Sender: TObject);
begin
...
procedure Tmdl_lstprj.Timer_traduzTimer(Sender: TObject);
begin
      Inc(xtimerresponse);
end;

procedure Tmdl_lstprj.Tradutor1AoTraduzir(Sender: TObject;
  Traducao: String);
begin
    xreceivetrad:=Traducao;
end;

function Tmdl_lstprj.recebetraducao(xtrad_text,xtrad_de,xtrad_para: String):String;
var xrecebido: Bool;
begin
      xrecebido:=false;
      xreceivetrad:='';
...
procedure Tmdl_lstprj.save_cfg(xcodlng: Integer);
var axcod : integer;
    axshowing: Boolean;
begin
     axshowing:= loadx.Showing;
...
procedure Tmdl_lstprj.monta_table3;
var listaLng_id,axcoddefault,xid_default: Integer;
    axshowing: Boolean;
    //inclui linguagens
...
procedure Tmdl_lstprj.atualiza_grade_pers;
var axshowing: Boolean;
    axlocid: Integer;
begin
     //banner
...
procedure Tmdl_lstprj.limpaphoto_pers(xupdateset:Boolean);
begin
...
procedure Tmdl_lstprj.bt_open_persClick(Sender: TObject);
var xfilext,xnamefile: String;
    xerror: Boolean;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
...
procedure Tmdl_lstprj.bt_limpar_persClick(Sender: TObject);
begin
      limpaphoto_pers(true);
end;

procedure Tmdl_lstprj.bt_save_persClick(Sender: TObject);
begin
      save_cfg(listaLng.cod_lng[PageControl_languagens.TabIndex]);
end;

procedure Tmdl_lstprj.PageControl1Change(Sender: TObject);
begin
...
procedure Tmdl_lstprj.MenuItem1Click(Sender: TObject);
var axshowing: Boolean;
begin
...
procedure Tmdl_lstprj.bt_del2Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
...
procedure Tmdl_lstprj.ComboBox_entChange(Sender: TObject);
begin
      if StrtoInt(AxDadosUsr.axparm) > 5000 then begin
...
procedure Tmdl_lstprj.bt_helpClick(Sender: TObject);
begin
      //Tópicos de ajuda
...
procedure Tmdl_lstprj.StatusBar1Click(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tmdl_lstprj.ToolBar_grpResize(Sender: TObject);
begin
     cb_group.Width := ToolBar_grp.Width - 50;
end;

procedure Tmdl_lstprj.Desfazer1Click(Sender: TObject);
begin
     //desfazer
...
procedure Tmdl_lstprj.Recortar1Click(Sender: TObject);
begin
     //recortar
     retorna_TRxRichEdit(PageControlab.TabIndex).CutToClipboard;
end;

procedure Tmdl_lstprj.Copiar1Click(Sender: TObject);
begin
     //copiar
     retorna_TRxRichEdit(PageControlab.TabIndex).CopyToClipboard;
end;

procedure Tmdl_lstprj.Colar1Click(Sender: TObject);
begin
      //colar
     retorna_TRxRichEdit(PageControlab.TabIndex).PasteFromClipboard;
end;

procedure Tmdl_lstprj.Cor1Click(Sender: TObject);
begin
     //cor da fonte
...
procedure Tmdl_lstprj.Negrito1Click(Sender: TObject);
begin
     //negrito
     if FUpdating then Exit;
...
procedure Tmdl_lstprj.Italico1Click(Sender: TObject);
begin
     //italico
     if FUpdating then Exit;
...
procedure Tmdl_lstprj.Sublinhado1Click(Sender: TObject);
begin
     //sublinhado
     if FUpdating then Exit;
...
procedure Tmdl_lstprj.Marcador1Click(Sender: TObject);
begin
     //marcador
     if FUpdating then Exit;
...
procedure Tmdl_lstprj.AlignButtonClick(Sender: TObject);
begin
...
procedure Tmdl_lstprj.esquerdo1Click(Sender: TObject);
begin
     retorna_LeftAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_lstprj.centro1Click(Sender: TObject);
begin
     retorna_CenterAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_lstprj.direita1Click(Sender: TObject);
begin
     retorna_RightAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_lstprj.justificado1Click(Sender: TObject);
begin
     retorna_JustifyAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_lstprj.re_editorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tmdl_lstprj.SelectionChange(Sender: TObject);
begin
  //if processoand then exit;
...
function Tmdl_lstprj.CurrText: TRxTextAttributes;
begin
...
procedure Tmdl_lstprj.SetEditRect;
var
  R: TRect;
begin
  with retorna_TRxRichEdit(PageControlab.TabIndex) do
...
procedure Tmdl_lstprj.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
function Tmdl_lstprj.retorna_TRxRichEdit(xidxcod:Integer):TRxRichEdit;
begin
      Case xidxcod of
          0: Result:=re_editor3;
          1: Result:=re_editor1;
...
function Tmdl_lstprj.retorna_fontName(xidxcod:Integer):TComboBox;
begin
      Case xidxcod of
          0: Result:=FontName3;
          1: Result:=FontName1;
...
function Tmdl_lstprj.retorna_fontSize(xidxcod:Integer):TEdit;
begin
      Case xidxcod of
          0: Result:=FontSize3;
          1: Result:=FontSize1;
...
function Tmdl_lstprj.retorna_BoldButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_lstprj.retorna_ItalicButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_lstprj.retorna_UnderlineButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_lstprj.retorna_LeftAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
          0: Result:=LeftAlign3;
          1: Result:=LeftAlign1;
...
function Tmdl_lstprj.retorna_CenterAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_lstprj.retorna_RightAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_lstprj.retorna_JustifyAlign(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
function Tmdl_lstprj.retorna_BulletsButton(xidxcod:Integer):TToolButton;
begin
      Case xidxcod of
...
procedure Tmdl_lstprj.cb_groupChange(Sender: TObject);
begin
...
procedure Tmdl_lstprj.monta_tableclass;
...
procedure Tmdl_lstprj.PanelclassResize(Sender: TObject);
begin
    ComboBox_class.Width := Panelclass.Width-11;
end;

procedure Tmdl_lstprj.EVsaveKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_lstprj.EVsave2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_lstprj.Cordefundo1Click(Sender: TObject);
begin
      //cor de fundo
      if ColorDialog1.Execute then begin
...
procedure Tmdl_lstprj.validaCbClass();
var axfilterIDCl,axfilterIDCl_ax1,axfilterIDCl_ax2: String;
begin
      if (ComboBox_class.ItemIndex > -1) then begin
          ...
          dm.ds_sql.Filter:='(('+xdsqlfilter+') AND (cod_class='+#39+'0'+#39+'))';
end;

function Tmdl_lstprj.Btsearch_auto_ax(pathdefault,filtrofind:string):Tbt_search_ax;
...
procedure Tmdl_lstprj.ComboBox_classKeyPress(Sender: TObject;
  var Key: Char);
begin
      if key=#13 then validaCbClass();
end;

procedure Tmdl_lstprj.ComboBox_classClick(Sender: TObject);
begin
      validaCbClass();
end;

procedure Tmdl_lstprj.bt_new3Click(Sender: TObject);
begin
      addreg3;
      cl_titulo.SetFocus;
end;

procedure Tmdl_lstprj.bt_del3Click(Sender: TObject);
...
procedure Tmdl_lstprj.DBGrid3CellClick(Column: TColumn);
begin
      atualiza_grade3;
end;

procedure Tmdl_lstprj.DBGrid3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade3;
end;

procedure Tmdl_lstprj.DBGrid3KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade3;
end;

procedure Tmdl_lstprj.DBGrid3MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade3;
end;

procedure Tmdl_lstprj.bt_save3Click(Sender: TObject);
var axcod,axcod2,axcl_id,xidxlng,xid_default,ax_cl_codinsert: integer;
    ...
    axposFASset: Integer;
begin
     if ComboBox_class2.ItemIndex<0 then
     ...
     try
        axposFASset:=SpinEditFAS.Value-1;
     except
        axposFASset:=0;
     end;
     Application.ProcessMessages;
     ...
     if cl_cod.Caption = 'novo' then begin
        ...
        for axcl_id:=0 to listaLng.count-1 do begin
            ...
            with dm.executa do begin
                 CommandText:= 'INSERT INTO '+LstTables.Table[0]+'_ax (codigo,cod_class,cod_web,cod_lng,cod_ls,cp1,cp2,cp3,xtp,xfas) '+
                 ...
                 Params[9].AsInteger:= axposFASset;
                 try
                     Execute;
                 except
                     try loadx.Close; except end;
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+LstTables.Table[0]+'_ax SET cp1=:p0,cp2=:p1,cp3=:p2,xfas=:p3 '+
                       'WHERE codigo=:p4';
            loadx.Progressloading.position:=20;
            ...
            xtxt3.Seek(0,soFromBeginning);
            Params[2].LoadFromStream(xtxt3,ftblob);
            Params[3].AsInteger:= axposFASset;
            Params[4].AsInteger:=ax_cl_cod;
            try
                Execute;
                loadx.Progressloading.position:=30;
            except
...
procedure Tmdl_lstprj.bt_exp1Click(Sender: TObject);
var xfiljpg,xfileext : String;
    xImagem_tmp : TMemoryStream;
begin
      if ExportarImgDialog.Execute then begin
         xfiljpg := ExportarImgDialog.FileName;
         xfileext:= ExtractFileExt(xfiljpg);
         if (LowerCase(xfileext) <> '.jpg') then
            xfiljpg := xfiljpg + '.jpg';
...
procedure Tmdl_lstprj.bt_exp2Click(Sender: TObject);
var xfiljpg,xfileext : String;
    xImagem_tmp : TMemoryStream;
begin
      if ExportarImgDialog.Execute then begin
...
procedure Tmdl_lstprj.bt_open3Click(Sender: TObject);
var xfilext,xnamefile: String;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
    codAbrirtmp: TPicture;
...
procedure Tmdl_lstprj.bt_limpar3Click(Sender: TObject);
begin
      limpaphoto3(True);
      if hwsf.BtConfirma('Gravar dados?') then begin
         bt_save3.Click;
...
procedure Tmdl_lstprj.bt_view3Click(Sender: TObject);
begin
      if dm.ds_sql6.RecordCount>0 then begin
         Application.CreateForm(Tmdl_richgalery_view,mdl_richgalery_view);
         mdl_richgalery_view.TableImage.xcp_cod:='codigo';
...
procedure Tmdl_lstprj.chk_view3Click(Sender: TObject);
begin
      if chk_view3.Checked then getLogo3;
end;

procedure Tmdl_lstprj.SpeedButton2Click(Sender: TObject);
...
procedure Tmdl_lstprj.bt_exp3Click(Sender: TObject);
var xfiljpg,xfileext : String;
    xImagem_tmp : TMemoryStream;
begin
      if ExportarImgDialog.Execute then begin
         ...
         xImagem_tmp.Clear;
         xImagem_tmp.Position:=0;
         xImagem_tmp := dm.GetBinaryStreamPH(LstTables.Table[1]+'_ax','cod_class','icon',cl_cod.Caption);
         xImagem_tmp.Seek(0,soFromBeginning);
         xImagem_tmp.SaveToFile(xfiljpg);
         xImagem_tmp.Free;
      end;
...
procedure Tmdl_lstprj.chk_view1Click(Sender: TObject);
begin
      if chk_view1.Checked then getLogo1;
end;

procedure Tmdl_lstprj.Panelbtview3Resize(Sender: TObject);
begin
    bt_view3.Height:=Panelbtview3.Height-49;
end;

procedure Tmdl_lstprj.PanelbtviewResize(Sender: TObject);
begin
    bt_view1.Height:=Panelbtview.Height-49;
end;

procedure Tmdl_lstprj.Panelbtview2Resize(Sender: TObject);
begin
    bt_view2.Height:=Panelbtview2.Height-49;
end;

procedure Tmdl_lstprj.bt_imgRotateh3Click(Sender: TObject);
begin
     if dm.rotateFoto(90,xlogotmp3,xlogosz3,xupdatelogo3,'0',LstTables.Table[0]+'_ax','cod_class','icon',cl_cod.Caption) then
        abretmplogo3;
end;

procedure Tmdl_lstprj.bt_imgRotatea3Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp3,xlogosz3,xupdatelogo3,'0',LstTables.Table[0]+'_ax','cod_class','icon',cl_cod.Caption) then
        abretmplogo3;
end;

procedure Tmdl_lstprj.bt_imgRotateh1Click(Sender: TObject);
begin
...
procedure Tmdl_lstprj.bt_imgRotatea1Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp1,xlogosz1,xupdatelogo1,ComboBox_model.Text,LstTables.Table[1],'cod_gal','icon',ab_cod.Caption) then
        abretmplogo1;
end;

procedure Tmdl_lstprj.bt_imgRotateh2Click(Sender: TObject);
begin
...
procedure Tmdl_lstprj.bt_imgRotatea2Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp2,xlogosz2,xupdatelogo2,ComboBox_model.Text,LstTables.Table[3],'cod_img','img_ico',ms_cod.Caption) then begin
        if dm.rotateFoto(-90,xampltmp2,xamplsz2,xupdatelogo2,ComboBox_model.Text,LstTables.Table[3],'cod_img','img_larg',ms_cod.Caption) then
           abretmplogo2
...
procedure Tmdl_lstprj.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
      hwsf.BtMensagem('AVISO: Falha na conexão.');
end;

procedure Tmdl_lstprj.PageControlabChange(Sender: TObject);
begin
     setPaCtrlLab;
end;

function Tmdl_lstprj.ResultFAS(xidx:Integer):String;
var xResultFAS: String;
begin
      Case xidx of
           0: xResultFAS:='01. Assinatura do Contrato entre '+#13+
                          '      Ecobio e Cliente.';
           1: xResultFAS:='02. Fornecimento pelos Clientes da Documentação '+#13+
                          '      Necessária para Projetos de Mecanismo de '+#13+
                          '      Desenvolvimento Limpo no Brasil (MDL)';
           2: xResultFAS:='03. Confecção do Documento de '+#13+
                          '      Concepção do Projeto (DCP)';
           3: xResultFAS:='04. Submissão do DCP a Entidade '+#13+
                          '      Operacional Designada no Brasil (E.O.D)';
           4: xResultFAS:='05. Verificação Preliminar da E.O.D e '+#13+
                          '      encaminhamento do DCP para a ONU';
           5: xResultFAS:='06. Período que o DCP fica na ONU para '+#13+
                          '      comentário das Partes Interessadas';
           6: xResultFAS:='07. Discussão do Relatório de Validação realizado pela E.O.D, '+#13+
                          '      prazo para Correções necessárias do DCP e '+#13+
                          '      visitas da E.O.D nos clientes';
           7: xResultFAS:='08. Encaminhamento da Documentação dos Clientes, '+#13+
                          '      encaminhamento do DCP corrigido e do relatório '+#13+
                          '      de Validação realizado pela D.O.E para a '+#13+
                          '      Comissão Interministerial de Mudança Climática';
           8: xResultFAS:='09. Validação do DCP pelo '+#13+
                          '      Governo Brasileiro';
           9: xResultFAS:='10. Validação do DCP pelo '+#13+
                          '      Executive Board da ONU';
          10: xResultFAS:='11. Prazo para Correções do DCP '+#13+
                          '      sugeridas pelo '+#13+
                          '      Executive Board da ONU';
          11: xResultFAS:='12. Pedido de Registro do DCP '+#13+
                          '      no Executive Board da ONU';
      End;
      Result:=xResultFAS;
end;

procedure Tmdl_lstprj.SpinEditFASChange(Sender: TObject);
begin
      AjustFas(0);
end;

procedure Tmdl_lstprj.TrackBarFASChange(Sender: TObject);
var axposFAS: Integer;
begin
     AjustFas(1);
end;

function Tmdl_lstprj.AjustFas(xidFAS:Integer):Boolean;
var axposFAS:Integer;
begin
      if axposFASexec then exit;
      axposFASexec:=true;
      try
          if xidFAS=0 then
             axposFAS:=SpinEditFAS.Value
          else axposFAS:=(11-TrackBarFAS.Position);
      except
          axposFAS:=-1;
      end;
      if (axposFAS>-1) then begin
         if xidFAS=0 then begin
            TrackBarFAS.Position:=(12-axposFAS);
            TrackBarFAS.Hint:=ResultFAS(TrackBarFAS.Position);
         end else begin
            SpinEditFAS.Value:=(axposFAS+1);
            TrackBarFAS.Hint:=ResultFAS(axposFAS);
         end;
      end;
      axposFASexec:=false;
      Result:=true;
