...
uses
  ...
  umdl_richemp in 'umdl_richemp.pas' {mdl_richemp},
...
begin
    ...
    Application.Title := 'HWScontrol Empresa';
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') and (ParamStr(4) <> '') then begin
          ...
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_richemp, mdl_richemp);
                if (ParamStr(5) <> '') then
                    mdl_richemp.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_richemp.xiniCodab:= 0;
             end else begin
