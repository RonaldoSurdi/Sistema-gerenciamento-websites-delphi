...
uses
  ...
  Mask, DBCtrls, OleCtrls, ExtDlgs, hwsfunctions, Menus, ToolWin,
...
type
  ...
  Tmdl_richdiscotxt = class(TForm)
    ...
    chk_imporiginal: TCheckBox;
    GroupBox2: TGroupBox;
    SpeedButton4: TSpeedButton;
    Label23: TLabel;
    CheckBoxMAIUSCULAS: TCheckBox;
    Panel2: TPanel;
    BitBtn1: TBitBtn;
    CheckBox3: TCheckBox;
    re_editor2txt: TMemo;
    ...
    procedure linkmp3Click(Sender: TObject);
  ...
  private
    ...
    strListPrdAll:TStringList;
    function executalistxml(OpenDialog_XML_Path:String):Boolean;
...
var
  mdl_richdiscotxt: Tmdl_richdiscotxt;
...
procedure Tmdl_richdiscotxt.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure Tmdl_richdiscotxt.FormCreate(Sender: TObject);
var axshowing: Boolean;
...
begin
     ...
     strListPrdAll:=TStringList.Create;
     strListPrdAll.Clear;
     ComboBox_idUPD:=-2;
     xupdatereg:=false;
     feedokupd:=false;
     ...
     mdl_richdiscotxt.Caption:= AxDadosUsr.axtitle;
     xinit:=true;
     axcod_ent:=0;
     PageControltxLast:=-1;
     ...
     resetWebBrowse;
     SendMessage(re_editor2txt.Handle, EM_LIMITTEXT, 0, $FFFFFF);
     //re_editor2.Justify;
...
procedure Tmdl_richdiscotxt.monta_lng;
...
procedure Tmdl_richdiscotxt.monta_table1;
var axshowing: Boolean;
    xfiltdata,xfiltCod: String;
begin
     axshowing:= loadx.Showing;
     if not axshowing then begin
        loadx := Tloadx.Create(Application);
        loadx.lbloading.Caption:='Montando Lista de Itens';
        loadx.Show;
        loadx.Update;
     end else begin
        loadx.lbloading.Caption:='Montando Lista de Itens';
...
procedure Tmdl_richdiscotxt.monta_table2(xnewgal,xuplng:Boolean);
begin
...
procedure Tmdl_richdiscotxt.FormResize(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.FormActivate(Sender: TObject);
begin
      try
        mdl_richdiscotxt.Refresh;
      except
      end;
      if xinit then begin
         xinit:=false;
...
procedure Tmdl_richdiscotxt.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      Case key of
...
procedure Tmdl_richdiscotxt.addreg;
begin
...
procedure Tmdl_richdiscotxt.travaformImages(xtrava: Boolean);
begin
     Panel7.Visible:=xtrava;
     if xtrava then begin
        ms_titulo.Color:=clWindow;
        //re_editor2txt.Color:=clWindow;
...
procedure Tmdl_richdiscotxt.addreg2;
begin
     ms_cod.Caption:='novo';
     ax_ms_cod:=0;
     ms_titulo.Text:='';
...
procedure Tmdl_richdiscotxt.atualiza_grade;
var axshowing: Boolean;
    axlocid: Integer;
     ...
     axshowing:= loadx.Showing;
      ...
      if dm.ds_sql.RecordCount <= 0 then begin
         ...
         with dm.ds_sql do begin
            ...
            //xStringMSG.SaveToFile('C:\Users\IRONSHADOW\Desktop\OsSerranos\03.Discografia\Os Serranos - '+ab_titulo.Text+'.html');
...
procedure Tmdl_richdiscotxt.limpaphoto1(xupdateset:Boolean);
begin
...
procedure Tmdl_richdiscotxt.atualiza_grade2;
var axshowing: Boolean;
     ...
     //banco de imagens
      ...
      if dm.ds_sql3.RecordCount <= 0 then begin
         ...
         with dm.ds_sql3 do begin
            ...
            ms_loc.Text:=fieldbyname('ms_loc').AsString;
            ms_loc2.Text:=fieldbyname('ms_loc2').AsString;
            ms_loc3.Text:=fieldbyname('ms_loc3').AsString;
            ...
            re_editor2txt.Lines.LoadFromStream(st_stream);
            st_stream.Free;
            loadx.Progressloading.position:=100;
         end;
      end;
...
procedure Tmdl_richdiscotxt.limpaphoto2(xupdateset:Boolean);
begin
...
procedure Tmdl_richdiscotxt.bt_new1Click(Sender: TObject);
begin
      addreg;
      ab_titulo.SetFocus;
end;

procedure Tmdl_richdiscotxt.bt_new2Click(Sender: TObject);
begin
      addreg2;
      ms_titulo.SetFocus;
end;

procedure Tmdl_richdiscotxt.bt_save1Click(Sender: TObject);
...
begin
     ...
     if ab_titulo.Text='' then begin
        hwsf.BtMensagem('É necessário digitar o título do Item.');
        ab_titulo.SetFocus;
        exit;
     end;
     ...
     Try
            ...
            if axcp4.Size=0 then begin
               loadx.Close;
                FreeMemStr;
                hwsf.BtMensagem('Digite texto sobre o item.');
                exit;
            end;
            axcp4.Position:=0;
    ...
    if ab_cod.Caption = 'novo' then begin
        ...
        for axgal_id:=0 to listaLng.count-1 do begin
            ...
            with dm.executa do begin
                 CommandText:= 'INSERT INTO '+LstTables.Table[0]+' (codigo,cod_gal,cod_web,cod_lng,cod_psn,cod_ls,descricao,textft,blb,xtp,cod_usrw,cod_class,loc,xdt,xstat) '+
                               'VALUES(:p0,:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,'+#39+FormatDateTime('yyyymmdd',Now)+' '+FormatDateTime('hh:nn:ss',Now)+#39+',:p13)';
        ...
        with dm.executa do begin
            ...
            CommandText:= 'UPDATE '+LstTables.Table[0]+' SET cod_psn=:p0,cod_ls=:p1,cod_class=:p2,loc=:p3 '+
                          'WHERE cod_gal=:p4';
            loadx.Progressloading.position:=40;
...
procedure Tmdl_richdiscotxt.ComboBox_webChange(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.bt_save2Click(Sender: TObject);
...
begin
     ...
     re_editor2txt.Lines.SaveToStream(xtxt3);
     xtxt3.Seek(0,soFromBeginning);
     xtxt2.Clear;
     xtxt2.LoadFromStream(xtxt3);//:= dm.RtfToHtml2('',false,re_editor2);
     if ms_cod.Caption = 'novo' then begin
        ...
        for axgal_id:=0 to listaLng.count-1 do begin
            ...
            with dm.executa do begin
                 CommandText:= 'INSERT INTO '+LstTables.Table[2]+' (codigo,cod_img,cod_gal,cod_lng,descricao,ms_loc,ms_loc2,ms_loc3,textft,blb) '+
                 ...
                 Params[5].AsString:=ms_loc.Text;
                 Params[6].AsString:=ms_loc2.Text;
                 Params[7].AsString:=ms_loc3.Text;
                 Params[8].Assign(xtxt2);
                 xtxt3.Seek(0,soFromBeginning);
                 Params[9].LoadFromStream(xtxt3,ftblob);
                 try
                     Execute;
                 except
                     try loadx.Close; except end;
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+LstTables.Table[2]+' SET descricao=:p0,ms_loc=:p1,ms_loc2=:p2,ms_loc3=:p3,textft=:p4,blb=:p5 '+
                       'WHERE codigo=:p6';
            Params[0].AsString:=ms_titulo.Text;
            Params[1].AsString:=ms_loc.Text;
            Params[2].AsString:=ms_loc2.Text;
            Params[3].AsString:=ms_loc3.Text;
            loadx.Progressloading.position:=20;
            Params[4].Assign(xtxt2);
            xtxt3.Seek(0,soFromBeginning);
            Params[5].LoadFromStream(xtxt3,ftblob);
            Params[6].AsInteger:=ax_ms_cod;
            loadx.Progressloading.position:=30;
        end;
        try
            loadx.Progressloading.position:=40;
...
procedure Tmdl_richdiscotxt.getLogo1;
...
procedure Tmdl_richdiscotxt.abretmplogo1;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xlogotmp1.Seek(0,soFromBeginning);
...
procedure Tmdl_richdiscotxt.getBanner;
begin
     xbannertmp.Clear;
...
procedure Tmdl_richdiscotxt.abretmpbanner;
var erro: Boolean;
    ax_filenm: TFileName;
begin
    xbannertmp.Seek(0,soFromBeginning);
...
procedure Tmdl_richdiscotxt.bt_limpar1Click(Sender: TObject);
begin
      limpaphoto1(True);
...
procedure Tmdl_richdiscotxt.bt_limpar2Click(Sender: TObject);
begin
      limpaphoto2(True);
      if hwsf.BtConfirma('Gravar dados?') then begin
         bt_save2.Click;
...
procedure Tmdl_richdiscotxt.bt_view1Click(Sender: TObject);
begin
      if dm.ds_sql.RecordCount>0 then begin
         ...
         mdl_richgalery_view.Left:=mdl_richdiscotxt.Width-mdl_richgalery_view.Width;
         mdl_richgalery_view.Top:=mdl_richdiscotxt.Height-mdl_richgalery_view.Height;
         mdl_richgalery_view.Show;
      end;
end;

procedure Tmdl_richdiscotxt.bt_open1Click(Sender: TObject);
...
begin
      ...
      if ImportarImgDialog.Execute then begin
         ...
         limpaphoto1(True);
         if ((xfilext='.swf') or (xfilext='.flv') or (chk_imporiginal.Checked)) then begin
              loadx.Progressloading.position:=80;
...
procedure Tmdl_richdiscotxt.bt_open2Click(Sender: TObject);
var xfilext,xnamefile: String;
    xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
    xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp: Integer;
...
procedure Tmdl_richdiscotxt.chk_view1Click(Sender: TObject);
begin
      if chk_view1.Checked then getLogo1;
end;

procedure Tmdl_richdiscotxt.Splitter1Moved(Sender: TObject);
begin
      ComboBox_web.Width:=ToolBar_web.Width-53;
end;

procedure Tmdl_richdiscotxt.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tmdl_richdiscotxt.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_richdiscotxt.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tmdl_richdiscotxt.DBGrid1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade;
end;

procedure Tmdl_richdiscotxt.DBGrid2CellClick(Column: TColumn);
begin
      atualiza_grade2;
end;

procedure Tmdl_richdiscotxt.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_richdiscotxt.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade2;
end;

procedure Tmdl_richdiscotxt.DBGrid2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      atualiza_grade2;
end;

procedure Tmdl_richdiscotxt.bt_recursiveClick(Sender: TObject);
      ...
      if bt_search_ax.filelist.Count>0 then begin
         ...
         for xfor_idimg:=0 to bt_search_ax.filelist.Count-1 do begin
             ...
             if inserimg(xnamefile,xfilext,
                ...
                if not xerror then begin
                   axstatInc.Add('arquivo incluído com sucesso.');
                end;
             end else begin
...
function Tmdl_richdiscotxt.inserimg(xnamefile,xfilext:String;
                      xic_w,xic_h,xic_dpi,xic_cp,xil_w,xil_h,xil_dpi,xil_cp,
                      xic_h_tmp,xic_w_tmp,xil_h_tmp,xil_w_tmp:Integer;
                      xic_ajt,xic_pr,xil_ajt,xil_pr: Boolean):Boolean;
...
begin
        ...
        Try
          ...
          xampltmp2_tmp.Clear;
          xampltmp2_tmp.Seek(0,soFromBeginning);
          if ((xfilext='.swf') or (xfilext='.flv') or (xfilext='.png') or (chk_imporiginal.Checked)) then begin
             xswf := false;
...
function Tmdl_richdiscotxt.retorna_TTabSheet(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtb1;
          1: Result:=lngtb2;
...
procedure Tmdl_richdiscotxt.PageControl_languagensChange(Sender: TObject);
begin
      if PageControl1.TabIndex=0 then begin
         ab_cod.Caption:='0';
         ms_cod.Caption:='0';
         xdsqlfilter:='cod_lng='+#39+IntToStr(listaLng.cod_lng[PageControl_languagens.TabIndex])+#39;
         if (PageControlab.TabIndex>0) then PageControlab.TabIndex:=0;
         monta_table1;
         if (PageControlab.TabIndex = 1) then monta_table2(False,True);
         if PageControl1.TabIndex=1 then monta_table3;
      end;
...
procedure Tmdl_richdiscotxt.Timer_traduzTimer(Sender: TObject);
begin
      Inc(xtimerresponse);
end;

procedure Tmdl_richdiscotxt.Tradutor1AoTraduzir(Sender: TObject;
  Traducao: String);
begin
    xreceivetrad:=Traducao;
end;

function Tmdl_richdiscotxt.recebetraducao(xtrad_text,xtrad_de,xtrad_para: String):String;
var xrecebido: Bool;
begin
      xrecebido:=false;
      xreceivetrad:='';
...
procedure Tmdl_richdiscotxt.save_cfg(xcodlng: Integer);
var axcod : integer;
    axshowing: Boolean;
begin
     axshowing:= loadx.Showing;
...
procedure Tmdl_richdiscotxt.monta_table3;
var listaLng_id,axcoddefault,xid_default: Integer;
    axshowing: Boolean;
    //inclui linguagens
...
procedure Tmdl_richdiscotxt.atualiza_grade3;
var axshowing: Boolean;
    axlocid: Integer;
begin
     //banner
...
procedure Tmdl_richdiscotxt.limpaphoto3(xupdateset:Boolean);
begin
...
procedure Tmdl_richdiscotxt.bt_open3Click(Sender: TObject);
var xfilext,xnamefile: String;
    xerror: Boolean;
    codBitmap: TBitmap;
    codJpeg: TJPEGImage;
...
begin
      if OpenBannerDialog.Execute then begin
         ...
         end else if ((xfilext='.swf') or (xfilext='.flv') or (chk_imporiginal.Checked)) then begin
            try
               xbannertmp.LoadFromFile(OpenBannerDialog.FileName);
            except
               xerror:=true;
...
procedure Tmdl_richdiscotxt.bt_limpar3Click(Sender: TObject);
begin
      limpaphoto3(true);
end;

procedure Tmdl_richdiscotxt.bt_save3Click(Sender: TObject);
begin
      save_cfg(listaLng.cod_lng[PageControl_languagens.TabIndex]);
end;

procedure Tmdl_richdiscotxt.PageControl1Change(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.MenuItem1Click(Sender: TObject);
var axshowing: Boolean;
begin
...
procedure Tmdl_richdiscotxt.bt_del2Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
...
procedure Tmdl_richdiscotxt.ComboBox_entChange(Sender: TObject);
begin
      if StrtoInt(AxDadosUsr.axparm) > 5000 then begin
...
procedure Tmdl_richdiscotxt.bt_helpClick(Sender: TObject);
begin
      //Tópicos de ajuda
...
procedure Tmdl_richdiscotxt.StatusBar1Click(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tmdl_richdiscotxt.ToolBar_grpResize(Sender: TObject);
begin
     cb_group.Width := ToolBar_grp.Width;
end;

procedure Tmdl_richdiscotxt.Desfazer1Click(Sender: TObject);
begin
     //desfazer
...
procedure Tmdl_richdiscotxt.Recortar1Click(Sender: TObject);
begin
     //recortar
     retorna_TRxRichEdit(PageControlab.TabIndex).CutToClipboard;
end;

procedure Tmdl_richdiscotxt.Copiar1Click(Sender: TObject);
begin
     //copiar
     retorna_TRxRichEdit(PageControlab.TabIndex).CopyToClipboard;
end;

procedure Tmdl_richdiscotxt.Colar1Click(Sender: TObject);
begin
      //colar
     retorna_TRxRichEdit(PageControlab.TabIndex).PasteFromClipboard;
end;

procedure Tmdl_richdiscotxt.Cor1Click(Sender: TObject);
begin
     //cor da fonte
...
procedure Tmdl_richdiscotxt.Negrito1Click(Sender: TObject);
begin
     //negrito
     if FUpdating then Exit;
...
procedure Tmdl_richdiscotxt.Italico1Click(Sender: TObject);
begin
     //italico
     if FUpdating then Exit;
...
procedure Tmdl_richdiscotxt.Sublinhado1Click(Sender: TObject);
begin
     //sublinhado
     if FUpdating then Exit;
...
procedure Tmdl_richdiscotxt.Marcador1Click(Sender: TObject);
begin
     //marcador
     if FUpdating then Exit;
...
procedure Tmdl_richdiscotxt.AlignButtonClick(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.esquerdo1Click(Sender: TObject);
begin
     retorna_LeftAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_richdiscotxt.centro1Click(Sender: TObject);
begin
     retorna_CenterAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_richdiscotxt.direita1Click(Sender: TObject);
begin
     retorna_RightAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_richdiscotxt.justificado1Click(Sender: TObject);
begin
     retorna_JustifyAlign(PageControlab.TabIndex).Click;
end;

procedure Tmdl_richdiscotxt.re_editorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tmdl_richdiscotxt.SelectionChange(Sender: TObject);
begin
  //if processoand then exit;
  try
...
function Tmdl_richdiscotxt.CurrText: TRxTextAttributes;
begin
...
procedure Tmdl_richdiscotxt.SetEditRect;
var
  R: TRect;
begin
  with retorna_TRxRichEdit(PageControlab.TabIndex) do
...
procedure Tmdl_richdiscotxt.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
function Tmdl_richdiscotxt.retorna_TRxRichEdit(xidxcod:Integer):TRxRichEdit;
begin
end;

function Tmdl_richdiscotxt.retorna_fontName(xidxcod:Integer):TComboBox;
begin
end;

function Tmdl_richdiscotxt.retorna_fontSize(xidxcod:Integer):TEdit;
begin
end;

function Tmdl_richdiscotxt.retorna_BoldButton(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_ItalicButton(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_UnderlineButton(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_LeftAlign(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_CenterAlign(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_RightAlign(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_JustifyAlign(xidxcod:Integer):TToolButton;
begin
end;

function Tmdl_richdiscotxt.retorna_BulletsButton(xidxcod:Integer):TToolButton;
begin
end;

procedure Tmdl_richdiscotxt.FontNameChange(Sender: TObject);
begin
     if FUpdating then Exit;
     CurrText.Name := retorna_fontName(PageControlab.TabIndex).Items[retorna_fontName(PageControlab.TabIndex).ItemIndex];
end;

procedure Tmdl_richdiscotxt.FontSizeChange(Sender: TObject);
begin
     if FUpdating then Exit;
     CurrText.Size := StrToInt(retorna_fontSize(PageControlab.TabIndex).Text);
end;

procedure Tmdl_richdiscotxt.PageControlabChange(Sender: TObject);
...
begin
      if (PageControlab.TabIndex = 1) then begin
         ...
         With dm.ds_sql do begin
             if (recordcount <= 0) then begin
                hwsf.BtMensagem('Não há Itens cadastrados.');
...
function Tmdl_richdiscotxt.atualizafeed:Boolean;
begin
    WebBrowser_feed.Navigate('http://'+AxDadosUsr.website+'/feeddisc_upd.php');
end;

procedure Tmdl_richdiscotxt.cb_groupChange(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.EVsaveKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_richdiscotxt.EVsave2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      //if processoand then exit;
      if (Shift = [ssCtrl]) then
...
procedure Tmdl_richdiscotxt.Cordefundo1Click(Sender: TObject);
begin
      //cor de fundo
      if ColorDialog1.Execute then begin
...
function Tmdl_richdiscotxt.Btsearch_auto_ax(pathdefault,filtrofind:string):Tbt_search_ax;
...
procedure Tmdl_richdiscotxt.bt_exp1Click(Sender: TObject);
var xfiljpg : String;
...
procedure Tmdl_richdiscotxt.bt_exp2Click(Sender: TObject);
var xfiljpg : String;
    xImagem_tmp : TMemoryStream;
begin
      if ExportarImgDialog.Execute then begin
...
procedure Tmdl_richdiscotxt.SpeedButton2Click(Sender: TObject);
...
procedure Tmdl_richdiscotxt.bt_imgRotateh1Click(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.bt_imgRotatea1Click(Sender: TObject);
begin
     if dm.rotateFoto(-90,xlogotmp1,xlogosz1,xupdatelogo1,ComboBox_model.Text,LstTables.Table[1],'cod_gal','icon',ab_cod.Caption) then
        abretmplogo1;
end;

procedure Tmdl_richdiscotxt.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
...
procedure Tmdl_richdiscotxt.resetWebBrowse;
...
procedure Tmdl_richdiscotxt.SetRepetirImagem(xreptid:Integer);
var axreptx: String;
begin
    Case xreptid of
        0: axreptx:='no-repeat';
...
procedure Tmdl_richdiscotxt.setxHtml(xabaED:Integer);
...
procedure Tmdl_richdiscotxt.InternalSaveDocumentToStream(
...
procedure Tmdl_richdiscotxt.InternalSaveBodyHTMLToStream(const Stream: TStream);
var
  HTMLStr: string;
  Doc: IHTMLDocument2;
...
procedure Tmdl_richdiscotxt.InternalSaveBodyHTMLToStringList(const StringList: TStringList);
var
  HTMLStr: string;
  Doc: IHTMLDocument2;
  BodyElement: IHTMLElement;
...
procedure Tmdl_richdiscotxt.InternalSaveBodyTextToStream(const Stream: TStream);
var
  TextStr: string;
  Doc: IHTMLDocument2;
  BodyElement: IHTMLElement;
...
procedure Tmdl_richdiscotxt.InternalLoadDocumentFromStream(const Stream: TStream);
var
  PersistStreamInit: IPersistStreamInit;
  StreamAdapter: IStream;
begin
...
procedure Tmdl_richdiscotxt.SpeedButton10Click(Sender: TObject);
begin
    resetWebBrowse;
end;

procedure Tmdl_richdiscotxt.SpeedButton9Click(Sender: TObject);
begin
      bt_save1.Click;
end;

procedure Tmdl_richdiscotxt.SpeedButton11Click(Sender: TObject);
begin
    InvokeCMD(WebBrowser1, HTMLID_FIND);
end;

procedure Tmdl_richdiscotxt.SpeedButton8Click(Sender: TObject);
var
...
procedure Tmdl_richdiscotxt.NoRepetir1Click(Sender: TObject);
begin
    if (not NoRepetir1.Checked) then
        SetRepetirImagem(0);
end;

procedure Tmdl_richdiscotxt.repetirimagem1Click(Sender: TObject);
begin
    if (not repetirimagem1.Checked) then
        SetRepetirImagem(1);
end;

procedure Tmdl_richdiscotxt.RepetirX1Click(Sender: TObject);
begin
    if (not RepetirX1.Checked) then
        SetRepetirImagem(2);
end;

procedure Tmdl_richdiscotxt.RepetirY1Click(Sender: TObject);
begin
    if (not RepetirY1.Checked) then
        SetRepetirImagem(3);
end;

procedure Tmdl_richdiscotxt.Fixarfundo1Click(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.Limpar1Click(Sender: TObject);
begin
       HTMLDocumento := WebBrowser1.Document as IHTMLDocument2;
...
procedure Tmdl_richdiscotxt.combofontChange(Sender: TObject);
begin
    HTMLDocumento := WebBrowser1.Document as IHTMLDocument2;
    HTMLDocumento.execCommand('FontName', False,ComboFont.Text);
end;

procedure Tmdl_richdiscotxt.ComboSizeChange(Sender: TObject);
begin
...
procedure Tmdl_richdiscotxt.SpeedButton1Click(Sender: TObject);
begin
    HTMLDocumento := WebBrowser1.Document as IHTMLDocument2;
    HTMLDocumento.execCommand('Bold', False,0);
end;

procedure Tmdl_richdiscotxt.SpeedButton3Click(Sender: TObject);
begin
    HTMLDocumento := WebBrowser1.Document as IHTMLDocument2;
    HTMLDocumento.execCommand('Italic', False,0);
end;

procedure Tmdl_richdiscotxt.SpeedButton5Click(Sender: TObject);
begin
    HTMLDocumento := WebBrowser1.Document as IHTMLDocument2;
    HTMLDocumento.execCommand('Underline', False,0);
end;

procedure Tmdl_richdiscotxt.SpeedButton6Click(Sender: TObject);
begin
    HTMLDocumento := WebBrowser1.Document as IHTMLDocument2;
    if cdColor.Execute then
...
procedure Tmdl_richdiscotxt.ComboSpeedButton1Click(Sender: TObject);
...
procedure Tmdl_richdiscotxt.WebBrowser_feedDownloadComplete(Sender: TObject);
begin
      feedokupd:=true;
end;

procedure Tmdl_richdiscotxt.bt_del1Click(Sender: TObject);
var axshowing: Boolean;
    axcod: Integer;
begin
     //banco de imagens
...
procedure Tmdl_richdiscotxt.linkmp3Click(Sender: TObject);
begin
    linkmp3.Checked:=true;
end;

function Tmdl_richdiscotxt.executalistxml(OpenDialog_XML_Path:String):Boolean;
var txmemo,txListNumT,txListTracks: TStringList;
    txstr,txstr1,txstr2: String;
    txid: integer;
    xfor_idimg,axgal_id,axcod: Integer;
    axResult:Boolean;
begin
   axResult:=FileExists(OpenDialog_XML_Path);
   if axResult then begin
     try
      try
       txmemo:= TStringList.Create;
       txListNumT:= TStringList.Create;
       txListTracks:= TStringList.Create;
       txmemo.Clear;
       txListNumT.Clear;
       txListTracks.Clear;
       txmemo.LoadFromFile(OpenDialog_XML_Path);
       for txid:=0 to txmemo.Count-1 do begin
           txstr:=txmemo.Strings[txid];
           txstr1:=copy(txstr,1,2);
           txListNumT.Add(txstr1);
           txstr2:=copy(txstr,4,Length(txstr)-3);
           if (copy(txstr2,1,1)=' ') then txstr2:=copy(txstr2,2,Length(txstr2));
           if CheckBoxMAIUSCULAS.Checked then txstr2:=UpperCase(txstr2);
           txListTracks.Add(txstr2);
       end;
       axResult:=true;
      except
      end;
   ...
   if axResult then begin
     try
      try
       for xfor_idimg:=0 to txListNumT.Count-1 do begin
           axcod_img:=0;
           txstr1:=txListNumT.Strings[xfor_idimg];
           txstr2:=txListTracks.Strings[xfor_idimg];
           if (txstr1<>'') then begin
                for axgal_id:=0 to listaLng.count-1 do begin
                    axcod:=dm.GetCodMax(LstTables.Table[2],'codigo','');
                    if axcod_img=0 then axcod_img:=axcod;
                    with dm.executa do begin
                         ...
                         Params[2].AsInteger:=axcod_gal;
                         Params[3].AsInteger:=listaLng.cod_lng[axgal_id];
                         Params[4].AsString:=txstr2;
                         Params[5].AsString:='';
                         Params[6].AsString:='track'+txstr1+'.mp3';
                         Params[7].AsString :='';
                         Params[8].AsString :='';
                         Params[9].AsString :='';
                         try
...
procedure Tmdl_richdiscotxt.SpeedButton4Click(Sender: TObject);
var AX_OpenDialog_XML_Path:String;
begin
  if OpenDialog_XML.Execute then begin
   AX_OpenDialog_XML_Path:=OpenDialog_XML.FileName;
   executalistxml(AX_OpenDialog_XML_Path);
  end;
end;

procedure Tmdl_richdiscotxt.SpeedButton16Click(Sender: TObject);
var xfilespath,xfiljpg : String;
    xImagem_tmp : TMemoryStream;
    xidcount,xtotimg: Integer;
    axshowing: Boolean;
begin
      ...
      if xfilespath = '' then begin
         ...
         if not axshowing then begin
            loadx := Tloadx.Create(Application);
            loadx.lbloading.Caption:='Exportando imagens [1'+'/'+inttostr(xtotimg)+']';
            loadx.Show;
            loadx.Update;
         end else begin
            loadx.lbloading.Caption:='Exportando imagens [1'+'/'+inttostr(xtotimg)+']';
...
{procedure Tmdl_richdisco.Button1Click(Sender: TObject);
var axshowing: Boolean;
    insertLine,LineData,LineDataTime,LineDataTimeZero:String;
    codprodid,codprodid_Pai,coddownid,cdid:Integer;
    strListPrd:TStringList;
    mp3Artist,mp3Album,mp3Title,mp3Track,mp3Mask:String;
    mp3IdTrack: Integer;
    albMetaDesc, albMetaKey, mp3MetaDesc, mp3MetaKey: String;
    function limpastring(xstrlimpa:String; xstrcl:String):String;
    var AxstrLimpa:String;
    begin
        AxstrLimpa:=xstrlimpa;
        AxstrLimpa:=StringReplace(AxstrLimpa, '\', xstrcl, [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '/', xstrcl, [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, ':', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '*', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '?', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '<', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '>', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '|', xstrcl, [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, #39, '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '"', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '"', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, ',', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, '.', '', [rfReplaceAll]);
        AxstrLimpa:=StringReplace(AxstrLimpa, ';', '', [rfReplaceAll]);
        if (xstrcl = '') then begin
           AxstrLimpa:=StringReplace(AxstrLimpa, '-', '', [rfReplaceAll]);
           AxstrLimpa:=StringReplace(AxstrLimpa, '(', '', [rfReplaceAll]);
           AxstrLimpa:=StringReplace(AxstrLimpa, ')', '', [rfReplaceAll]);
        end;
    ...
    function limitstring(xstrlimit:String; xstrcount:Integer):String;
    var Axstrlimit:String;
    begin
        Axstrlimit:=xstrlimit;
        if (Length(xstrlimit)>xstrcount) then begin
            Axstrlimit:=copy(Axstrlimit,0,xstrcount);
        end;
        Result:=Axstrlimit;
    end;
begin
     //banco de imagens
      ...
      cdid:=strtoint(LabeledEdit1.text);
      codprodid:=strtoint(LabeledEdit3.text);//483; //455 - 482
      codprodid_Pai:=strtoint(LabeledEdit2.text);//455;
      LineData:='2012-10-01';
      LineDataTime:='2012-10-01 00:00:00';
      LineDataTimeZero:='0000-00-00 00:00:00';
      coddownid:=strtoint(LabeledEdit4.text);//420;
      strListPrd:=TStringList.Create;
      strListPrd.Clear;
      with dm.ds_sql3 do begin
          First;
          mp3Album:=dm.ds_sql.fieldbyname('descricao').AsString;
          mp3Artist:='Os Monarcas '+mp3Album;//copy(mp3Album,1,4)
          //tags
          albMetaDesc:=limpastring(mp3Artist,'');
          albMetaKey:=limpastring(mp3Album,'');
          albMetaKey:='Os Monarcas, Música, Gaúcha, Tradicionalismo, '+albMetaKey;
          albMetaDesc:=limitstring(albMetaDesc,255);
          albMetaKey:=limitstring(albMetaKey,255);
          //*****
          mp3IdTrack:=1;
          While not eof do begin
            loadx.Progressloading.position:=60;
            mp3Track:=formatfloat('00',mp3IdTrack);
            mp3Title:=fieldbyname('descricao').AsString;
            mp3Mask:=mp3Artist+' - '+mp3Track+' '+mp3Title;
            mp3Mask:=limpastring(mp3Mask,'-');
            //tags
            mp3MetaDesc:=limpastring(mp3Mask,'');
            mp3MetaKey:=limpastring(mp3Title,'');
            mp3MetaKey:=albMetaKey+', '+mp3MetaKey;
            mp3MetaDesc:=limitstring(mp3MetaDesc,255);
            mp3MetaKey:=limitstring(mp3MetaKey,255);
            //*****
            mp3Title:=StringReplace(mp3Title, #39, '\'+#39, [rfReplaceAll]);
            mp3Title:=StringReplace(mp3Title, '  ', ' ', [rfReplaceAll]);
            //*****
            loadx.Progressloading.position:=70;
            insertLine:= 'INSERT INTO product (product_id,model,sku,upc,location,quantity,stock_status_id,image,manufacturer_id,shipping,price,points,tax_class_id,date_available,weight,weight_class_id,length,width,height,'+
                         'length_class_id,subtract,minimum,sort_order,status,date_added,date_modified,viewed,mp3demofile) '+
                         'VALUES ('+IntToStr(codprodid)+','+#39+'MIDIA'+#39+','+#39+''+#39+','+#39+''+#39+','+#39+''+#39+',1,7,'+#39+'data/capas/cd'+formatfloat('00',cdid)+'.jpg'+#39+',0,0,'+#39+'1.49'+#39+',10,0,'+#39+LineData+#39+',0,1,0,0,0,4,0,1,1,1,'+#39+LineDataTime+#39+','+#39+LineDataTimeZero+#39+',0,'+#39+'cd'+formatfloat('00',cdid)+'/track'+mp3Track+'.mp3'+#39+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_description (product_id,language_id,name,description,meta_description,meta_keyword) '+
                         'VALUES ('+IntToStr(codprodid)+',2,'+#39+mp3Track+'. '+mp3Title+#39+','+#39+''+#39+','+#39+mp3MetaDesc+#39+','+#39+mp3MetaKey+#39+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_mp3 (product_id_pai,product_id_filho) '+
                         'VALUES ('+IntToStr(codprodid_Pai)+','+IntToStr(codprodid)+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_to_category (product_id,category_id) '+
                         'VALUES ('+IntToStr(codprodid)+',61);';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_to_store (product_id,store_id) '+
                         'VALUES ('+IntToStr(codprodid)+',0);';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_reward (product_id,customer_group_id,points) '+
                         'VALUES ('+IntToStr(codprodid)+',8,0);';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO download (download_id,filename,mask,remaining,date_added) '+
                         'VALUES ('+IntToStr(coddownid)+','+#39+'cd'+formatfloat('00',cdid)+'/track'+mp3Track+'.mp3'+#39+','+#39+mp3Mask+'.mp3'+#39+',1,'+#39+LineDataTime+#39+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO download_description (download_id,language_id,name) '+
                         'VALUES ('+IntToStr(coddownid)+',2,'+#39+mp3Track+'. '+mp3Title+#39+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_to_download (product_id,download_id) '+
                         'VALUES ('+IntToStr(codprodid)+','+IntToStr(coddownid)+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
            insertLine:= 'INSERT INTO product_to_download (product_id,download_id) '+
                         'VALUES ('+IntToStr(codprodid_Pai)+','+IntToStr(coddownid)+');';
            strListPrd.Add(insertLine);
            strListPrd.Add('');
      ...
      insertLine:= 'UPDATE product SET model='+QuotedStr('Álbum Completo MP3')+',price='+QuotedStr(IntToStr(mp3IdTrack-1))+',points='+QuotedStr(IntToStr((mp3IdTrack-1)*10))+' WHERE product_id='+QuotedStr(IntToStr(codprodid_Pai))+';';
      strListPrd.Add(insertLine);
      strListPrd.Add('');
      insertLine:= 'UPDATE product_description set meta_description='+QuotedStr(albMetaDesc)+',meta_keyword='+QuotedStr(albMetaKey)+' WHERE product_id='+QuotedStr(IntToStr(codprodid_Pai))+';';
      strListPrd.Add(insertLine);
      strListPrd.Add('');
      strListPrdAll.AddStrings(strListPrd);
      //strListPrd.SaveToFile('C:\MONARCAS\cd'+formatfloat('00',cdid)+'.txt');
      try loadx.Close; except end;
      //ShowMessage('Concluido');
      Inc(cdid);
      Inc(codprodid_Pai);
      LabeledEdit1.text:=IntToStr(cdid);
      LabeledEdit2.text:=IntToStr(codprodid_Pai);
      LabeledEdit3.text:=IntToStr(codprodid);
      LabeledEdit4.text:=IntToStr(coddownid);
      Sleep(1000);
      strListPrd.Free;
      if (cdid < cb_group.Items.Count) then begin
          if (cdid = 31) then begin
             cb_group.ItemIndex:=cb_group.ItemIndex+2;
             Inc(codprodid_Pai);
             Inc(codprodid);
             Inc(coddownid);
             LabeledEdit2.text:=IntToStr(codprodid_Pai);
             LabeledEdit3.text:=IntToStr(codprodid);
             LabeledEdit4.text:=IntToStr(coddownid);
          end else cb_group.ItemIndex:=cb_group.ItemIndex+1;
          if dm.ds_sql.Locate('descricao',cb_group.Text,[]) then begin
             atualiza_grade;
             monta_table2(False,True);
             Button1.Click;
          end;
      end else strListPrdAll.SaveToFile('C:\MONARCAS\cdAll.sql');
end; }

{

function Tmdl_richdisco.executalistxml:Boolean;
var txmemo: TStringList;
    txstr,txstr1,txstr2: String;
    txid: integer;
begin
   if FileExists(OpenDialog_XML_Path) then begin
   try
   txmemo:= TStringList.Create;
   txmemo.LoadFromFile(OpenDialog_XML_Path);
   Memo1.Clear;
   Memo2.Clear;
   for txid:=2 to txmemo.Count-2 do begin
       txstr:=txmemo.Strings[txid];
       txstr1:=copy(txstr,26,11);
       Memo1.Lines.Add(txstr1);
       txstr2:=copy(txstr,52,Length(txstr)-54);
       if (copy(txstr2,1,1)=' ') then txstr2:=copy(txstr2,2,Length(txstr2));
       Memo2.Lines.Add(txstr2);
   end;
   finally
      txmemo.free;
   end;
...
procedure Tmdl_richdisco.SpeedButton4Click(Sender: TObject);
begin
  Memo1.Clear;
  if OpenDialog_XML.Execute then begin
   OpenDialog_XML_Path:=OpenDialog_XML.FileName;
   executalistxml;
  end;
end;

procedure Tmdl_richdisco.SpeedButton16Click(Sender: TObject);
var xfor_idimg,axgal_id,axcod: Integer;
    txstr1,txstr2: String;
begin
       for xfor_idimg:=0 to Memo1.Lines.Count-1 do begin
           axcod_img:=0;
           txstr1:=Memo2.Lines.Strings[xfor_idimg];
           txstr2:=Memo1.Lines.Strings[xfor_idimg];
           if (txstr1<>'') then begin
                for axgal_id:=0 to listaLng.count-1 do begin
                    axcod:=dm.GetCodMax(LstTables.Table[2],'codigo','');
                    if axcod_img=0 then axcod_img:=axcod;
                    with dm.executa do begin
                         ...
                         Params[2].AsInteger:=axcod_gal;
                         Params[3].AsInteger:=listaLng.cod_lng[axgal_id];
                         Params[4].AsString:=txstr1;
                         Params[5].AsString:='';
                         Params[6].AsString:=txstr2;
                         Params[7].AsString :='';
                         Params[8].AsString :='';
                         Params[9].AsString :='';
                         try
...
procedure Tmdl_richdisco.SpeedButton17Click(Sender: TObject);
var X_OpenDialog_XML_Path,X_OpenDialog_XML_Pathx: String;
begin
     if (cb_group.ItemIndex = cb_group.Items.Count-1) then begin
         SpeedButton17.Enabled:=false;
         exit;
     end else begin
         cb_group.ItemIndex:=cb_group.ItemIndex+1;
         if dm.ds_sql.Locate('descricao',cb_group.Text,[]) then begin
             atualiza_grade;
             monta_table2(False,True);
         end;
     end;
     X_OpenDialog_XML_Path:= copy(OpenDialog_XML_Path,1,Length(OpenDialog_XML_Path)-11)+
                              FormatFloat('00',(strtofloat(copy(OpenDialog_XML_Path,Length(OpenDialog_XML_Path)-10,2))+1))+
                              '\sons.xml';
     if FileExists(X_OpenDialog_XML_Path) then begin
        OpenDialog_XML_Path:=X_OpenDialog_XML_Path;
        executalistxml;
     end;
end;
}
