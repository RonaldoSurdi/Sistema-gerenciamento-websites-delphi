...
uses
  ...
  uadm_usrwebsites in 'uadm_usrwebsites.pas' {adm_usrwebsites};
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol usuários';
    loadx.Show;
    loadx.Update;
    ...
    HprevHist := FindWindow(Nil, PChar('HWScontrol usuários'));
    if HprevHist = 0 then begin
    Application.Title := 'HWScontrol usuários';
  ...
  dm.winsysreg(1,false,false);
          if dm.conecta_bd(ParamStr(1),ParamStr(3),'') then begin
             Application.CreateForm(Tadm_usrwebsites, adm_usrwebsites);
          end else begin
