...
procedure Tform_restrict.MountedGrid;
begin
...
procedure Tform_restrict.atualizaReg;
...
begin
     ...
     axshowing:= loadx.Showing;
      ...
      Case axtypPerm of
         ...
         3: begin
            xnameTitle:=dm2.ds_sql6.fieldbyname('cp1').AsString;
...
procedure Tform_restrict.SpeedButton1Click(Sender: TObject);
var axcodIT: Integer;
    axshowing: Boolean;
begin
     Case axtypPerm of
...
procedure Tform_restrict.SpeedButton7Click(Sender: TObject);
begin
     Case axtypPerm of
          1: begin
             if not dm2.ds_sql5.Active then exit;
             ...
             if hwsf.BtConfirma('Tem certeza que desejas remover as Permissões para o Usuário '+dm2.ds_sql5.fieldbyname('cp3').AsString+'?') then begin
                if dm2.deleta_reg(mdl_filemanager.LstTables.Table[7],'codigo',dm2.ds_sql5.fieldbyname('codigo').AsString,false,false) then begin
                   hwsf.BtMensagem('Permissões removidas');
                   Atualiza_PTR;
                end;
             end;
          end;
          2: begin
             ...
             if hwsf.BtConfirma('Tem certeza que desejas remover as Permissões para o Usuário '+dm2.ds_sql10.fieldbyname('cp3').AsString+'?') then begin
                if dm2.deleta_reg(mdl_filemanager.LstTables.Table[8],'codigo',dm2.ds_sql10.fieldbyname('codigo').AsString,false,false) then begin
                   hwsf.BtMensagem('Permissões removidas');
                   Atualiza_PTR;
                end;
             end;
          ...
          3: begin
             ...
             if hwsf.BtConfirma('Tem certeza que desejas remover as Permissões para o Usuário '+dm2.ds_sql9.fieldbyname('cp3').AsString+'?') then begin
                if dm2.deleta_reg(mdl_filemanager.LstTables.Table[7]+'_ax','codigo',dm2.ds_sql9.fieldbyname('codigo').AsString,false,false) then begin
                   hwsf.BtMensagem('Permissões removidas');
                   Atualiza_PTR;
                end;
             end;
...
procedure Tform_restrict.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
      Action:=caHide;
end;

procedure Tform_restrict.FormShow(Sender: TObject);
begin
      if mdl_filemanager.axformInit then atualizaReg;
end;

procedure Tform_restrict.FormCreate(Sender: TObject);
begin
      axMountedGrid:=false;
end;
