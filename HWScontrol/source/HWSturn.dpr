...
uses
  ...
  umdl_turn in 'umdl_turn.pas' {mdl_turn},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol Turnês';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol Turnês';
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_turn, mdl_turn);
                if (ParamStr(5) <> '') then
                    mdl_turn.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_turn.xiniCodab:= 0;
             end else begin
