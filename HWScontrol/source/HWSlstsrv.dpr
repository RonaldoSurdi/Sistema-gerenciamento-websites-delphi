...
uses
  ...
  umdl_lstsrv in 'umdl_lstsrv.pas' {mdl_lstsrv},
...
begin
    ...
    loadx.lbloading.Caption:='HWScontrol ListServices';
    loadx.Show;
    loadx.Update;
    ...
    Application.Title := 'HWScontrol ListServices';
  ...
  dm.winsysreg(1,false,false);
          if (dm.conecta_bd(ParamStr(1),ParamStr(3),ParamStr(4))) then begin
             ...
             if HprevHist = 0 then begin
                Application.CreateForm(Tmdl_lstsrv, mdl_lstsrv);
                if (ParamStr(5) <> '') then
                    mdl_lstsrv.xiniCodab:= StrToInt(ParamStr(5))
                else mdl_lstsrv.xiniCodab:= 0;
             end else begin
