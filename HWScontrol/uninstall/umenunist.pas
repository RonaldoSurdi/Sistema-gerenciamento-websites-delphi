...
begin
      ...
      Deleteicones('HWScontrol',1);
      Deleteicones('HWScontrol',2);
...
begin
  ...
  for i := 1 to strs.Count-1 do begin
    ...
    if (copy(LowerCase(axMdlclose),1,10) = 'hwscontrol') then begin
...
begin
      If Application.MessageBox('Desinstalar HWScontrol?','Confirmação',MB_ICONQUESTION+ MB_YESNO+MB_DEFBUTTON2)=idyes then begin
         if not closeAllForms() then begin
            ShowMessage('É necessário finalizar o sistema HWScontrol!!!');
            exit;
         end;
         inexecut:=true;
