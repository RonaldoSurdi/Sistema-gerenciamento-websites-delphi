...
uses
  ...
  uadm_vlrModaux in 'uadm_vlrModaux.pas' {adm_vlrModaux};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS preços mód. Aux'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS preços mód. Aux';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_vlrModaux, adm_vlrModaux);
          end else begin
