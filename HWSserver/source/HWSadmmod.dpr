...
uses
  ...
  uadm_admmod in 'uadm_admmod.pas' {adm_admmod};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Módulos Administração'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Módulos Administração';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_admmod, adm_admmod);
          end else begin
