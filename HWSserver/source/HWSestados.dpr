...
uses
  ...
  uadm_estados in 'uadm_estados.pas' {adm_estados};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Estados'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Estados';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_estados, adm_estados);
          end else begin
