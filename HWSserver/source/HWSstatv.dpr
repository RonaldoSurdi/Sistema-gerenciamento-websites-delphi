...
uses
  ...
  uadm_hdatividades in 'uadm_hdatividades.pas' {adm_hdatividades};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS ST - Atividades'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS ST - Atividades';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_hdatividades, adm_hdatividades);
          end else begin
