...
uses
  ...
  uadm_vlrModadm in 'uadm_vlrModadm.pas' {adm_vlrModadm};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS preços mód. Adm'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS preços mód. Adm';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_vlrModadm, adm_vlrModadm);
          end else begin
