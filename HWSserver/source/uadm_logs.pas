...
begin
     ...
     DBGrid1.Columns.Add;
     DBGrid1.Columns.Items[2].FieldName:='decode(usuario,'+#39+cv2+#39+')';
     ...
     With dm.ds_int3 do begin
        CommandText:= 'select stat,codigo,decode(usuario,'+#39+cv2+#39+'),decode(password,'+#39+cv2+#39+'),dt_cad,cod_ent from '+EnDecryptString(Serverconect.vrf19[1],37249);
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
begin
     ...
     With dm.ds_int do begin
        ...
        CommandText:= 'SELECT codigo,data_time,decode(ip,'+#39+'3542'+#39+'),obs,cod_usr from '+EnDecryptString(Serverconect.vrf22[1],37249)+
        xtxtfilter+
        ' ORDER BY data_time';
        Open;
        Filter:='';
...
begin
    ...
    Try
      With dm.executa_int do begin
        if (text_filtro<>'') AND (filtra_usr<>'') then CommandText := 'DELETE FROM '+EnDecryptString(Serverconect.vrf22[1],37249)+' WHERE '+text_filtro+' AND '+filtra_usr
        else if (text_filtro<>'') then CommandText := 'DELETE FROM '+EnDecryptString(Serverconect.vrf22[1],37249)+' WHERE '+text_filtro
        else if (filtra_usr<>'') then CommandText := 'DELETE FROM '+EnDecryptString(Serverconect.vrf22[1],37249)+' WHERE '+filtra_usr
        else CommandText := 'DELETE FROM '+EnDecryptString(Serverconect.vrf22[1],37249);
        Execute;
      end;
    Except
