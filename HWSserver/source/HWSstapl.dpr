...
uses
  ...
  uadm_hdapli in 'uadm_hdapli.pas' {adm_hdapli};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS ST - Aplicativos'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS ST - Aplicativos';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_hdapli, adm_hdapli);
          end else begin
