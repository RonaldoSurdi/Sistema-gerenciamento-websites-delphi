...
type
  ...
  End;
  Tadm_admcustomhelp = class(TForm)
    Panel1: TPanel;
    pn_tit: TPanel;
    Image3: TImage;
...
var
  adm_admcustomhelp: Tadm_admcustomhelp;
...
procedure Tadm_admcustomhelp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     xmn_mdl_ext.Free;
     dm.ds_int.Filter:='';
...
procedure Tadm_admcustomhelp.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_admcustomhelp.FormCreate(Sender: TObject);
...
begin
     ...
     With dm.ds_int do begin
        CommandText:='SELECT codigo,descricao,sz,tp,wh,hg FROM '+EnDecryptString(HelpSys.vrf03[1],37249)+' ORDER BY descricao';
        Open;
        Filter:='';
        Filtered:=true;
     end;
     With dm.ds_int2 do begin
        CommandText:='SELECT codigo,descricao,tp FROM '+EnDecryptString(HelpSys.vrf03[1],37249)+' ORDER BY descricao';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
procedure Tadm_admcustomhelp.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_admcustomhelp.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_admcustomhelp.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_admcustomhelp.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_admcustomhelp.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_admcustomhelp.Left:= adm_admcustomhelp.Left-(mv_mouse.x-x);
      adm_admcustomhelp.Top:= adm_admcustomhelp.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_admcustomhelp.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_admcustomhelp.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_admcustomhelp.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_admcustomhelp.bt_maximizaClick(Sender: TObject);
begin
    if adm_admcustomhelp.WindowState=wsMaximized then adm_admcustomhelp.WindowState:=wsNormal
    else adm_admcustomhelp.WindowState:=wsMaximized;
end;

procedure Tadm_admcustomhelp.FormResize(Sender: TObject);
begin
...
procedure Tadm_admcustomhelp.barraMoved(Sender: TObject);
begin
      bt_barra.Left:=barra.Left;
      bt_menu.Left:=StatusBar.Left+1;
      Panel_ocultared.Width:= TabSheet_editor.Width;
...
procedure Tadm_admcustomhelp.bt_barraClick(Sender: TObject);
begin
...
procedure Tadm_admcustomhelp.Novo1Click(Sender: TObject);
begin
...
procedure Tadm_admcustomhelp.bt_menuClick(Sender: TObject);
begin
      PopupMenu_editor.Popup(adm_admcustomhelp.Left+GroupBox_menu.Width+20,
                             adm_admcustomhelp.Top+adm_admcustomhelp.Height-40);
end;

procedure Tadm_admcustomhelp.SetModified(Value: Boolean);
begin
...
procedure Tadm_admcustomhelp.CheckFileSave;
begin
...
procedure Tadm_admcustomhelp.SelectionChange(Sender: TObject);
begin
...
function Tadm_admcustomhelp.CurrText: TRxTextAttributes;
begin
...
procedure Tadm_admcustomhelp.GetFontNames;
var
  DC: HDC;
begin
  DC := GetDC(0);
...
procedure Tadm_admcustomhelp.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
procedure Tadm_admcustomhelp.SetupRuler;
var
  I: Integer;
  S: String;
begin
...
procedure Tadm_admcustomhelp.SetEditRect;
var
  R: TRect;
begin
  if xupxmenumn then exit;
...
procedure Tadm_admcustomhelp.PerformFileOpen(const AFileName: string);
begin
...
procedure Tadm_admcustomhelp.Abrir1Click(Sender: TObject);
begin
  CheckFileSave;
...
procedure Tadm_admcustomhelp.Imprimir1Click(Sender: TObject);
begin
  if PrintDialog.Execute then
     retorna_TRxRichEdit(PageControl_editor.TabIndex).Print('HWS editor help');
end;

procedure Tadm_admcustomhelp.Salvar1Click(Sender: TObject);
...
begin
    ...
    //for xind_trad:=0 to menutmp.count-1 do begin
        ...
        with dm.executa_int do begin
             CommandText:= 'UPDATE '+EnDecryptString(HelpSys.vrf02[1],37249)+' SET tx=:p0,img=:p1 '+
                        'WHERE codigo=:p2';
...
procedure Tadm_admcustomhelp.Copiar1Click(Sender: TObject);
begin
      retorna_TRxRichEdit(PageControl_editor.TabIndex).CopyToClipboard;
end;

procedure Tadm_admcustomhelp.Recortar1Click(Sender: TObject);
begin
      retorna_TRxRichEdit(PageControl_editor.TabIndex).CutToClipboard;
end;

procedure Tadm_admcustomhelp.Colar1Click(Sender: TObject);
begin
      retorna_TRxRichEdit(PageControl_editor.TabIndex).PasteFromClipboard;
end;

procedure Tadm_admcustomhelp.Desfazer1Click(Sender: TObject);
begin
      with retorna_TRxRichEdit(PageControl_editor.TabIndex) do
      if HandleAllocated then SendMessage(Handle, EM_UNDO, 0, 0);
end;

procedure Tadm_admcustomhelp.FontNameChange(Sender: TObject);
begin
      if FUpdating then Exit;
      CurrText.Name := FontName.Items[FontName.ItemIndex];
end;

procedure Tadm_admcustomhelp.FontSizeChange(Sender: TObject);
begin
      if FUpdating then Exit;
      CurrText.Size := StrToInt(FontSize.Text);
end;

procedure Tadm_admcustomhelp.Negrito1Click(Sender: TObject);
begin
      if FUpdating then Exit;
...
procedure Tadm_admcustomhelp.Italico1Click(Sender: TObject);
begin
      if FUpdating then Exit;
...
procedure Tadm_admcustomhelp.Sublinhado1Click(Sender: TObject);
begin
      if FUpdating then Exit;
...
procedure Tadm_admcustomhelp.Cor1Click(Sender: TObject);
begin
      if FUpdating then Exit;
...
procedure Tadm_admcustomhelp.RulerItemMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure Tadm_admcustomhelp.RulerItemMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if FDragging then
    TLabel(Sender).Left :=  TLabel(Sender).Left+X-FDragOfs;
...
procedure Tadm_admcustomhelp.FirstIndMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure Tadm_admcustomhelp.LeftIndMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure Tadm_admcustomhelp.RightIndMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure Tadm_admcustomhelp.re_editorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
...
procedure Tadm_admcustomhelp.FormPaint(Sender: TObject);
begin
      SetEditRect;
end;

procedure Tadm_admcustomhelp.FormShow(Sender: TObject);
begin
      adm_admcustomhelp.Refresh;
      UpdateCursorPos;
      DragAcceptFiles(Handle, True);
      RichEditChange(nil);
end;

procedure Tadm_admcustomhelp.RichEditChange(Sender: TObject);
begin
      if xupxmenumn then exit;
      SetModified(retorna_TRxRichEdit(PageControl_editor.TabIndex).Modified);
end;

procedure Tadm_admcustomhelp.re_ReplaceDialogFind(Sender: TObject);
var FoundAt: LongInt;
...
procedure Tadm_admcustomhelp.re_ReplaceDialogReplace(Sender: TObject);
var SelPos: Integer;
begin
...
procedure Tadm_admcustomhelp.Localizar1Click(Sender: TObject);
begin
      //localizar e substituir
      re_ReplaceDialog.Execute;
end;

procedure Tadm_admcustomhelp.Arquivotexto1Click(Sender: TObject);
begin
...
procedure Tadm_admcustomhelp.Exportar1Click(Sender: TObject);
...
procedure Tadm_admcustomhelp.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_admcustomhelp.filtrar_reg;
begin
     if Edit_search.Text='' then dm.ds_int.Filter:=''
     else dm.ds_int.filter:= 'descricao LIKE '+QuotedStr(Edit_search.Text+'%');
end;

procedure Tadm_admcustomhelp.atualiza_grade;
begin
...
procedure Tadm_admcustomhelp.addreg;
begin
     lbcp1.Text:='novo';
     lbcp2.Text:= '';
     lbcp3.Text:= 'nenhum';
...
procedure Tadm_admcustomhelp.bt_enable(xbtenab:Boolean);
begin
     bt_limpar.Enabled:=xbtenab;
     bt_visualizar.Enabled:=xbtenab;
end;

procedure Tadm_admcustomhelp.destructionimg;
...
procedure Tadm_admcustomhelp.getLogo;
begin
     loadx := Tloadx.Create(Application);
...
function Tadm_admcustomhelp.retorna_ImgStream(xcodref: Integer; aximgstr: TMemoryStream):TMemoryStream;
begin
     With dm.executa_int do begin
...
function Tadm_admcustomhelp.retorna_ImgType(xcodref: Integer):Integer;
begin
     With dm.executa_int do begin
        CommandText:='SELECT tp FROM '+EnDecryptString(HelpSys.vrf03[1],37249)+' WHERE codigo='+#39+InttoStr(xcodref)+#39;
        Open;
...
procedure Tadm_admcustomhelp.abretmplogo;
var erro: Boolean;
    ax_filenm: TFileName;
    isshowing: Boolean;
begin
...
procedure Tadm_admcustomhelp.ProgresTimerTimer(Sender: TObject);
begin
  if pn_mp3player.Visible then begin
...
procedure Tadm_admcustomhelp.bt_playmp3Click(Sender: TObject);
begin
      mp3player.Play;
end;

procedure Tadm_admcustomhelp.bt_stopmp3Click(Sender: TObject);
begin
      mp3player.Stop;
end;

procedure Tadm_admcustomhelp.bt_closemp3Click(Sender: TObject);
var ax_filenm: string;
begin
      mp3player.Close;
      pn_mp3player.Visible:=false;
...
procedure Tadm_admcustomhelp.bt_limparClick(Sender: TObject);
begin
      xmodupdate:=true;
      destructionimg;
      xlogotmp.Clear;
...
procedure Tadm_admcustomhelp.bt_localizarClick(Sender: TObject);
var ax_ext: string;
...
procedure Tadm_admcustomhelp.bt_visualizarClick(Sender: TObject);
begin
      abretmplogo;
end;

procedure Tadm_admcustomhelp.bt_priorClick(Sender: TObject);
begin
    dm.ds_int.Prior;
    atualiza_grade;
end;

procedure Tadm_admcustomhelp.bt_nextClick(Sender: TObject);
begin
    dm.ds_int.Next;
    atualiza_grade;
end;

procedure Tadm_admcustomhelp.bt_newClick(Sender: TObject);
begin
    addreg;
    lbcp2.SetFocus;
end;

procedure Tadm_admcustomhelp.bt_delClick(Sender: TObject);
begin
    if (dm.ds_int.RecordCount>0) and (lbcp1.Text<>'novo') then begin
        if dm.deleta_reg2(EnDecryptString(HelpSys.vrf03[1],37249),'codigo',lbcp1.Text,true) then begin
           dm.ds_int.Close;
           dm.ds_int.Open;
           dm.ds_int2.Close;
           dm.ds_int2.Open;
...
procedure Tadm_admcustomhelp.bt_saveClick(Sender: TObject);
...
begin
     ...
     if lbcp1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(HelpSys.vrf03[1],37249),2);
        with dm.executa_int do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(HelpSys.vrf03[1],37249)+' (codigo,cod_lng,descricao,sz,tp,wh,hg,img) '+
        ...
        with dm.executa_int do begin
            if xmodupdate then begin
               CommandText:= 'UPDATE '+EnDecryptString(HelpSys.vrf03[1],37249)+' SET descricao=:p0,sz=:p1,tp=:p2,wh=:p3,hg=:p4,img=:p5 '+
                          'WHERE codigo=:p6';
               xlogotmp.Seek(0,soFromBeginning);
               Params[5].LoadFromStream(xlogotmp,ftblob);
               Params[6].AsInteger:=axcod;
            end else begin
               CommandText:= 'UPDATE '+EnDecryptString(HelpSys.vrf03[1],37249)+' SET descricao=:p0,sz=:p1,tp=:p2,wh=:p3,hg=:p4 '+
                          'WHERE codigo=:p5';
...
procedure Tadm_admcustomhelp.NumKeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_admcustomhelp.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tadm_admcustomhelp.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_admcustomhelp.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

function Tadm_admcustomhelp.retorna_TTabSheet(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtb1;
          1: Result:=lngtb2;
...
function Tadm_admcustomhelp.retorna_TTreeView(xidxcod:Integer):TTreeView;
begin
      Case xidxcod of
...
function Tadm_admcustomhelp.retorna_TTabSheet2(xidxcod:Integer):TTabSheet;
begin
      Case xidxcod of
          0: Result:=lngtbre1;
          1: Result:=lngtbre2;
...
function Tadm_admcustomhelp.retorna_TRxRichEdit(xidxcod:Integer):TRxRichEdit;
begin
      Case xidxcod of
          0: Result:=re_editor1;
          1: Result:=re_editor2;
...
procedure Tadm_admcustomhelp.xmn_montamenusEditar;
...
function Tadm_admcustomhelp.localiza_mdl(xcodmdl:Integer):Integer;
...
procedure Tadm_admcustomhelp.addreg4;
begin
     xmn_text.ReadOnly:=false;
     xmn_text.Color:=clWindow;
     xmn_mdl.Color:=clWindow;
...
procedure Tadm_admcustomhelp.xmn_montamenus1Change(Sender: TObject;
  Node: TTreeNode);
begin
      xnoteselect:= Node;
...
procedure Tadm_admcustomhelp.xmn_montamenus1Click(Sender: TObject);
begin
      xmn_montamenusEditar;
end;

procedure Tadm_admcustomhelp.bt_it1Click(Sender: TObject);
begin
     addreg4;
     xmn_text.SetFocus;
end;

procedure Tadm_admcustomhelp.bt_it3Click(Sender: TObject);
...
begin
     ...
     if chk_vincular.Checked then begin
        ...
        xerror:=false;
        if xcoditem>0 then
        if not dm.deleta_reg2(EnDecryptString(HelpSys.vrf02[1],37249),'cod_item',IntToStr(xcoditem),false) then xerror:=true;
     end else begin
        retorna_TTreeView(PageControl_languagens.TabIndex).Selected.Delete;
        if not dm.deleta_reg2(EnDecryptString(HelpSys.vrf02[1],37249),'codigo',InttoStr(editorlng.codigo[PageControl_languagens.TabIndex]),false) then xerror:=true;
     end;
     atualiza_nitens;
     xmn_text.Text:='';
     xmn_mdl.ItemIndex:=0;
...
procedure Tadm_admcustomhelp.bt_it2Click(Sender: TObject);
begin
...
procedure Tadm_admcustomhelp.bt_it4Click(Sender: TObject);
begin
      IncMenu(0);
end;

procedure Tadm_admcustomhelp.bt_it5Click(Sender: TObject);
begin
      IncMenu(1);
end;

procedure Tadm_admcustomhelp.atualiza_nitens;
begin
     Lb_countreg4.Caption:=IntToStr(retorna_TTreeView(PageControl_languagens.TabIndex).Items.Count);
end;

procedure Tadm_admcustomhelp.xmn_montamenusDisable;
begin
     bt_it1.Enabled:=true;
     bt_it2.Enabled:=false;
     bt_it3.Enabled:=false;
...
procedure Tadm_admcustomhelp.IncMenu(xtype_inc:byte);
...
begin
        ...
        if not xitemcheck then begin
           ...
           xtypemdl:=2;
           With dm.executa_int do begin
                CommandText := 'SELECT MAX(cod_item) FROM '+EnDecryptString(HelpSys.vrf02[1],37249);
                Open;
           ...
           for xind_trad:=0 to menutmp.count-1 do begin
               loadx.Progressloading.Position:=loadx.Progressloading.Position+1;
               ax_coditem:=dm.GetCodMax(EnDecryptString(HelpSys.vrf02[1],37249),2);
               with dm.executa_int do begin
                    CommandText:= 'INSERT INTO '+EnDecryptString(HelpSys.vrf02[1],37249)+' (codigo,cod_lng,cod_item) '+
                               'VALUES(:p0,:p1,:p2)';
           ...
           if xerror then begin
              dm.deleta_reg2(EnDecryptString(HelpSys.vrf02[1],37249),'cod_item',IntToStr(x_coditem),false);
              loadx.Close;
              xmodupdate:=false;
              xupxmenumn:=false;
        ...
        if xmn_mdl.ItemIndex>0 then begin
           if dm.ds_int5.Locate('mdl',xmn_mdl_ext.Strings[xmn_mdl.ItemIndex],[]) then
              x_codmod:=dm.ds_int5.fieldbyname('codigo').AsInteger;
        end else x_codmod:=0;
           ...
           if chk_traduzir.Checked then begin
              ...
              if (xid_default=xidxmenu) then
                 ...
                 if xtxt1 = '<|erro|>' then begin
                    loadx.Close;
                    if xitemcheck then dm.deleta_reg2(EnDecryptString(HelpSys.vrf02[1],37249),'cod_item',IntToStr(x_coditem),false);
                    xmodupdate:=false;
                    xupxmenumn:=false;
                    Panel1.Enabled:=true;
                    TabSheet_editor.Enabled:=xstatedenable;
...
procedure Tadm_admcustomhelp.chk_vincularClick(Sender: TObject);
begin
      if chk_vincular.Checked then begin
...
procedure Tadm_admcustomhelp.Tradutor1AoTraduzir(Sender: TObject;
  Traducao: String);
begin
    xreceivetrad:=Traducao;
end;

function Tadm_admcustomhelp.recebetraducao(xtrad_text,xtrad_de,xtrad_para: String):String;
var xrecebido: Bool;
begin
      xrecebido:=false;
      xreceivetrad:='';
...
procedure Tadm_admcustomhelp.IncItemMenu(xidxmenu,x_codmod,x_coditem :Integer; xtype_inc,xtypemdl:byte; xtxtdesc: string);
var xnode:TTreeNode;
    MenuRecPtr: PmenuRec;
begin
        New(MenuRecPtr);
...
procedure Tadm_admcustomhelp.bt_new4Click(Sender: TObject);
begin
     if hwsf.BtConfirma('Restaurar menu?') then
        montalng_menus;
end;

procedure Tadm_admcustomhelp.bt_del4Click(Sender: TObject);
var xmnmenu: Integer;
    xerror: Boolean;
    msg_error: String;
begin
     if hwsf.BtConfirma('Limpar todos os itens do menu, processo irreversível?') then begin
        ...
        if chk_vincular.Checked then begin
           ...
           if not dm.deleta_reg2(EnDecryptString(HelpSys.vrf02[1],37249),'cod_lng>','0',false) then xerror:=true;
        end else begin
           ...
           if not dm.deleta_reg2(EnDecryptString(HelpSys.vrf02[1],37249),'cod_lng',IntToStr(menutmp.cod_lng[PageControl_languagens.TabIndex]),false) then xerror:=true;
        end;
        atualiza_nitens;
        xmn_text.Text:='';
        xmn_mdl.ItemIndex:=0;
...
procedure Tadm_admcustomhelp.atualiza_xmenumnt;
begin
     //monta menu
...
function Tadm_admcustomhelp.xmn_montamenus_treeview:Boolean;
var xformn: Integer;
    xmnmenu: Integer;
    MenuRecPtr: PmenuRec;             
begin
...
function Tadm_admcustomhelp.update_itensmenus:Boolean;
...
begin
      ...
      loadx.Progressloading.position:=25;
      for xmnmenu:=0 to menutmp.count-1 do begin
          if dm.executa_int5.Locate('cod_lng',menutmp.cod_lng[xmnmenu],[]) then begin
             axcod:=dm.executa_int5.fieldbyname('codigo').AsInteger;
             loadx.Progressloading.position:=50;
             with dm.executa_int do begin
                  CommandText:= 'UPDATE '+EnDecryptString(HelpSys.vrf01[1],37249)+' SET descricao=:p0,cod_lng=:p1,obs=:p2,tvw=:p3 '+
                             ' WHERE codigo=:p4';
             ...
             axcod:=dm.GetCodMax(EnDecryptString(HelpSys.vrf01[1],37249),2);
             loadx.Progressloading.position:=50;
             with dm.executa_int do begin
                  CommandText:= 'INSERT INTO '+EnDecryptString(HelpSys.vrf01[1],37249)+' (codigo,descricao,cod_lng,obs,tvw) '+
...
function Tadm_admcustomhelp.xmn_montamenus_str:Boolean;
var xformn: Integer;
    xmnmenu: Integer;
begin
     xupxmenumn:=true;
...
function Tadm_admcustomhelp.preenchezeros(xcodnum,xquantzeros: Integer):string;
...
procedure Tadm_admcustomhelp.montalng_menus;
...
procedure Tadm_admcustomhelp.PageControl_languagensChange(Sender: TObject);
begin
     xmn_montamenusEditar;
     Edit_title.Text:=ax_traduzirall_title[PageControl_languagens.TabIndex];
end;

procedure Tadm_admcustomhelp.SpeedButton1Click(Sender: TObject);
begin
...
procedure Tadm_admcustomhelp.Timer_traduzTimer(Sender: TObject);
begin
      Inc(xtimerresponse);
end;

function Tadm_admcustomhelp.BitmapToRtf(graph:TBitmap):string;
...
procedure Tadm_admcustomhelp.incluiimgeditor(xcodimg,xlinha,xidxrxre: Integer);
...
procedure Tadm_admcustomhelp.incluiimgeditor2(xlinha,xidxrxre: Integer);
var SS: TStringStream;
    axstrbmp: TMemoryStream;
    axbmp: TBitmap;
begin
...
procedure Tadm_admcustomhelp.re_editorURLClick(Sender: TObject;
...
procedure Tadm_admcustomhelp.Selecionartudo1Click(Sender: TObject);
begin
      retorna_TRxRichEdit(PageControl_editor.TabIndex).SelectAll;
end;

procedure Tadm_admcustomhelp.PageControl_editorChange(Sender: TObject);
begin
      StatusBar.Panels[3].Text:=retorna_TTabSheet2(PageControl_editor.TabIndex).Caption;
end;

procedure Tadm_admcustomhelp.SpeedButton2Click(Sender: TObject);
var xnode:TTreeNode;
...
procedure Tadm_admcustomhelp.SpeedButton3Click(Sender: TObject);
var xnode:TTreeNode;
    xIdnode,xmnmenu,xmnmenucout:Integer;
    xerror,error_vinc:Bool;
begin
...
procedure Tadm_admcustomhelp.Imagem1Click(Sender: TObject);
var axcodimg,axlinimg,axtabidx: Integer;
begin
     try
         dm.ds_int2.Filter:='tp='+#39+'2'+#39;
         Application.CreateForm(Tadm_customSelectImg,adm_customSelectImg);
         adm_customSelectImg.axfilter:=dm.ds_sql2.Filter;
         adm_customSelectImg.xtabela:=EnDecryptString(HelpSys.vrf03[1],37249);
         adm_customSelectImg.DBGridimg.DataSource:=dm.source_int2;
         adm_customSelectImg.xdataset:=dm.ds_int2;
         adm_customSelectImg.xexecuta:=dm.executa_int;
...
procedure Tadm_admcustomhelp.Hyperlink1Click(Sender: TObject);
var xdesc: string;
begin
...
procedure Tadm_admcustomhelp.Arquivomultimedia1Click(Sender: TObject);
var axcodimg,axlinimg,axtabidx: Integer;
begin
     try
         dm.ds_int2.Filter:='tp<'+#39+'2'+#39;
...
procedure Tadm_admcustomhelp.re_editorKeyUp(Sender: TObject;
