...
uses
  ...
  uadm_vlrModcli in 'uadm_vlrModcli.pas' {adm_vlrModcli};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS preços mód. Cli'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS preços mód. Cli';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_vlrModcli, adm_vlrModcli);
          end else begin
