...
var
  adm_hdatividades: Tadm_hdatividades;
...
procedure Tadm_hdatividades.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     dm.ds_sql3.Filter:='';
...
procedure Tadm_hdatividades.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_hdatividades.FormCreate(Sender: TObject);
...
begin
     ...
     With dm.ds_sql3 do begin
        CommandText:= 'SELECT * FROM '+EnDecryptString(Helpdesk.vrf03,37249)+' ORDER BY codigo';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
procedure Tadm_hdatividades.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tadm_hdatividades.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_hdatividades.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_hdatividades.atualiza_grade;
begin
      if dm.ds_sql3.RecordCount <= 0 then begin
         ...
         with dm.ds_sql3 do begin
            ...
            lbcp3.Text:= formatfloat('0.00',fieldbyname('vhst').AsFloat);
            lbcp4.Lines.Assign(fieldbyname('obs'));
         end;
      end;
      Lb_countreg.Caption:=Inttostr(dm.ds_sql3.RecordCount);
end;

procedure Tadm_hdatividades.addreg;
begin
     lbcp1.Text:='novo';
     lbcp2.Text:= '';
     lbcp3.Text:= '0,00';
...
procedure Tadm_hdatividades.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_hdatividades.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_hdatividades.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_hdatividades.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_hdatividades.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_hdatividades.Left:= adm_hdatividades.Left-(mv_mouse.x-x);
      adm_hdatividades.Top:= adm_hdatividades.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_hdatividades.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_hdatividades.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_hdatividades.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_hdatividades.bt_maximizaClick(Sender: TObject);
begin
    if adm_hdatividades.WindowState=wsMaximized then adm_hdatividades.WindowState:=wsNormal
    else adm_hdatividades.WindowState:=wsMaximized;
end;

procedure Tadm_hdatividades.FormResize(Sender: TObject);
begin
...
procedure Tadm_hdatividades.bt_newClick(Sender: TObject);
begin
    addreg;
    lbcp2.SetFocus;
end;

procedure Tadm_hdatividades.bt_delClick(Sender: TObject);
begin
    if (dm.ds_sql3.RecordCount>0) and (lbcp1.Text<>'novo') then begin
        if dm.deleta_reg(EnDecryptString(Helpdesk.vrf03,37249),'codigo',lbcp1.Text,true) then begin
           dm.ds_sql3.Close;
           dm.ds_sql3.Open;
           atualiza_grade;
        end;
...
procedure Tadm_hdatividades.bt_saveClick(Sender: TObject);
var axcod : integer;
begin
     if lbcp1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Helpdesk.vrf03,37249),1);
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Helpdesk.vrf03,37249)+' (codigo,descricao,vhst,obs) '+
                       'VALUES(:p0,:p1,:p2,:p3)';
            Params[0].AsInteger:=axcod;
            Params[1].AsString:=lbcp2.Text;
            Params[2].AsFloat:=StrToFloat(lbcp3.Text);
            Params[3].Assign(lbcp4.Lines);
        end;
     end else begin
        axcod:=StrtoInt(lbcp1.Text);
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Helpdesk.vrf03,37249)+' SET descricao=:p0,vhst=:p1,obs=:p2 '+
                       'WHERE codigo=:p3';
            Params[0].AsString:=lbcp2.Text;
            Params[1].AsFloat:=StrToFloat(lbcp3.Text);
            Params[2].Assign(lbcp4.Lines);
...
procedure Tadm_hdatividades.bt_nextClick(Sender: TObject);
begin
    dm.ds_sql3.Next;
    atualiza_grade;
end;

procedure Tadm_hdatividades.bt_priorClick(Sender: TObject);
begin
    dm.ds_sql3.Prior;
    atualiza_grade;
end;

procedure Tadm_hdatividades.Edit_searchChange(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_hdatividades.filtrar_reg;
begin
...
procedure Tadm_hdatividades.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_hdatividades.ComboBox_searchChange(Sender: TObject);
begin
...
procedure Tadm_hdatividades.Edit_searchKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tadm_hdatividades.lbcp3KeyPress(Sender: TObject;
  var Key: Char);
begin
   Case key of
     '0'..'9' :;
