...
procedure Tadm_usrpass.bt_fecharClick(Sender: TObject);
begin
    close;
end;
...
procedure Tadm_usrpass.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_usrpass.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_usrpass.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_usrpass.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_usrpass.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_usrpass.Left:= adm_usrpass.Left-(mv_mouse.x-x);
      adm_usrpass.Top:= adm_usrpass.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_usrpass.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_usrpass.bt_minimizeClick(Sender: TObject);
begin
...
procedure Tadm_usrpass.FormResize(Sender: TObject);
begin
...
begin
     ...
     With dm.executa do begin
          Try
              CommandText:='Select * from '+EnDecryptString(Serverconect.vrf19[0],37249)+' Where decode(usuario,'+#39+cv+#39+') = '+#39+lbcp1.Text+#39+
     ...
     //grava dados
     with dm.executa do begin
          CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf19[0],37249)+' SET usuario=encode(:p0,:p1),password=encode(:p2,:p3) '+
                     'WHERE codigo=:p4';
