...
uses
  ...
  uestatisticas in 'uestatisticas.pas' {estatisticas};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Estatísticas'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Estatísticas';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Testatisticas, estatisticas);
          end else begin
