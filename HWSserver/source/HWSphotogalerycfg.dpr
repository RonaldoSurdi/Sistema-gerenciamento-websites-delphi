...
uses
  ...
  umdl_photogalerycfg in 'umdl_photogalerycfg.pas' {mdl_photogalerycfg};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Configurações para galeria de fotos'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Configurações para galeria de fotos';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tmdl_photogalerycfg, mdl_photogalerycfg);
          end else begin
