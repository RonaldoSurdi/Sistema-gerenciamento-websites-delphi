...
begin
     ...
     With dm.ds_sql3 do begin
        CommandText:= 'select stat,codigo,usuario,dt_cad,cod_web,cod_ent,dt_cad,xconf from '+EnDecryptString(Serverconect.vrf19[0],37249);
        Open;
        Filter:='';
        Filtered:=true;
     end;
     ...
     With dm.ds_sql5 do begin
        CommandText:= 'SELECT DISTINCT B.codigo,B.descricao,B.mdl,B.lastver,A.cod_web FROM '+EnDecryptString(Serverconect.vrf32[0],37249)+' A, '+
                               EnDecryptString(Serverconect.vrf24[0],37249)+' B '+
                               'WHERE B.codigo=A.cod_grp ORDER BY B.descricao';//A.obs,
        Open;
     ...
     With dm.ds_sql do begin
        CommandText:= 'select codigo,descricao,ordem from '+EnDecryptString(Serverconect.vrf37[0],37249)+' order by ordem';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
begin
     ...
     With dm.executa do begin
        CommandText:='SELECT foto FROM '+EnDecryptString(Serverconect.vrf34,37249)+'_ax WHERE codigo='+#39+IntToStr(axcodBlb)+#39;
        Open;
...
begin
     try
        axcodBlb:=StrtoInt(lbce1.Text);
     Except
        axcodBlb:=0;
     End;
     xicotmp.Clear;
     With dm.executa do begin
        CommandText:='SELECT ico FROM '+EnDecryptString(Serverconect.vrf37[0],37249)+' WHERE codigo='+#39+IntToStr(axcodBlb)+#39;
        Open;
        if not fields[0].IsNull then begin
...
begin
     ...
     if lbcp1.Text = 'novo' then begin
        ...
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf19[0],37249)+' (codigo,cod_web,cod_ent,usuario,password,stat,dt_cad,xconf) '+
                          'VALUES(:p0,:p1,:p2,:p3,:p4,:p5,'+#39+FormatDateTime('yyyymmdd hh:mm:ss.000',Now)+#39+',:p6)';
        ...
        with dm.executa do begin
            if lbcp7.Text<>'' then
               CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf19[0],37249)+' SET cod_web=:p0,cod_ent=:p1,usuario=:p2,password=:p3,stat=:p4,xconf=:p5 '+
                             'WHERE codigo=:p6'
            else
               CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf19[0],37249)+' SET cod_web=:p0,cod_ent=:p1,usuario=:p2,stat=:p3,xconf=:p4 '+
                             'WHERE codigo=:p5';
...
begin
     ...
     if marcar then begin
        if not dm.ds_sql5.Locate('descricao',mod_combo.Text,[]) then begin
           hwsf.BtMensagem('Módulo selecionado inválido.');
           exit;
        end;
        With dm.executa do begin
            axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf33[0],37249),1);
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf33[0],37249)+' (codigo,cod_usr,cod_grp,cod_menu,ordem) '+
...
begin
     if lbcb1.Text = 'novo' then begin
        ...
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf34,37249)+' (codigo,cod_ent,cod_fc,cod_set,nome,email,dtnasc,telefone1,telefone2,fax,celular,endereco,numero,complemento,cod_cidade,cep,obs) '+
                       'VALUES(:p0,:p1,:p2,:p3,:p4,:p5,'+#39+FormatDateTime('yyyymmdd hh:mm:ss.000',lbcb10.DateTime)+#39+',:p6,:p7,:p8,:p9,:p10,:p11,:p12,:p13,:p14,:p15)';
            Params[0].AsInteger:=axcod;
            try
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf34,37249)+' SET cod_ent=:p0,cod_fc=:p1,cod_set=:p2,nome=:p3,email=:p4,dtnasc='+#39+FormatDateTime('yyyymmdd hh:mm:ss.000',lbcb10.DateTime)+#39+
                       ',telefone1=:p5,telefone2=:p6,fax=:p7,celular=:p8,endereco=:p9,numero=:p10,complemento=:p11,cod_cidade=:p12,cep=:p13,obs=:p14 '+
                       'WHERE codigo=:p15';
            try
    ...
    // end;
     ...
     xexecutaimg:=true;
     if lbcp1.Text = 'novo' then begin
        xexecutaimg:=dm.PostBinaryStream(EnDecryptString(Serverconect.vrf34,37249)+'_ax','INST','codigo','','foto','','','',IntToStr(axcod),xlogotmp,nil);
     end else begin
        xexecutaimg:=dm.PostBinaryStream(EnDecryptString(Serverconect.vrf34,37249)+'_ax','UPDT','codigo','','foto','','','',IntToStr(axcod),xlogotmp,nil);
     end;
...
begin
     if lbce1.Text = 'novo' then begin
        ...
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf37[0],37249)+' (codigo,descricao,ordem) '+
                       'VALUES(:p0,:p1,:p2)';
            Params[0].AsInteger:=axcod;
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf37[0],37249)+' SET descricao=:p0 '+
                       'WHERE codigo=:p1';
     ...
     xicotmp.Seek(0,soFromBeginning);
     xexecutaimg:=dm.PostBinaryStream(EnDecryptString(Serverconect.vrf37[0],37249),'UPDT','codigo','','ico','','','',IntToStr(axcod),xicotmp,nil);
     if not xexecutaimg then begin
        hwsf.BtMensagem('Erro ao gravar dados binários.');
        exit;
     end;
...
begin
     with dm.executa do begin
          if dm.ds_sql.RecordCount > 1 then begin
             ...
             CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf37[0],37249)+' SET ordem=:p0 '+
                        'WHERE ordem=:p1';
...
begin
     with dm.executa do begin
          if dm.ds_sql4.RecordCount > 1 then begin
             ...
             CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf33[0],37249)+' SET ordem=:p0 '+
                        'WHERE codigo=:p1';
             Params[0].AsInteger:=ax_ord1;
...
begin
     With dm.ds_sql4 do begin
        ...
        CommandText:='SELECT A.codigo, A.ordem, A.obs, B.descricao FROM '+EnDecryptString(Serverconect.vrf33[0],37249)+' A, '+
                             EnDecryptString(Serverconect.vrf24[0],37249)+' B '+
...
begin
     With dm.ds_sql2 do begin
        if Active then Close;
        CommandText:='SELECT MAX(ordem) from '+EnDecryptString(Serverconect.vrf37[0],37249);
...
begin
     ...
     if hwsf.BtConfirma('Reorganizar menus?') then begin
        ...
        Try
          With dm.ds_sql2 do begin
               CommandText:= 'select codigo,descricao,ordem from '+EnDecryptString(Serverconect.vrf37[0],37249)+' order by descricao';
               Open;
               loadx.Progressloading.Max:=RecordCount;
               First;
               While not Eof do begin
                    loadx.Progressloading.position:= ax_ord;
                    dm.executa.CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf37[0],37249)+' SET ordem=:p0 '+
                                              'WHERE codigo=:p1';
...
begin
     ...
     if hwsf.BtConfirma('Reorganizar itens?') then begin
        ...
        Try
          With dm.ds_sql2 do begin
               CommandText:='SELECT A.codigo, A.ordem, B.descricao FROM '+EnDecryptString(Serverconect.vrf33[0],37249)+' A, '+
                             EnDecryptString(Serverconect.vrf24[0],37249)+' B '+
               ...
               While not Eof do begin
                    loadx.Progressloading.position:= ax_ord;
                    dm.executa.CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf33[0],37249)+' SET ordem=:p0 '+
                                              'WHERE codigo=:p1';
                    dm.executa.Params[0].AsInteger:=ax_ord;
                    dm.executa.Params[1].AsInteger:=fieldbyname('codigo').AsInteger;
                    try
...
procedure Tadm_usrwebsites.bt_automdlClick(Sender: TObject);
var xcodgrp: String;
    axcod: Integer;
    axshowing,axlocate: Boolean;
begin
      //copy mdls
      if not hwsf.BtConfirma('Reatribuir Menu para usuário '+lbcp6.Text) then
          exit;
      xcodgrp:= hwsf.BtDesc('Digite o código do usuário');
      if (xcodgrp = '') then begin
         hwsf.BtMensagem('Digite o código do usuário.');
         exit;
      end;
      axshowing:= loadx.Showing;
      if not axshowing then begin
         loadx := Tloadx.Create(Application);
         loadx.lbloading.Caption:='Copiando lista de itens';
         loadx.Show;
         loadx.Update;
      end else begin
         loadx.lbloading.Caption:='Copiando lista de itens';
      ...
      loadx.Progressloading.position:=10;
      With dm.executa do begin
            loadx.lbloading.Caption:='Removendo Menu antigo';
            loadx.lbloading.Repaint;
            CommandText:= 'DELETE FROM '+EnDecryptString(Serverconect.vrf33[0],37249)+
                          ' WHERE cod_usr='+#39+lbcp1.Text+#39;
      ...
      loadx.lbloading.Caption:='Localizando itens do Menu';
      loadx.lbloading.Repaint;
      dm.executa2.CommandText:='SELECT codigo,cod_usr,cod_grp,cod_menu,ordem FROM '+EnDecryptString(Serverconect.vrf33[0],37249)+
                        ' WHERE cod_usr='+#39+xcodgrp+#39+
                        ' ORDER BY cod_menu';
      ...
      While not dm.executa2.Eof do begin
         With dm.executa do begin
            ...
            loadx.lbloading.Caption:='Adicionando item '+IntToStr(loadx.Progressloading.position)+' de '+IntToStr(loadx.Progressloading.Max);
            loadx.lbloading.Repaint;
            ...
            Params[0].AsInteger:=axcod;
            Params[1].AsInteger:=StrtoInt(lbcp1.Text);
            Params[2].AsInteger:=dm.executa2.fieldbyname('cod_grp').AsInteger;
            Params[3].AsInteger:=dm.executa2.fieldbyname('cod_menu').AsInteger;
            Params[4].AsInteger:=dm.executa2.fieldbyname('ordem').AsInteger;
            Try
               Execute;
            Except
               loadx.Progressloading.position:=0;
