...
uses
  ...
  uadm_cidades in 'uadm_cidades.pas' {adm_languagens};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Cidades'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Cidades';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_cidades, adm_cidades);
          end else begin
