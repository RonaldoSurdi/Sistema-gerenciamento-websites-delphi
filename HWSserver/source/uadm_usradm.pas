...
procedure Tadm_usradm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     xlogotmp.Free;
     xicotmp.Free;
     dm.ds_int.Filter:='';
...
procedure Tadm_usradm.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_usradm.FormCreate(Sender: TObject);
...
begin
     ...
     DBGrid2.DataSource:=dm.source_int4;
     DBGrid3.DataSource:=dm.source_int;
     ...
     DBGrid1.Columns.Items[4].Title.Caption:='Colaborador';
     DBGrid1.Columns.Add;
     DBGrid1.Columns.Items[5].FieldName:='dt_cad';
     DBGrid1.Columns.Items[5].Title.Caption:='Data de cadastro';
     With dm.ds_int3 do begin
        CommandText:= 'select stat,codigo,decode(usuario,'+chr(39)+cv+chr(39)+'),decode(password,'+chr(39)+cv+chr(39)+'),cod_ent,dt_cad from '+EnDecryptString(Serverconect.vrf19[1],37249);
        Open;
        Filter:='';
        Filtered:=true;
     end;
     ...
     With dm.ds_sql do begin
        CommandText:='SELECT * FROM '+EnDecryptString(Serverconect.vrf34,37249)+' ORDER BY nome';
        Open;
        First;
        lbcp5.Clear;
     ...
     With dm.ds_int5 do begin
        CommandText:= 'SELECT DISTINCT B.codigo,B.descricao,B.mdl,B.lastver,A.obs,A.cod_ent FROM '+EnDecryptString(Serverconect.vrf32[1],37249)+' A, '+
     ...
     With dm.ds_int do begin
        CommandText:= 'select codigo,descricao,ordem from '+EnDecryptString(Serverconect.vrf37[1],37249)+' order by ordem';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
procedure Tadm_usradm.Remonta_menu(xcod_usr,xcod_menu: Integer);
begin
     With dm.ds_int4 do begin
        if Active then Close;
        CommandText:='SELECT A.codigo, A.ordem, A.obs, B.descricao FROM '+EnDecryptString(Serverconect.vrf33[1],37249)+' A, '+
                             EnDecryptString(Serverconect.vrf24[1],37249)+' B '+
...
function Tadm_usradm.GetMaxorder_menu:Integer;
begin
     With dm.ds_int2 do begin
        if Active then Close;
        CommandText:='SELECT MAX(ordem) from '+EnDecryptString(Serverconect.vrf37[1],37249);
...
function Tadm_usradm.GetMaxorder:Integer;
begin
     With dm.ds_int2 do begin
        if Active then Close;
        CommandText:='SELECT MAX(ordem) from '+EnDecryptString(Serverconect.vrf33[1],37249)+
                             ' WHERE cod_usr='+#39+lbcp1.Text+#39+' AND cod_menu='+#39+lbce1.Text+#39;
        Open;
        If RecordCount>0 then Result:= (Fields[0].AsInteger+1)
        else Result:=1;
...
procedure Tadm_usradm.Remonta_mod(xcod_ent: Integer);
begin
     With dm.ds_int5 do begin
        Filter:='cod_ent='+#39+InttoStr(xcod_ent)+#39;
        mod_combo.Clear;
        First;
        While not Eof do begin
...
procedure Tadm_usradm.DBGrid1CellClick(Column: TColumn);
begin
     if not impmodule then atualiza_grade
     else impmodule:=false;
end;

procedure Tadm_usradm.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     atualiza_grade;
end;

procedure Tadm_usradm.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
     atualiza_grade;
end;

procedure Tadm_usradm.atualiza_grade;
begin
...
procedure Tadm_usradm.atualiza_modulos;
var xodmenu: integer;
begin
     ...
     if (lbcp1.Text<>'') and (lbcp1.Text<>'novo') then
         Remonta_menu(StrtoInt(lbcp1.Text),xodmenu);
end;

procedure Tadm_usradm.atualiza_colaborador;
begin
     if dm.ds_sql.Locate('codigo',lbcp4.Text,[]) then begin
        With dm.ds_sql do begin
...
procedure Tadm_usradm.getLogo;
begin
     xlogotmp.Clear;
     With dm.executa do begin
        CommandText:='SELECT foto FROM '+EnDecryptString(Serverconect.vrf34,37249)+'_ax WHERE codigo='+#39+lbcb1.Text+#39;
        Open;
...
procedure Tadm_usradm.abretmplogo;
var erro: Boolean;
    ax_filenm: TFileName;
begin
...
procedure Tadm_usradm.getLogo2;
begin
     xicotmp.Clear;
     With dm.executa_int do begin
        CommandText:='SELECT ico FROM '+EnDecryptString(Serverconect.vrf37[1],37249)+' WHERE codigo='+#39+lbce1.Text+#39;
        Open;
        if not fields[0].IsNull then begin
...
procedure Tadm_usradm.abretmplogo2;
var erro: Boolean;
    ax_filenm: TFileName;
begin
...
procedure Tadm_usradm.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_usradm.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_usradm.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_usradm.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_usradm.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_usradm.Left:= adm_usradm.Left-(mv_mouse.x-x);
      adm_usradm.Top:= adm_usradm.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_usradm.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_usradm.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_usradm.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_usradm.bt_maximizaClick(Sender: TObject);
begin
    if adm_usradm.WindowState=wsMaximized then adm_usradm.WindowState:=wsNormal
    else adm_usradm.WindowState:=wsMaximized;
end;

procedure Tadm_usradm.FormResize(Sender: TObject);
begin
...
procedure Tadm_usradm.bt_newClick(Sender: TObject);
begin
    addreg;
    lbcp4.SetFocus;
end;

procedure Tadm_usradm.bt_delClick(Sender: TObject);
begin
    if (dm.ds_int3.RecordCount>0) and (lbcp1.Text<>'novo') then begin
        if dm.deleta_reg2(EnDecryptString(Serverconect.vrf19[1],37249),'codigo',lbcp1.Text,true) then begin
           dm.ds_int3.Close;
           dm.ds_int3.Open;
           atualiza_grade;
        end;
...
procedure Tadm_usradm.bt_saveClick(Sender: TObject);
var axcod : integer;
begin
     if not dm.ds_sql.Locate('codigo',lbcp4.Text,[]) then begin
     ...
     if lbcp1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf19[1],37249),2);
        with dm.executa_int do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf19[1],37249)+' (codigo,cod_ent,usuario,password,stat,dt_cad) '+
                       'VALUES(:p0,:p1,encode(:p2,:p3),encode(:p4,:p5),:p6,:p7)';
            ...
            Params[7].AsString:=FormatDateTime('yyyymmdd hhmmss',Now);
        end;
     end else begin
        axcod:=StrtoInt(lbcp1.Text);
        with dm.executa_int do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf19[1],37249)+' SET cod_ent=:p0,usuario=encode(:p1,:p2),password=encode(:p3,:p4),stat=:p5 '+
                       'WHERE codigo=:p6';
            Params[0].AsInteger:=StrToInt(lbcp4.Text);
            Params[1].AsString:=lbcp6.Text;
            Params[2].AsString:=cv;
            Params[3].AsString:=lbcp7.Text;
...
procedure Tadm_usradm.bt_nextClick(Sender: TObject);
begin
    dm.ds_int3.Next;
    if dm.ds_sql.Locate('nome',lbcp5.Text,[]) then begin
       lbcp4.Text:=dm.ds_sql.fieldbyname('codigo').AsString;
    end else lbcp4.Text:='';
    atualiza_grade;
end;

procedure Tadm_usradm.bt_priorClick(Sender: TObject);
begin
    dm.ds_int3.Prior;
    if dm.ds_sql.Locate('nome',lbcp5.Text,[]) then begin
       lbcp4.Text:=dm.ds_sql.fieldbyname('codigo').AsString;
...
procedure Tadm_usradm.filtrar_reg;
var axsearch : string;
begin
     ...
     if Edit_search.Text='' then begin
        ...
        if ComboBox_search.ItemIndex=2 then begin
           if dm.executa5.Locate('nome',Edit_search.Text,[]) then begin
               ...
               dm.ds_int3.filter:= cp_search[ComboBox_search.ItemIndex]+'='+QuotedStr(axsearch);
           end;
        end else begin
...
procedure Tadm_usradm.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_usradm.ComboBox_searchChange(Sender: TObject);
begin
...
procedure Tadm_usradm.marcarmodulo(marcar: boolean; axcod: integer);
begin
     if mod_combo.Items.Count < 1 then exit;
     if marcar then begin
        With dm.executa_int do begin
            axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf33[1],37249),2);
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf33[1],37249)+' (codigo,cod_usr,cod_grp,cod_menu,ordem) '+
                       'VALUES(:p0,:p1,:p2,:p3,:p4)';
            Params[0].AsInteger:=axcod;
            Params[1].AsInteger:=StrtoInt(lbcp1.Text);
            if dm.ds_int5.Locate('descricao',mod_combo.Text,[]) then
               Params[2].AsInteger:=dm.ds_int5.FieldByName('codigo').AsInteger;
        ...
        dm.deleta_reg2(EnDecryptString(Serverconect.vrf33[1],37249),'codigo',InttoStr(axcod),false);
     end;
     dm.ds_int4.Close;
     dm.ds_int4.Open;
     atualiza_itens;
...
procedure Tadm_usradm.bt_prior3Click(Sender: TObject);
begin
    dm.ds_int4.Prior;
end;

procedure Tadm_usradm.bt_next3Click(Sender: TObject);
begin
    dm.ds_int4.Next;
end;

procedure Tadm_usradm.Edit_searchKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tadm_usradm.Edit_searchChange(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_usradm.lbcp4KeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_usradm.lbcb3KeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_usradm.lbcb5KeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_usradm.lbcb7KeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_usradm.lbcb16KeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_usradm.lbcb20KeyPress(Sender: TObject; var Key: Char);
begin
     Case key of
       '0'..'9' :;
       #13, #8, #27:
...
procedure Tadm_usradm.bt_buscacepClick(Sender: TObject);
begin
      JumpTo('www.correios.com.br/servicos/cep/cep_default.cfm');
end;

procedure Tadm_usradm.lbcb18Change(Sender: TObject);
begin
...
procedure Tadm_usradm.lbcb4Change(Sender: TObject);
begin
     atualiza_modulos;
end;

procedure Tadm_usradm.lbcb3Change(Sender: TObject);
begin
     if lbcb3.Text <> '' then
...
procedure Tadm_usradm.lbcb5Change(Sender: TObject);
begin
     if lbcb5.Text <> '' then
...
procedure Tadm_usradm.lbcb6Change(Sender: TObject);
begin
...
procedure Tadm_usradm.lbcb7Change(Sender: TObject);
begin
     if lbcb7.Text <> '' then
...
procedure Tadm_usradm.lbcb8Change(Sender: TObject);
begin
...
procedure Tadm_usradm.lbcp5Change(Sender: TObject);
begin
     if dm.ds_sql.Locate('nome',lbcp5.Text,[]) then begin
        lbcp4.Text:=dm.ds_sql.fieldbyname('codigo').AsString;
     end else lbcp4.Text:='';
...
procedure Tadm_usradm.lbcp4Change(Sender: TObject);
begin
     if lbcp4.Text <> '' then begin
       if dm.ds_sql.Locate('codigo',lbcp4.Text,[]) then begin
          lbcp5.Text:=dm.ds_sql.fieldbyname('nome').AsString;
       end else lbcp5.Text:='';
       atualiza_colaborador;
     end;
end;

procedure Tadm_usradm.bt_importClick(Sender: TObject);
begin
...
procedure Tadm_usradm.bt_clearClick(Sender: TObject);
begin
     Try
       xlogotmp.Clear;
       abretmplogo;
...
procedure Tadm_usradm.bt_save2Click(Sender: TObject);
var axcod : integer;
    axcolab : string;
    xexecutaimg: Boolean;
begin
     if lbcb1.Text = 'novo' then begin
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf34,37249)+' SET cod_ent=:p0,cod_fc=:p1,cod_set=:p2,nome=:p3,email=:p4,dtnasc=:p5,telefone1=:p6,telefone2=:p7,fax=:p8,celular=:p9,endereco=:p10,numero=:p11,complemento=:p12,cod_cidade=:p13,cep=:p14,obs=:p15 '+
                       'WHERE codigo=:p16';
            try
     ...
     xlogotmp.Seek(0,soFromBeginning);
     xexecutaimg:=true;
     xexecutaimg:=dm.PostBinaryStream(EnDecryptString(Serverconect.vrf34,37249),'UPDT','codigo','','foto','','','',IntToStr(axcod),xlogotmp,nil);
...
procedure Tadm_usradm.bt_new2Click(Sender: TObject);
begin
     addreg2;
     lbcb2.SetFocus;
end;

procedure Tadm_usradm.bt_prior2Click(Sender: TObject);
begin
...
procedure Tadm_usradm.bt_next2Click(Sender: TObject);
begin
...
procedure Tadm_usradm.bt_del2Click(Sender: TObject);
begin
...
procedure Tadm_usradm.bt_next5Click(Sender: TObject);
begin
    dm.ds_int.Next;
    atualiza_menu;
end;

procedure Tadm_usradm.bt_prior4Click(Sender: TObject);
begin
    dm.ds_int.Prior;
    atualiza_menu;
end;

procedure Tadm_usradm.bt_import4Click(Sender: TObject);
begin
     if OpenPictureDialog1.Execute then begin
        try
           impmodule:=true;
...
procedure Tadm_usradm.bt_clear4Click(Sender: TObject);
begin
     Try
       xicotmp.Clear;
       abretmplogo2;
...
procedure Tadm_usradm.bt_del4Click(Sender: TObject);
begin
    if (dm.ds_int.RecordCount>0) and (lbce1.Text<>'novo') then begin
        if dm.deleta_reg2(EnDecryptString(Serverconect.vrf37[1],37249),'codigo',lbce1.Text,true) then begin
           dm.ds_int.Close;
           dm.ds_sql.Open;
...
procedure Tadm_usradm.atualiza_menu;
var xodmenu: Integer;
begin
     if dm.ds_int3.RecordCount <= 0 then begin
         addreg3;
     end else begin
         lbce1.Text:=dm.ds_int.fieldbyname('codigo').AsString;
         lbce2.Text:=dm.ds_int.fieldbyname('descricao').AsString;
...
procedure Tadm_usradm.DBGrid3CellClick(Column: TColumn);
begin
      atualiza_menu;
end;

procedure Tadm_usradm.DBGrid3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_menu;
end;

procedure Tadm_usradm.DBGrid3KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_menu;
end;

procedure Tadm_usradm.atualizaComboMenu(idxmenu:integer);
begin
     dm.ds_int.First;
     ComboBox_menu.Clear;
     While not dm.ds_int.Eof do begin
          ComboBox_menu.Items.Add(dm.ds_int.fieldbyname('descricao').AsString);
          dm.ds_int.Next;
     end;
     if idxmenu = 0 then dm.ds_int.First
     else dm.ds_int.Locate('codigo',idxmenu,[]);
     atualiza_menu;
end;

procedure Tadm_usradm.bt_new4Click(Sender: TObject);
begin
     addreg3;
     lbce2.SetFocus;
end;

procedure Tadm_usradm.bt_save4Click(Sender: TObject);
var axcod: integer;
begin
     if lbce1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf37[1],37249),2);
        with dm.executabin_int do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf37[1],37249)+' (codigo,descricao,ordem,ico) '+
                       'VALUES(:p0,:p1,:p2,:p3)';
            Params[0].AsInteger:=axcod;
        ...
        with dm.executabin_int do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf37[1],37249)+' SET descricao=:p0,ico=:p1 '+
                       'WHERE codigo=:p2';
...
procedure Tadm_usradm.ordmenu(tipoord: byte);
var ax_ord1, ax_ord2: integer;
begin
     with dm.executa_int do begin
          if dm.ds_int.RecordCount > 1 then begin
             Case tipoord of
                1: begin
                   ax_ord1:=dm.ds_int.Fieldbyname('ordem').AsInteger;
                   ax_ord2:=ax_ord1-1;
                ...
                2: begin
                   ax_ord1:=dm.ds_int.Fieldbyname('ordem').AsInteger;
                   ax_ord2:=ax_ord1+1;
                   if ax_ord2 = dm.ds_int.RecordCount+1 then exit;
                end;
             end;
             if not (dm.conecta_int.InTransaction) then begin
                TD.TransactionID := HourOfTheYear(now)+MinuteOfTheMonth(now)+SecondOfTheYear(now);
             ...
             CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf37[1],37249)+' SET ordem=:p0 '+
                        'WHERE ordem=:p1';
             ...
             Params[1].AsInteger:=dm.ds_int.Fieldbyname('codigo').AsInteger;
             try
                Execute;
             except
                hwsf.BtMensagem('Ocorreu algum erro.');
...
procedure Tadm_usradm.ordmod(tipoord: byte);
var ax_ord1, ax_ord2, ax_cod1, ax_cod2: integer;
begin
     with dm.executa_int do begin
          if dm.ds_int4.RecordCount > 1 then begin
             Case tipoord of
                1: begin
                   ax_ord1:=dm.ds_int4.Fieldbyname('ordem').AsInteger;
                   if ax_ord1 = 1 then exit;
                   ax_ord2:=ax_ord1-1;
                   ax_cod1:=dm.ds_int4.Fieldbyname('codigo').AsInteger;
                   dm.ds_int4.Prior;
                   ax_cod2:=dm.ds_int4.Fieldbyname('codigo').AsInteger;
                   dm.ds_int4.Next;
                end;
                2: begin
                   ax_ord1:=dm.ds_int4.Fieldbyname('ordem').AsInteger;
                   if ax_ord1 = dm.ds_int4.RecordCount then exit;
                   ax_ord2:=ax_ord1+1;
                   ax_cod1:=dm.ds_int4.Fieldbyname('codigo').AsInteger;
                   dm.ds_int4.Next;
                   ax_cod2:=dm.ds_int4.Fieldbyname('codigo').AsInteger;
             ...
             CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf33[1],37249)+' SET ordem=:p0 '+
                        'WHERE codigo=:p1';
             Params[0].AsInteger:=ax_ord2;
...
procedure Tadm_usradm.bt_acima4Click(Sender: TObject);
begin
...
procedure Tadm_usradm.bt_abaixo4Click(Sender: TObject);
begin
     if dm.ds_int.RecordCount > 0 then begin
        ordmenu(2);
        dm.ds_int.Next;
...
procedure Tadm_usradm.ComboBox_menuChange(Sender: TObject);
begin
     if dm.ds_int.Locate('descricao',ComboBox_menu.Text,[]) then begin
        atualiza_menu;
     end;
end;

procedure Tadm_usradm.SpeedButton1Click(Sender: TObject);
var ax_ord: Longint;
begin
     if dm.ds_int.RecordCount > 0 then
     if hwsf.BtConfirma('Reorganizar menus?') then begin
        ...
        Try
          With dm.ds_int2 do begin
               CommandText:= 'select codigo,descricao,ordem from '+EnDecryptString(Serverconect.vrf37[1],37249)+' order by descricao';
               Open;
               loadx.Progressloading.Max:=RecordCount;
               First;
               While not Eof do begin
                    loadx.Progressloading.position:= ax_ord;
                    dm.executa_int.CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf37[1],37249)+' SET ordem=:p0 '+
                                              'WHERE codigo=:p1';
                    dm.executa_int.Params[0].AsInteger:=ax_ord;
                    dm.executa_int.Params[1].AsInteger:=fieldbyname('codigo').AsInteger;
                    try
                        dm.executa_int.Execute;
                    except end;
                    Inc(ax_ord);
...
procedure Tadm_usradm.mod_addClick(Sender: TObject);
begin
...
procedure Tadm_usradm.mod_delClick(Sender: TObject);
begin
    if dm.ds_int4.RecordCount > 0 then
       marcarmodulo(false,dm.ds_int4.FieldByName('codigo').AsInteger);
end;

procedure Tadm_usradm.bt_acima3Click(Sender: TObject);
begin
     if dm.ds_int4.RecordCount > 0 then begin
        ordmod(1);
        dm.ds_int4.Prior;
     end;
end;

procedure Tadm_usradm.bt_abaixo3Click(Sender: TObject);
begin
     if dm.ds_int4.RecordCount > 0 then begin
        ordmod(2);
        dm.ds_int4.Next;
...
procedure Tadm_usradm.SpeedButton2Click(Sender: TObject);
var ax_ord: Longint;
begin
     if dm.ds_int4.RecordCount > 0 then
     if hwsf.BtConfirma('Reorganizar itens?') then begin
        ...
        Try
          With dm.ds_int2 do begin
               CommandText:='SELECT A.codigo, A.ordem, B.descricao FROM '+EnDecryptString(Serverconect.vrf33[1],37249)+' A, '+
                             EnDecryptString(Serverconect.vrf24[1],37249)+' B '+
               ...
               While not Eof do begin
                    loadx.Progressloading.position:= ax_ord;
                    dm.executa_int.CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf33[1],37249)+' SET ordem=:p0 '+
                                              'WHERE codigo=:p1';
                    dm.executa_int.Params[0].AsInteger:=ax_ord;
                    dm.executa_int.Params[1].AsInteger:=fieldbyname('codigo').AsInteger;
                    try
...
procedure Tadm_usradm.mod_clearClick(Sender: TObject);
var countreg : integer;
begin
     if dm.ds_int4.RecordCount = 0 then exit;
     ...
     With dm.ds_int4 do begin
          ...
          While not Eof do begin
              ...
              dm.deleta_reg2(EnDecryptString(Serverconect.vrf33[1],37249),'codigo',fieldbyname('codigo').AsString,false);
              Next;
              Inc(countreg);
          end;
          Close;
...
procedure Tadm_usradm.DBGrid2CellClick(Column: TColumn);
begin
      atualiza_itens;
end;

procedure Tadm_usradm.atualiza_itens;
begin
      With dm.ds_int4 do begin
...
procedure Tadm_usradm.DBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_itens;
end;

procedure Tadm_usradm.DBGrid2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_itens;
end;
