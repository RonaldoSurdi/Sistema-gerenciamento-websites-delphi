...
Módulo: Cadastro de aplicativos.
...
procedure Tadm_hdapli.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     dm.ds_sql3.Filter:='';
...
procedure Tadm_hdapli.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_hdapli.FormCreate(Sender: TObject);
...
begin
     ...
     With dm.ds_sql3 do begin
        CommandText:= 'SELECT * FROM '+EnDecryptString(Helpdesk.vrf04,37249)+' ORDER BY codigo';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
procedure Tadm_hdapli.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tadm_hdapli.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_hdapli.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_hdapli.atualiza_grade;
begin
...
procedure Tadm_hdapli.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_hdapli.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_hdapli.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_hdapli.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_hdapli.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_hdapli.Left:= adm_hdapli.Left-(mv_mouse.x-x);
      adm_hdapli.Top:= adm_hdapli.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_hdapli.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_hdapli.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_hdapli.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_hdapli.bt_maximizaClick(Sender: TObject);
begin
    if adm_hdapli.WindowState=wsMaximized then adm_hdapli.WindowState:=wsNormal
    else adm_hdapli.WindowState:=wsMaximized;
end;

procedure Tadm_hdapli.FormResize(Sender: TObject);
begin
...
procedure Tadm_hdapli.bt_newClick(Sender: TObject);
begin
    addreg;
    lbcp2.SetFocus;
end;

procedure Tadm_hdapli.bt_delClick(Sender: TObject);
begin
    if (dm.ds_sql3.RecordCount>0) and (lbcp1.Text<>'novo') then begin
        if dm.deleta_reg(EnDecryptString(Helpdesk.vrf04,37249),'codigo',lbcp1.Text,true) then begin
           dm.ds_sql3.Close;
           dm.ds_sql3.Open;
           atualiza_grade;
        end;
...
procedure Tadm_hdapli.bt_saveClick(Sender: TObject);
var axcod : integer;
begin
     if lbcp1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Helpdesk.vrf04,37249),1);
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Helpdesk.vrf04,37249)+' (codigo,descricao,obs) '+
                       'VALUES(:p0,:p1,:p2)';
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Helpdesk.vrf04,37249)+' SET descricao=:p0,obs=:p1 '+
                       'WHERE codigo=:p2';
...
procedure Tadm_hdapli.bt_nextClick(Sender: TObject);
begin
    dm.ds_sql3.Next;
    atualiza_grade;
end;

procedure Tadm_hdapli.bt_priorClick(Sender: TObject);
begin
    dm.ds_sql3.Prior;
    atualiza_grade;
end;

procedure Tadm_hdapli.Edit_searchChange(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_hdapli.filtrar_reg;
begin
...
procedure Tadm_hdapli.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_hdapli.ComboBox_searchChange(Sender: TObject);
begin
...
procedure Tadm_hdapli.Edit_searchKeyPress(Sender: TObject;
  var Key: Char);
begin
