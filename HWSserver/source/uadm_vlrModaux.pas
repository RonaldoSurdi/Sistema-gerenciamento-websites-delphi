...
procedure Tadm_vlrModaux.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     dm.ds_sql3.Filter:='';
...
procedure Tadm_vlrModaux.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_vlrModaux.FormCreate(Sender: TObject);
...
begin
     ...
     DBGrid1.Columns.Add;
     DBGrid1.Columns.Items[2].FieldName:='cod_moe';
     DBGrid1.Columns.Items[2].Title.Caption:='Moeda';
     DBGrid1.Columns.Add;
     DBGrid1.Columns.Items[3].FieldName:='vlr';
     DBGrid1.Columns.Items[3].Title.Caption:='Valor Instalação';
     DBGrid1.Columns.Add;
     DBGrid1.Columns.Items[4].FieldName:='vlr2';
     DBGrid1.Columns.Items[4].Title.Caption:='Valor Mensal';
     With dm.ds_sql3 do begin
        CommandText:= 'SELECT codigo,descricao,cod_moe,vlr,vlr2 FROM '+EnDecryptString(Serverconect.vrf46,37249)+' ORDER BY codigo';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
procedure Tadm_vlrModaux.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tadm_vlrModaux.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_vlrModaux.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_vlrModaux.atualiza_grade;
var axshowing: Boolean;
begin
      axshowing:= loadx.Showing;
      if not axshowing then begin
...
procedure Tadm_vlrModaux.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_vlrModaux.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_vlrModaux.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_vlrModaux.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_vlrModaux.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_vlrModaux.Left:= adm_vlrModaux.Left-(mv_mouse.x-x);
      adm_vlrModaux.Top:= adm_vlrModaux.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_vlrModaux.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_vlrModaux.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_vlrModaux.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_vlrModaux.bt_maximizaClick(Sender: TObject);
begin
    if adm_vlrModaux.WindowState=wsMaximized then adm_vlrModaux.WindowState:=wsNormal
    else adm_vlrModaux.WindowState:=wsMaximized;
end;

procedure Tadm_vlrModaux.FormResize(Sender: TObject);
begin
...
procedure Tadm_vlrModaux.bt_saveClick(Sender: TObject);
var axcod : integer;
    axvlr,axvlr2: Real;
begin
     ...
     if lbcp1.Text = 'nulo' then begin
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf46,37249)+' SET cod_moe=:p0,vlr=:p1,vlr2=:p2 '+
                       'WHERE codigo=:p3';
...
procedure Tadm_vlrModaux.bt_nextClick(Sender: TObject);
begin
    dm.ds_sql3.Next;
    atualiza_grade;
end;

procedure Tadm_vlrModaux.bt_priorClick(Sender: TObject);
begin
    dm.ds_sql3.Prior;
    atualiza_grade;
end;

procedure Tadm_vlrModaux.Edit_searchChange(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_vlrModaux.filtrar_reg;
begin
...
procedure Tadm_vlrModaux.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_vlrModaux.ComboBox_searchChange(Sender: TObject);
begin
...
procedure Tadm_vlrModaux.Edit_searchKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tadm_vlrModaux.lbcp3Change(Sender: TObject);
begin
     if lbcp3.Text <> '' then
...
procedure Tadm_vlrModaux.lbcp4Change(Sender: TObject);
begin
...
procedure Tadm_vlrModaux.NumKeyPress(Sender: TObject; var Key: Char);
begin
     if Key = '.' then key:=',';
     Case key of
       '0'..'9' :;
