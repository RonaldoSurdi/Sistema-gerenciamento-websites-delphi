...
uses
  ...
  uadm_ramosatv in 'uadm_ramosatv.pas' {adm_ramosatv};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Ramos de atividade'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Ramos de atividade';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_ramosatv, adm_ramosatv);
          end else begin
