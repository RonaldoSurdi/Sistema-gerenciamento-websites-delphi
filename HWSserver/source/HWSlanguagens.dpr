...
uses
  ...
  uadm_languagens in 'uadm_languagens.pas' {adm_languagens};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Languagens'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Languagens';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_languagens, adm_languagens);
          end else begin
