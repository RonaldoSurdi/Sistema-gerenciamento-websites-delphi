...
uses
  ...
  uadm_modulos in 'uadm_modulos.pas' {adm_modulos};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Módulos Client'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Módulos Client';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_modulos, adm_modulos);
          end else begin
