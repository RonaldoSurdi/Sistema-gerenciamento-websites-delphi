...
procedure Tadm_help.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     xmn_mdl_ext.Free;
     dm.ds_sql6.Filter:='';
...
procedure Tadm_help.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_help.FormCreate(Sender: TObject);
...
begin
     ...
     With dm.executa3 do begin
        CommandText:='SELECT codigo,tp FROM '+EnDecryptString(HelpSys.vrf03[0],37249)+' ORDER BY codigo';
        Open;
     end;
...
procedure Tadm_help.montalng_menus;
begin
     loadx := Tloadx.Create(Application);
...
procedure Tadm_help.atualiza_xmenumnt;
begin
     //monta menu
...
function Tadm_help.xmn_montamenus_treeview:Boolean;
var xformn: Integer;
    MenuRecPtr: PmenuRec;
begin
    Result:=False;
...
procedure Tadm_help.atualiza_nitens;
begin
     Lb_countreg4.Caption:=IntToStr(xmn_montamenus1.Items.Count);
end;

procedure Tadm_help.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_help.lb_titleClick(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_help.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_help.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_help.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_help.Left:= adm_help.Left-(mv_mouse.x-x);
      adm_help.Top:= adm_help.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_help.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_help.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_help.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_help.bt_maximizaClick(Sender: TObject);
begin
    if adm_help.WindowState=wsMaximized then adm_help.WindowState:=wsNormal
    else adm_help.WindowState:=wsMaximized;
end;

procedure Tadm_help.FormResize(Sender: TObject);
begin
...
procedure Tadm_help.barraMoved(Sender: TObject);
begin
      bt_barra.Left:=barra.Left;
...
procedure Tadm_help.bt_barraClick(Sender: TObject);
begin
...
procedure Tadm_help.Localizar1Click(Sender: TObject);
begin
      //localizar
      FindDialog1.Execute;
end;

procedure Tadm_help.filtrar_reg;
begin
     //filtrar e localizar intens
     xmn_montamenus2.Items.Clear;
end;

procedure Tadm_help.destructionimg;
...
function Tadm_help.retorna_ImgStream(xcodref: Integer; aximgstr: TMemoryStream): Integer;
begin
     Result:=-1;
...
procedure Tadm_help.abretmplogo;
var erro: Boolean;
    ax_filenm: TFileName;
    isshowing: Boolean;
begin
...
procedure Tadm_help.ProgresTimerTimer(Sender: TObject);
begin
  if pn_mp3player.Visible then begin
...
procedure Tadm_help.bt_playmp3Click(Sender: TObject);
begin
      mp3player.Play;
end;

procedure Tadm_help.bt_stopmp3Click(Sender: TObject);
begin
      mp3player.Stop;
end;

procedure Tadm_help.bt_closemp3Click(Sender: TObject);
var ax_filenm: string;
begin
      mp3player.Close;
      pn_mp3player.Visible:=false;
...
procedure Tadm_help.xmn_montamenusEditar;
        ...
        if xcoditem>0 then begin
           ...
           With dm.executa4 do begin
                ...
                for xforimg:=0 to editorlng.tmpimglst.Count-1 do begin
                    ...
                    if dm.executa3.Locate('codigo',xforimg_cod,[]) then begin
                       if dm.executa3.FieldByName('tp').AsInteger=2 then
                          incluiimgeditor(xforimg_cod,xforimg_lin)
                       else begin
                          if playList.count <=200 then begin
                             playList.codigo[playList.count]:=xforimg_cod;
                             playList.descricao[playList.count]:=dm.executa3.fieldbyname('descricao').asstring;
...
function Tadm_help.localiza_mdl(xcodmdl:Integer):Integer;
...
procedure Tadm_help.xmn_montamenus1Change(Sender: TObject;
  Node: TTreeNode);
begin
      if ax_searh then exit;
      xnoteselect:= Node;
...
procedure Tadm_help.xmn_montamenus1Click(Sender: TObject);
begin
      xmn_montamenusEditar;
end;

function Tadm_help.BitmapToRtf(graph:TBitmap):string;
...
procedure Tadm_help.incluiimgeditor(xcodimg,xlinha: Integer);
...
procedure Tadm_help.executamultimedia(xinitcount: Integer);
begin
    loadx := Tloadx.Create(Application);
    loadx.lbloading.Caption:='Carregando';
    loadx.Show;
...
procedure Tadm_help.re_editorURLClick(Sender: TObject;
...
procedure Tadm_help.FindDialog1Find(Sender: TObject);
var FoundAt: LongInt;
...
procedure Tadm_help.FormActivate(Sender: TObject);
begin
      adm_help.Refresh;
      if xinit then begin
         montalng_menus;
...
procedure Tadm_help.bt_searchClick(Sender: TObject);
...
procedure Tadm_help.SpeedButton1Click(Sender: TObject);
begin
      FindDialog1.Execute;
end;

procedure Tadm_help.xmn_montamenus2Change(Sender: TObject;
  Node: TTreeNode);
begin
      if ax_searh then exit;
      xnoteselect:= Node;
...
procedure Tadm_help.xmn_montamenus2Click(Sender: TObject);
begin
      xmn_montamenusEditar;
end;

procedure Tadm_help.Edit_searchKeyPress(Sender: TObject; var Key: Char);
begin
