...
uses
  ...
  umdl_lstcampos in 'umdl_lstcampos.pas' {mdl_lstcampos};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWSlstcampos Configurações'));
    if HprevHist = 0 then begin
    Application.Title := 'HWSlstcampos Configurações';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tmdl_lstcampos, mdl_lstcampos);
          end else begin
