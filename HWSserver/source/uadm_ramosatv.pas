...
Módulo: Cadastro de ramos de atividade.
...
procedure Tadm_ramosatv.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     dm.ds_sql3.Filter:='';
...
procedure Tadm_ramosatv.bt_fecharClick(Sender: TObject);
begin
    close;
end;

procedure Tadm_ramosatv.FormCreate(Sender: TObject);
...
begin
     ...
     With dm.ds_sql3 do begin
        CommandText:= 'SELECT * FROM '+EnDecryptString(Serverconect.vrf31,37249)+' ORDER BY codigo';
        Open;
        Filter:='';
        Filtered:=true;
     end;
...
procedure Tadm_ramosatv.DBGrid1CellClick(Column: TColumn);
begin
      atualiza_grade;
end;

procedure Tadm_ramosatv.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_ramosatv.DBGrid1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      atualiza_grade;
end;

procedure Tadm_ramosatv.atualiza_grade;
begin
...
procedure Tadm_ramosatv.addreg;
begin
     lbcp1.Text:='novo';
     lbcp2.Text:= '';
     lbcp3.Clear;
...
procedure Tadm_ramosatv.bt_sobreClick(Sender: TObject);
begin
      hwsf.BtSobre;
end;

procedure Tadm_ramosatv.YuSoftLabel2Click(Sender: TObject);
begin
      JumpTo('www.hws.com.br');
end;

procedure Tadm_ramosatv.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
      SetCapture(adm_ramosatv.Handle);
      cp_mouse := true;
      mv_mouse.X := x;
      mv_mouse.Y := Y;
end;

procedure Tadm_ramosatv.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if cp_mouse then begin
      adm_ramosatv.Left:= adm_ramosatv.Left-(mv_mouse.x-x);
      adm_ramosatv.Top:= adm_ramosatv.Top - (mv_mouse.y-y);
  end;
end;

procedure Tadm_ramosatv.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if cp_mouse then begin
      ReleaseCapture;
...
procedure Tadm_ramosatv.bt_minimizeClick(Sender: TObject);
begin
     Application.Minimize;
end;

procedure Tadm_ramosatv.tabtoenterpress(var Msg: TMsg; var Handled: Boolean);
begin
...
procedure Tadm_ramosatv.bt_maximizaClick(Sender: TObject);
begin
    if adm_ramosatv.WindowState=wsMaximized then adm_ramosatv.WindowState:=wsNormal
    else adm_ramosatv.WindowState:=wsMaximized;
end;

procedure Tadm_ramosatv.FormResize(Sender: TObject);
begin
...
procedure Tadm_ramosatv.bt_newClick(Sender: TObject);
begin
    addreg;
    lbcp2.SetFocus;
end;

procedure Tadm_ramosatv.bt_delClick(Sender: TObject);
begin
    if (dm.ds_sql3.RecordCount>0) and (lbcp1.Text<>'novo') then begin
        if dm.deleta_reg(EnDecryptString(Serverconect.vrf31,37249),'codigo',lbcp1.Text,true) then begin
           dm.ds_sql3.Close;
           dm.ds_sql3.Open;
           atualiza_grade;
        end;
...
procedure Tadm_ramosatv.bt_saveClick(Sender: TObject);
var axcod : integer;
begin
     if lbcp1.Text = 'novo' then begin
        axcod:=dm.GetCodMax(EnDecryptString(Serverconect.vrf31,37249),1);
        with dm.executa do begin
            CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf31,37249)+' (codigo,descricao,obs) '+
                       'VALUES(:p0,:p1,:p2)';
        ...
        with dm.executa do begin
            CommandText:= 'UPDATE '+EnDecryptString(Serverconect.vrf31,37249)+' SET descricao=:p0,obs=:p1 '+
                       'WHERE codigo=:p2';
...
procedure Tadm_ramosatv.bt_nextClick(Sender: TObject);
begin
    dm.ds_sql3.Next;
    atualiza_grade;
end;

procedure Tadm_ramosatv.bt_priorClick(Sender: TObject);
begin
    dm.ds_sql3.Prior;
    atualiza_grade;
end;

procedure Tadm_ramosatv.Edit_searchChange(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_ramosatv.filtrar_reg;
begin
...
procedure Tadm_ramosatv.bt_searchClick(Sender: TObject);
begin
     filtrar_reg;
end;

procedure Tadm_ramosatv.ComboBox_searchChange(Sender: TObject);
begin
...
procedure Tadm_ramosatv.Edit_searchKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tadm_ramosatv.bt_updateClick(Sender: TObject);
var axshowing: Boolean;
    axupdt: TStringList;
...
begin
      ...
      if OpenDialog_update.Execute then begin
         ...
         with dm.executa do begin
             CommandText:= 'DELETE FROM '+EnDecryptString(Serverconect.vrf31,37249);
             try
                  dm.executa.Execute;
             except
                  if not axshowing then loadx.Free;
         ...
         for xidupdt:=0 to axupdt.Count-1 do begin
             ...
             if xcodinc > 0 then begin
                with dm.executa do begin
                   CommandText:= 'INSERT INTO '+EnDecryptString(Serverconect.vrf31,37249)+' (codigo,descricao) '+
                                 'VALUES(:p0,:p1)';
