...
uses
  ...
  uadm_admpass in 'uadm_admpass.pas' {adm_admpass};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS user password'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS user password';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_admpass, adm_admpass);
          end else begin
