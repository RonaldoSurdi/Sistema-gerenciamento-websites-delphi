...
uses
  ...
  uadm_colsetores in 'uadm_colsetores.pas' {adm_colsetores};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Setores'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Setores';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_colsetores, adm_colsetores);
          end else begin
