...
program HWSphotogalerycfgDivulg;
...
uses
  ...
  umdl_photogalerycfgDivulg in 'umdl_photogalerycfgDivulg.pas' {mdl_photogalerycfgDivulg};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Configurações para Banners Divulgação'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Configurações para Banners Divulgação';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tmdl_photogalerycfgDivulg, mdl_photogalerycfgDivulg);
          end else begin
