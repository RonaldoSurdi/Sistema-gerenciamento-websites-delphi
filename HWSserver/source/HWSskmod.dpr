...
uses
  ...
  uadm_skmdl in 'uadm_skmdl.pas' {adm_skmdl};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS SK - Módulos auxiliares'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS SK - Módulos auxiliares';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_skmdl, adm_skmdl);
          end else begin
