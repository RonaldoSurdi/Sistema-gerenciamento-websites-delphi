...
uses
  ...
  uadm_usradm in 'uadm_usradm.pas' {adm_usradm};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS administradores'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS administradores';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_usradm, adm_usradm);
          end else begin
