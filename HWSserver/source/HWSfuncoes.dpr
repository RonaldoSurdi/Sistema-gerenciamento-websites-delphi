...
uses
  ...
  uadm_colfuncoes in 'uadm_colfuncoes.pas' {adm_colfuncoes};
...
begin
    ...
    HprevHist := FindWindow(Nil, PChar('HWS Funções'));
    if HprevHist = 0 then begin
    Application.Title := 'HWS Funções';
    ...
    Try
       ...
       if (ParamStr(1) <> '') and (ParamStr(2) = 'Å?Ö¤¶•') and (ParamStr(3) <> '') then begin
          if dm.conecta_bd(true,true,ParamStr(1),ParamStr(3)) then begin
             Application.CreateForm(Tadm_colfuncoes, adm_colfuncoes);
          end else begin
